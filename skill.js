...
const skills = {
	...
	**/
	...
	"sayaka_qiangyin": {
		...
		async content(event, trigger, player) {
			...
			if (result.bool) {
				...
				player.addTempSkill("sayaka_wuwei", { player: "phaseBeginStart" })
				player.storage.sayaka_wuwei = target;

				target.addTempSkill("sayaka_wuwei", { player: "dieAfter" });

				player.markSkill("sayaka_wuwei");
				target.markSkill("sayaka_wuwei");
			};
		},
		ai: {
			order: 9,
			result: {
				player(player) {
					if (game.hasPlayer(target => {
						return target != player && get.attitude(player, target) > 0 && target.isDamaged();
					}))
						return 1;
					return -1;
				},
	...
	"sayaka_wuwei": {
		...
		frequent: true,
		onremove(player) {
			player.storage.sayaka_wuwei?.removeSkill("sayaka_wuwei");
			delete player.storage.sayaka_wuwei;
		},
		intro: {
	...
	"yachiyo_zhishui": {
		...
		direct: true,
		async content(event, trigger, player) {
			const result = await player.chooseTarget(get.prompt2("yachiyo_zhishui"), function (card, player, target) {
				return player != target;
			}).set("ai", function (target) {
				const player = _status.event.player;
				...
				if (att > 0) {
					const skillck = target.hasSkillTag("noturnOver") ? 2 : 0;
					if (target.isTurnedOver()) {
						if (target.hasSkill("sana_touming_3")) {
							let rp = current;
							for (let i = 0; i < 3; i++) {
								rp = rp.getNext();
								if (rp == target)
									return att + 40 + skillck;
							}
							return att + 10 + skillck;
						}
						return att + 60 + skillck
					}
					if (draw < 3)
						return -1 + skillck;
					if (draw == 3) {
						const rcct = player.countCards("h", card => {
							const name = get.name(card);
							return (name == "tao" || name == "jiu") && player.canUse(card, player);
						})
						if (player.hp == 1 && rcct == 0)
							return att + 50 + skillck;
						return -1 + skillck;
					}
					if (draw > 3) {
						if (current == target)
							return att + 40 + skillck;
						return att + 8 + draw + skillck;
					}
				} else {
					const cards = target.countCards("h");
					const skillck2 = target.hasSkill("yachiyo_zhishui_mark") ? 5 : 0
					const skillck = skillck2 + (get.damageEffect(target, player) > 0 ? 0 : -4);
					if (target.hp == 1 && get.damageEffect(target, player) > 0)
						return -att + 40 + Math.min(cards, draw) + skillck;
					if (target.isTurnedOver() && draw <= 3)
						return -att + 7 + Math.min(cards, draw) + skillck;
					if (draw <= 3)
						return Math.max(-att + 10 + skillck2 + (target.hasSkillTag("noturnOver") ? -11 : 0), 0.1)
					if (draw > 3)
						return -att + 8 + Math.min(cards, draw) + skillck;
				}
			}).forResult();

			if (!result.bool || result.targets.length == 0) return;

			player.logSkill("yachiyo_zhishui", result.targets);
			const target = result.targets[0];
			const n = player.getDamagedHp()
			const str1 = "翻面并摸" + n + "张牌"
			const str2 = "弃" + n + "张牌并失去1点体力"
			const choice = await player.chooseControl([str1, str2])
				.set("target", target)
				.set("ai", () => {
					const att = get.attitude(player, target);
					if (att >= 0)
						...
						if (n <= 3 && !target.isTurnedOver() && !target.hasSkillTag("noturnOver") && !(target.hp == 1 && get.damageEffect(target, player) > 0))
							return str1;
						else
							return str2;
					}
			...
			switch (choice) {
				case str1:
					...
					await target.draw(n);
					target.addSkill("yachiyo_zhishui_mark");
					target.addMark("yachiyo_zhishui", 1);
					break;
				case str2:
					await target.chooseToDiscard(n, "he", true).set("ai", card => {
						return skills.duexcept_ai(20 - get.value(card), card, player);
					});
					await target.loseHp();
					break;
			}
		...
		intro: {
			name: "止水",
			content: "你响应七海八千代的【杀】或【决斗】时需要使用或打出两张【闪】或【杀】。",
		},
		ai: {
			maixie: true,
			maixie_hp: true,
	...
	"yachiyo_jueyu": {
		trigger: { source: "damageBegin1" },
		filter(event, player) {
			return event.player.hasMark("yachiyo_zhishui");
		},
		async cost(event, trigger, player) {
			event.result = await player.chooseBool(get.prompt2("yachiyo_jueyu"))
		...
		async content(event, trigger, player) {
			trigger.player.removeMark("yachiyo_zhishui", 1, false);
			trigger.num++;
		},
		group: ["yachiyo_jueyu_sha", "yachiyo_jueyu_juedou"],
		...
		subSkill: {
			sha: {
				...
				forced: true,
				filter(event, player) {
					return event.card.name == "sha" && !event.getParent().directHit.includes(event.target) && event.target.hasMark("yachiyo_zhishui");
				},
				logTarget: "target",
				async content(event, trigger, player) {
			...
			juedou: {
				...
				filter(event, player) {
					return event.card.name == "juedou" && event.target.hasMark("yachiyo_zhishui");
				},
				async content(event, trigger, player) {
	...
	"yachiyo_xiji": {
		...
		async content(event, trigger, player) {
			...
			const { result } = await player.chooseBool("是否获得" + get.translation(trigger.player) + "所有牌？")
				.set("ai", () => {
					let totalValue = 0;
					cards.forEach(card => {
						totalValue += get.value(card);
					});
					if (totalValue <= 0) return false;
					if (totalValue > 24) return true;
					const duct = cards.filter(card => { return card.name == "du" }).length + player.countCards("h", card => { return card.name == "du" });
					const taoct = cards.filter(card => { return card.name == "tao" }).length + player.countCards("h", card => { return card.name == "tao" });
					return totalValue > 0 && duct < Math.min(player.maxHp, taoct + player.hp);
				})
			if (result.bool) {
				await player.gain(cards, trigger.player, "giveAuto", "bySelf");
			}
		},
		group: ["yachiyo_xiji_draw"],
		subSkill: {
			draw: {
	...
	"yachiyo_gujun": {
		...
		async cost(event, trigger, player) {
			// 防止其他势力触发
			if (trigger.player.group != "Kamihama_Magia_Union") return false;
	...
	"homura2_jihuo": {
		init(player) {
			if (player.storage.homura2_jihuo3?.length >= 4)
				player.useSkill("homura2_jihuo")
		},
		async content(event, trigger, player) {
			await player.draw(2);
			await player.removeSkill("homura2_jihuo3");
			...
			for (let name of lib.inpile) {
				...
				const card = { name: name, isCard: true, storage: { homura2_jihuo2: true } };
				if (
		...
		mod: {
			cardUsable(card, player, num) {
				if (card.storage?.homura2_jihuo2) {
					return Infinity;
				}
			},
		},
			...
			discard: {
				...
				async cost(event, trigger, player) {
					...
					for (var i = 0; i < cards.length; i++) {
						if (cards[i].original != "j" && !player.storage.homura2_jihuo_used?.includes(get.color(cards[i], player)) && get.position(cards[i], true) == "d")
					...
					const result = await player.chooseButton(["集火：选择要获得的颜色各不相同的牌", validCards], [1, validCards.length])
						...
						.set("ai", function (button) {
							if (player.storage.homura2_jihuo3?.includes(get.suit(button.link)))
				...
				async content(event, trigger, player) {
					...
					if (cards?.length) {
						...
						for (let i = 0; i < cards.length; i++) {
							if (!player.storage.homura2_jihuo3?.includes(get.suit(cards[i]))){
								await player.addSkill("homura2_jihuo3");
								await player.markAuto("homura2_jihuo3", get.suit(cards[i]))
							}
							if (!player.storage.homura2_jihuo_used?.includes(get.color(cards[i]))){
								await player.addTempSkill("homura2_jihuo_used", "phaseAfter");
								await player.markAuto("homura2_jihuo_used", get.color(cards[i]));
							}
						}
						if (player.storage.homura2_jihuo3?.length >= 4)
							await player.useSkill("homura2_jihuo");
					}
				}
			},
			judge: {
				...
				async cost(event, trigger, player) {
					...
					if (get.position(trigger.cards[0], true) != "d" || player.storage.homura2_jihuo_used?.includes(get.color(trigger.cards[0], player)))
						return false;
	...
	"homura2_juhuo": {
		...
		hiddenCard(player, name) {
			return name == "huogong" && player.hasCard(card => {
				return !player.storage.homura2_juhuo_used?.includes(get.suit(card));
			}, "he");
		},
		filterCard(card, player) {
			return !player.storage.homura2_juhuo_used?.includes(get.suit(card));
		...
		prompt: "将一张牌当火攻使用",
		async precontent(event, trigger, player) {
			player.addTempSkill("homura2_juhuo_used", "phaseAfter");
			player.markAuto("homura2_juhuo_used", get.suit(event.result.cards[0]));
		},
		check(card) {
			...
			effect: {
				...
				async content(event, trigger, player) {
					...
					for (var i = 0; i < suits.length; i++) {
						if (!player.storage.homura2_jihuo3?.includes(suits[i]))
							ran.push(suits[i]);
					}
					...
					if (ran.length) {
						...
						for (var i = 0; i < ran.length; i++)
							await player.markAuto("homura2_jihuo3", ran[i])
					}
						
					
					if (player.storage.homura2_jihuo3?.length >= 4 && player.hasSkill("homura2_jihuo"))
						await player.useSkill("homura2_jihuo");
				},
			},
		},
	...
	"homura2_liandan": {
		...
		hiddenCard(player, name) {
			return name == "tiesuo" && player.hasCard(card => {
				return !player.storage.homura2_liandan_used?.includes(get.suit(card));
			}, "he");
		},
		filterCard(card, player) {
			return !player.storage.homura2_liandan_used?.includes(get.suit(card));
		...
		prompt: "将一张牌当铁索连环使用",
		async precontent(event, trigger, player) {
			player.addTempSkill("homura2_liandan_used", "phaseAfter");
			player.markAuto("homura2_liandan_used", get.suit(event.result.cards[0]));
		},
		ai1(card) {
	...
	"felicia_yongbing": {
		...
		filter(event, player) {
			return event.source && event.num >= 2;
		},
		forced: true,
		preHidden: true,
		async content(event, trigger, player) {
			...
			2: {
				...
				mod: {
					globalFrom: function (from, to, current) {
						return from.hp <= 2 ? current - 2 : current - 1;
					},
				...
				const result = await player.chooseCardTarget({
					...
					goon: player.hasCard(function (card) {
						return get.type(card) == "equip" && game.hasPlayer(function (current) {
							return get.attitude(player, current) > 0 && current.hasEmptySlot(get.subtype(card)) && current.canUse(card, current)
						}), "h"
					}),
					ai1(card) {
						if (!_status.event.goon)
	...
	"sana_touming": {
		...
		subSkill: {
			2: {
				...
				async content(event, trigger, player) {
					const result = await player.chooseTarget("你选择一名角色：直到下个你的回合开始，只要你翻面，【杀】对其无效", true, function (card, player, target) {
					...
					player.storage.sana_touming_3 = result.targets[0];

					result.targets[0].storage.sana_touming_4 = player;
					result.targets[0].addTempSkill("sana_touming_4", { player: "dieAfter" });
			...
			4: {
				...
				mark: "character",
				intro: {
					content: "只要$翻面，【杀】对你无效",
				},
				onremove(player) {
					delete player.storage.sana_touming_4;
				},
				ai: {
					effect: {
						target: function (card, player, target) {
							if (card.name == "sha" && target.getStorage("sana_touming_4").isTurnedOver()) return "zeroplayertarget";
						},
					},
				},
			}
	...
	"mabayu_jingying":{
		...
		intro: {
			content(storage) {
				...
				return "阴：你可以跳过出牌阶段，然后选择一名其他角色，于其回合结束后其立刻进行一个没有弃牌阶段的完整回合，其获得你所有黑色手牌。或者不跳过出牌阶段受到一点伤害。";
			}
		},
		async content(event, trigger, player) {
        	if (player.storage.mabayu_jingying){
				...
				player.skip("phaseDiscard")
					...
					const result = await player.chooseTarget("镜影：跳过出牌阶段并选择一名其他角色，其将在其回合结束后立刻进行一个回合", lib.filter.notMe)
						.set("ai", target => {
		...
		subSkill: {
			...
			dis: {
				...
				ai: {
					pretao: true,
					value(card, player){
						if(player.storage.mabayu_jingying_dis == player.countCards("h"))
							return get.value(card) - 99999
						return get.value(card)
					}
				}
			}
	...
	"mabayu_jingxiang": {
		...
		async content(event, trigger, player) {
			...
			if (player != target) {
				const n = player.hp
				await player.damage(player.hp - 1)
				await target.recoverTo(Math.min(n, target.maxHp))
			} else {
				...
				for (const current of players) {
					...
					const result = await current.chooseBool(
						"镜像：是否受到"+ (current.hp - 1) +"点伤害，让" + get.translation(player) + "回复至" + Math.min(player.maxHp, current.hp)
					).set("ai", () => {
					...
					if (result?.bool) {
						const n = current.hp
						await current.damage(current.hp - 1)
						await player.recoverTo(Math.min(n, player.maxHp))
						break;
					}
				}
			}
			...
			gs:{
				...
				async content(event, trigger, player) {
					...
					const result = await player.chooseControl([skill1, skill2])
						...
						.set("choiceList", [
							`【甜噬】当场上一名其他角色回复体力时，①每回合各限一次，你可以选择一项：1.令其摸两张牌。2.令其交给你一张非【毒】牌。②若其体力与你相等，你摸两张牌。`,
							`【天惠】每回合限一次，当场上有角色的牌因弃置进入弃牌堆后，你可以选择(X为弃置牌数量)：1.若X=1，你可以令其获得与弃牌花色相同的一张牌。2.若X>=2，你可以令其回复一点体力。3.你摸X张牌（至多为5）。`
						])
						.set("ai", () => {
	...
	"nagisa_tianhui":{
		...
		async cost(event, trigger, player) {
			...
			const str3 = "自己摸" + get.cnNumber(Math.min(num, 5)) + "张牌"

			let choice = "选项三"
			if (num >= 5)
				choice = "选项三"
			else if (f2 && recoveratt >= 0)
				choice = "选项二"
	...
	"kanagi_huishi": {
		...
		content() {
			...
			player
				.judge(function (result) {
					var evt = _status.event.getParent("kanagi_huishi");
					if (evt && evt.suits && evt.suits.includes(get.suit(result))) {
						return 0;
					}
					return 1;
				})
				.set("callback", lib.skill.kanagi_huishi.callback).judge2 = function (result) {
		...
		callback() {
			...
			if (event.getParent().result.bool && player.maxHp < 10) {
				...
				player.gainMaxHp();
				player.chooseBool("是否继续发动【慧识】？").set("frequentSkill", "kanagi_huishi");
			} else {
	...
	"kanagi_duxin": {
		...
		async content(event, trigger, player) {
			...
			if (!target || !target.countCards("he")) {
				...
				let card = await player.chooseCardButton(target, target.getCards("he")).forResult();
				if (card.bool) {
	...
	"tsuruno_jizhi": {
		derivation: ["tsuruno_jiang", "tsuruno_yingfa", "tsuruno_yanzhan", "tsuruno_yanwu"],
		trigger: { player: "changeHp" },
		firstDo: true,
		silent: true,
		init2(player) {
			player.removeSkill(["tsuruno_jiang", "tsuruno_yingfa", "tsuruno_yanzhan", "tsuruno_yanwu"]);
			if (player.hp <= 4)
				player.addSkill("tsuruno_jiang");
			if (player.hp <= 3)
		...
		subSkill: {
			juedou: {
				...
				ai: {
					maixie: true,
					skillTagFilter(player, tag, target) {
						if (player.hp <= 2 && tag == "maixie") return false;
					},
					order: 6,
					result: {
						target(player, target) {
		...
		async content(event, trigger, player) {
			if (event.cards) {
				...
				if (get.suit(card) == "diamond") {
					let result = await player.chooseTarget("请选择【乐不思蜀】的目标", true, function (card, player, target) {
						return target != player;
					}).set("ai", function (target) {
				...
				filter(event, player) {
					...
					return (
						...
						player.getHistory("gain", function (evt) {
							return evt.getParent(2).name == "mami_zhongmu_draw";
						}).length < game.countPlayer()
					);
				},
				content() {
					player.draw();
