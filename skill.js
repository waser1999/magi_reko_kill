...
const skills = {
   ...
   //真相触发器
	...
	"nanaka_huaxin": {
		...
		ai: {
			...
			effect: {
				target: function(card, player, target, current) {
					if (player === target && get.type(card) === "equip" && !get.cardtag(card, "gifts")) {
						var lost = player.storage.nanaka_huaxin_lost_types || [];
						var subtype = get.subtype(card);
						var hasConflict = player.getEquip(subtype);

						if (lost.length === 2 && !lost.includes("equip")) {
							return [1, 5];
						}
						if (player.storage.nanaka_huaxin_type === "equip" && player.storage.nanaka_huaxin_turn !== game.phaseNumber) {
							if (hasConflict) return [1, 4];
						}
						if (hasConflict && !lost.includes("equip")) {
							return [1, 3];
						}
					}
				}
		...
		async content(event, trigger, player) {
			...
			const result = await player.chooseButton(
				["华心：选择一种牌的类型，直至下回合开始前生效", `<div class="text center">牌的类型</div>`, [typeList, "tdnodes"]]
			).set("selectButton", 1).set("forced", true).set("ai", button => {
				const player = _status.event.player;
				const type = button.link;
				const lost = player.storage.nanaka_huaxin_lost_types || [];
				const count = player.countCards("h", card => (get.type(card) === 'delay' ? 'trick' : get.type(card)) === type);

				if (lost.length === 2 && !lost.includes(type)) {
			...
			if (result.bool && result.links?.length) {
				player.storage.nanaka_huaxin_type = result.links[0];
				player.storage.nanaka_huaxin_lost_types = [];
				player.markSkill("nanaka_huaxin_tracker");
				game.log(player, '选择了', '#y' + get.translation(result.links[0]) + '牌');
			}
		},
		subSkill: {
			tracker: {
				trigger: { global: ["loseAfter", "loseAsyncAfter", "equipAfter", "addJudgeAfter"] },
				filter: function(event, player) {
					if (event.player !== player) return false;
					if (!player.storage.nanaka_huaxin_type) return false;
					if (event.name === 'equip' || event.name === 'addJudge') return event.cards && event.cards.length > 0;
				...
				content: async function(event, trigger, player) {
					...
					var chosenType = player.storage.nanaka_huaxin_type;
					var lostTypes = player.storage.nanaka_huaxin_lost_types || [];
					var newTypes = types.filter(t => !lostTypes.includes(t));

					var shouldDraw2 = false;
					if (chosenType && types.includes(chosenType) && player.storage.nanaka_huaxin_turn !== game.phaseNumber) {
						shouldDraw2 = true;
					}

					if (shouldDraw2 || newTypes.length > 0) {
						if (shouldDraw2) {
							player.storage.nanaka_huaxin_turn = game.phaseNumber;
							player.logSkill('nanaka_huaxin');
							game.log(player, '本回合首次失去了', '#y' + get.translation(chosenType) + '牌', '，触发了', '#g【华心】');
							await player.draw(2);
						}

						if (newTypes.length > 0) {
							lostTypes.addArray(newTypes);
							player.storage.nanaka_huaxin_lost_types = lostTypes;
							player.markSkill("nanaka_huaxin_tracker"); 
							
							if (lostTypes.length >= 3) {
								player.logSkill('nanaka_huaxin');
								game.log(player, '共计失去了三种类型的牌，触发了', '#g【华心】', '重置效果');
								await player.draw(1);
								player.storage.nanaka_huaxin_lost_types = [];
								delete player.storage.nanaka_huaxin_turn;
								
								const typeList = [["basic", "基本牌"], ["trick", "锦囊牌"], ["equip", "装备牌"]];
								const validTypes = typeList.filter(t => t[0] !== chosenType);
								const result = await player.chooseButton(
									["华心：重新选择一种牌的类型", `<div class="text center">牌的类型</div>`, [validTypes, "tdnodes"]]
								).set("selectButton", 1).set("forced", true).set("ai", button => {
									const player = _status.event.player;
									const type = button.link;
									return player.countCards("h", card => (get.type(card) === 'delay' ? 'trick' : get.type(card)) === type) + 1;
								}).forResult();

								if (result.bool && result.links?.length) {
									player.storage.nanaka_huaxin_type = result.links[0];
									player.markSkill("nanaka_huaxin_tracker");
									game.log(player, '重新选择了类型', '#y' + get.translation(result.links[0]) + '牌');
								}
							}
						}
					}
				...
				intro: {
					content: function(storage, player) {
						var type = player.storage.nanaka_huaxin_type;
						var lost = player.storage.nanaka_huaxin_lost_types || [];
						var str = "";
						if (type) str += "当前选择类型：<b>" + get.translation(type) + "牌</b><br>";
						str += "已失去类型：";
						if (lost.length) {
							str += lost.map(t => get.translation(t) + "牌").join("、");
						} else {
							str += "无";
						}
						return str;
			...
			clear: {
				...
				silent: true,
				content: function(event, trigger, player) {
					delete player.storage.nanaka_huaxin_type;
					delete player.storage.nanaka_huaxin_lost_types;
					delete player.storage.nanaka_huaxin_turn;
					player.unmarkSkill("nanaka_huaxin_tracker");
				}
			}
		}
	},
	...
	"nanaka_guanhuo": {
		...
		position: "h",
		viewAs: { name: "zhibi" },
		prompt: "将一张手牌当做【知己知彼】使用，结算后可弃置你与目标手牌中原牌花色、点数、类型相同的手牌各一张",
		check: function(card) {
			var player = _status.event.player;
			var type = get.type(card) === 'delay' ? 'trick' : get.type(card);
			var val = 8 - get.value(card);
			
			if (player.storage.nanaka_huaxin_type === type && player.storage.nanaka_huaxin_turn !== game.phaseNumber) {
				val += 50; 
			}
			return val;
		},
		ai: {
			order: function() {
				return get.order({ name: "zhibi" }) || 7.5;
			},
			result: {
				player: 1,
				target: function(player, target) {
					...
					return -2 - Math.min(target.countCards("h"), 3);
				}
			}
		},
		group: "nanaka_guanhuo_discard",
		subSkill: {
			discard: {
				trigger: { player: "useCardAfter" },
				filter: function(event, player) {
					return event.skill === "nanaka_guanhuo" && event.targets && event.targets.length > 0 && event.cards && event.cards.length > 0;
				},
				forced: true,
				popup: false,
				content: async function(event, trigger, player) {
					var target = trigger.targets[0];
					var origCard = trigger.cards[0];
					
					var origSuit = get.suit(origCard);
					var origNum = get.number(origCard);
					var origType = get.type(origCard) === 'delay' ? 'trick' : get.type(origCard);

					var myCards = player.getCards("h");
					var targetCards = target.getCards("h");

					var matchCond = c => {
						return (origSuit && origSuit !== 'none' && get.suit(c) === origSuit) ||
							   (origNum && get.number(c) === origNum) ||
							   (origType && (get.type(c) === 'delay' ? 'trick' : get.type(c)) === origType);
					};
					myCards = myCards.filter(matchCond);
					targetCards = targetCards.filter(matchCond);

					if (myCards.length === 0 && targetCards.length === 0) return;

					var dialogArgs = [
						`观火：可自由挑选弃置符合原牌“花色、点数、类型”的手牌，每种条件至多各占一张<br>原牌属性：【${get.translation(origSuit)}，点数 ${origNum}，${get.translation(origType)}牌】`
					];
					if (myCards.length) {
						dialogArgs.push(`<div class="text center">你的手牌</div>`);
						dialogArgs.push(myCards);
					}
					if (targetCards.length) {
						dialogArgs.push(`<div class="text center">${get.translation(target)}的手牌</div>`);
						dialogArgs.push(targetCards);
					}

					var chooseRes = await player.chooseButton(dialogArgs, [1, 3]).set("filterButton", function(button) {
						var cards = ui.selected.buttons.map(b => b.link);
						cards.push(button.link);
						
						var origSuit = _status.event.origSuit;
						var origNum = _status.event.origNum;
						var origType = _status.event.origType;

						var conditions = [];
						if (origSuit && origSuit !== 'none') conditions.push(c => get.suit(c) === origSuit);
						if (origNum) conditions.push(c => get.number(c) === origNum);
						if (origType) conditions.push(c => (get.type(c) === 'delay' ? 'trick' : get.type(c)) === origType);

						var usedCond = {};
						function dfs(index) {
							if (index === cards.length) return true;
							for (var i = 0; i < conditions.length; i++) {
								if (!usedCond[i] && conditions[i](cards[index])) {
									usedCond[i] = true;
						...
						return dfs(0);
					}).set("origSuit", origSuit).set("origNum", origNum).set("origType", origType)
					.set("myCards", myCards).set("targetCards", targetCards).set("target", target)
					.set("ai", function(button) {
						var evt = _status.event;
						
						if (!evt.bestComboCalculated) {
							evt.bestComboCalculated = true;
							evt.bestCombo = [];
							
							var allCards = evt.myCards.concat(evt.targetCards);
							var conditions = [];
							if (evt.origSuit && evt.origSuit !== 'none') conditions.push(c => get.suit(c) === evt.origSuit);
							if (evt.origNum) conditions.push(c => get.number(c) === evt.origNum);
							if (evt.origType) conditions.push(c => (get.type(c) === 'delay' ? 'trick' : get.type(c)) === evt.origType);

							var isValid = function(subset) {
								if (subset.length === 0) return true;
								var usedCond = {};
								function dfs(index) {
									if (index === subset.length) return true;
									for (var i = 0; i < conditions.length; i++) {
										if (!usedCond[i] && conditions[i](subset[index])) {
											usedCond[i] = true;
											if (dfs(index + 1)) return true;
											usedCond[i] = false;
										}
							...
							var getSubsets = function(array, maxLen) {
								var result = [[]];
								for (var i = 0; i < array.length; i++) {
									var len = result.length;
									for (var j = 0; j < len; j++) {
										var newSubset = result[j].concat(array[i]);
										if (newSubset.length <= maxLen) result.push(newSubset);
									}
								}
								return result;
							};
							
							var subsets = getSubsets(allCards, conditions.length);
							var maxScore = -10000;
							var bestSubset = [];
							...
							for (var i = 0; i < subsets.length; i++) {
								...
								if (isValid(subset)) {
									...
									for (var j = 0; j < subset.length; j++) {
										...
										var cVal = get.value(c);
										if (owner === evt.target) {
											var trgName = evt.target.name1 || evt.target.name;
											if (trgName === 'meiyui' || evt.target.name2 === 'meiyui' || evt.target.hasSkill('meiyui_canghai')) {
												if (get.subtype(c) === 'equip3' || get.subtype(c) === 'equip2') score += 15;
											}
											if (trgName === 'kako' || evt.target.name2 === 'kako' || evt.target.hasSkill('kako_xiamu')) {
												score += 10;
											}
											score += cVal;
										} else {
											var myCost = cVal;
											var p = evt.player;
											var cType = get.type(c) === 'delay' ? 'trick' : get.type(c);
											
											if (p.storage.nanaka_huaxin_type === cType && p.storage.nanaka_huaxin_turn !== game.phaseNumber) {
												myCost -= 20; 
											} else if (p.storage.nanaka_huaxin_type && p.storage.nanaka_huaxin_lost_types && !p.storage.nanaka_huaxin_lost_types.includes(cType)) {
												myCost -= 5; 
											}
											score -= myCost;
										}
							...
							evt.bestCombo = bestSubset;
						}
						
						return evt.bestCombo.includes(button.link) ? 100 : 0;
					}).forResult(); 

					if (chooseRes.bool && chooseRes.links && chooseRes.links.length > 0) {
						var toDiscardMe = chooseRes.links.filter(c => get.owner(c) === player);
						var toDiscardTarget = chooseRes.links.filter(c => get.owner(c) === target);

						if (toDiscardMe.length) await player.discard(toDiscardMe);
						if (toDiscardTarget.length) await target.discard(toDiscardTarget);
					}
				}
			}
		}
	...
	"meiyui_canghai": {
		...
		subSkill: {
			red: {
				...
				viewAs: { name: "sha", nature: "stab", isCard: true },
				viewAsFilter: function(player) {
					if (player.storage.meiyui_canghai_red_turn === game.phaseNumber) return false;
					return player.hasCards("he", { color: "red" });
				},
				prompt: "苍海：将一张红色牌当刺【杀】使用（每回合限一次）",
				check: function(card) {
					var player = _status.event.player;
					if (player.hp <= 1 && (get.name(card) === 'tao' || get.name(card) === 'shan')) return 0;
				...
				ai: {
					order: function() {
						var player = _status.event.player;
						var hasWeakEnemy = game.hasPlayer(function(current) {
							return get.attitude(player, current) < 0 && 
							       player.canUse({name: 'sha'}, current) && 
							       (current.hp <= 1 || current.countCards('h') <= 1);
						});
						if (hasWeakEnemy) return get.order({name: 'sha'}) + 0.3;
						return get.order({name: 'sha'});
					},
					result: {
						player: function(player, target) {
							if (player.hp <= 1 && player.countCards('h') <= 2) return 0;
							return 1;
						},
						target: function(player, target) {
							var att = get.attitude(player, target);
							if (att >= 0) return 0;
							var score = -1.5;
							if (player.inRange(target)) score -= 2.0;
							if (target.hp <= 1) score -= 3.0;
							if (target.countCards('h') <= 1) score -= 2.0;
							return score;
						}
					}
				}
			...
			black: {
				...
				position: "he",
				filter: function(event, player) {
					if (player.storage.meiyui_canghai_black_turn === game.phaseNumber) return false;
					if (!player.hasCards("he", { color: "black" })) return false;
					if (event.filterCard) {
						return event.filterCard({ name: "lve", isCard: true }, player, event) || 
				...
				viewAs: function(cards, player) {
					return { name: "lve", isCard: true, respondShan: true };
				},
				viewAsFilter: function(player) {
					if (player.storage.meiyui_canghai_black_turn === game.phaseNumber) return false;
					return player.hasCards("he", { color: "black" });
				},
				prompt: "苍海：将一张黑色牌当【掠】打出或响应（每回合限一次）",
				check: function(card) {
					var player = _status.event.player;
					if (player.hp <= 1) return 15 - get.value(card);
					if (get.position(card) === 'e') return 9 - get.value(card);
					return 7 - get.value(card);
				},
				ai: {
					respondShan: true,
					...
					result: {
						player: function(player) {
							var evt = _status.event;
							var attacker = evt.respondTo ? evt.respondTo[0] : null;
							var base = 1;
							if (player.hp <= 1) base += 2;
							if (attacker && attacker.isIn()) {
								if (player.inRange(attacker)) base += 2;
								if (attacker.countCards('e') > 0) {
									if (attacker.getEquip(2)) base += 2.5;
									if (attacker.getEquip(1)) base += 1.5;
								}
							}
							return base;
						}
			...
			strike: {
				...
				popup: false,
				filter: function(event, player) {
					if (event.skill !== "meiyui_canghai_red" && event.skill !== "meiyui_canghai_black") return false;
					var target = null;
					if (event.skill === "meiyui_canghai_red") {
						target = event.targets ? event.targets[0] : null;
					} else if (event.skill === "meiyui_canghai_black") {
						target = event.respondTo ? event.respondTo[0] : null;
					}
					return target && target.isIn() && player.inRange(target);
				},
				content: async function(event, trigger, player) {
					if (trigger.skill === "meiyui_canghai_red") {
						player.storage.meiyui_canghai_red_turn = game.phaseNumber;
					} else if (trigger.skill === "meiyui_canghai_black") {
						player.storage.meiyui_canghai_black_turn = game.phaseNumber;
					}

					var target = (trigger.skill === "meiyui_canghai_red") ? trigger.targets[0] : trigger.respondTo[0];
					player.logSkill("meiyui_canghai", target);
					game.log(player, "对", target, "触发了", "#g【搏击】");

					await player.draw(1);

					if (target.isIn() && target.countCards("he") > 0) {
						await player.discardPlayerCard(target, "he", "苍海·搏击：是否弃置" + get.translation(target) + "的一张牌？").set("ai", function(button) {
							return get.attitude(_status.event.player, _status.event.target) < 0 ? get.buttonValue(button) : 0;
						});
					}
				}
			},
			turn_clear: {
				...
				charlotte: true,
				content: function(event, trigger, player) {
					player.storage.meiyui_canghai_red_turn = null;
					player.storage.meiyui_canghai_black_turn = null;
				}
			}
		}
	},
	...
	"meiyui_xuanhuo": {
		...
		mod: {
			globalTo: function(from, to, distance) {
				if (from.storage.meiyui_xuanhuo_dist) {
					return distance - from.storage.meiyui_xuanhuo_dist;
				}
			}
		},
		filter: function(event, player) {
			return event.player.isDamaged() && player.countCards('he') > 0 && (player.storage.meiyui_xuanhuo_round || 0) < 2;
		},
		cost: async function(event, trigger, player) {
			var target = trigger.player;
			var next = player.chooseCard("he", "眩惑：你可以将一张牌当做【移花接木】对" + get.translation(target) + "使用");
			next.set("ai", function(card) {
				var target2 = trigger.player;
				var att = get.attitude(player, target2);

				if (att > 0) {
					var tName = target2.name1 || target2.name;
					if (tName === 'kako' || target2.name2 === 'kako' || target2.hasSkill('kako_shuqian')) return 12 - get.value(card);
					if (tName === 'nanaka' || target2.name2 === 'nanaka' || target2.hasSkill('nanaka_huaxin')) return 11 - get.value(card);
					if (target2.countCards('h') >= 3) return 8 - get.value(card);
					return 0;
				}
			...
			if (res.bool && res.cards && res.cards.length > 0) {
				event.result = { bool: true, cards: res.cards };
			}
		},
		content: async function(event, trigger, player) {
			var target = trigger.player;
			player.storage.meiyui_xuanhuo_round = (player.storage.meiyui_xuanhuo_round || 0) + 1;
			player.storage.meiyui_xuanhuo_dist = (player.storage.meiyui_xuanhuo_dist || 0) + 1;
			game.log(player, "本轮计算与其他角色的距离 -", player.storage.meiyui_xuanhuo_dist);

			var rawCards = event.cards.slice(0);
			var targetHandBefore = target.countCards("h");

			player.addTempSkill('meiyui_xuanhuo_damage_tracker');
			var useEvent = player.useCard({ name: 'yihuajiemu', isCard: true }, event.cards, target);
			useEvent.meiyui_xuanhuo_source = true;
			useEvent.meiyui_xuanhuo_me = player;
			await useEvent;
			player.removeSkill('meiyui_xuanhuo_damage_tracker');

			var targetGaveCards = (target.countCards("h") < targetHandBefore);
			var rawCardsInDiscard = rawCards.filter(c => get.position(c) === "d");

			if (targetGaveCards && rawCardsInDiscard.length > 0 && target.isIn()) {
				player.logSkill("meiyui_xuanhuo", target);
				game.log(player, "将【移花接木】对应的实体牌交给", target);
				await target.gain(rawCardsInDiscard, "gain2");
			}
		},
		subSkill: {
			round_clear: {
				...
				charlotte: true,
				content: function(event, trigger, player) {
					player.storage.meiyui_xuanhuo_round = 0;
					player.storage.meiyui_xuanhuo_dist = 0;
				}
			},
			damage_tracker: {
				trigger: { global: "damageEnd" },
				...
				filter: function(event, player) {
					...
					var evt = event.getParent("useCard");
					if (!evt || evt.card.name !== "sha") return false;
					
					var parentEvt = evt.parent;
					while (parentEvt) {
						if (parentEvt.name === "useCard" && parentEvt.meiyui_xuanhuo_source) {
							return true;
						}
						parentEvt = parentEvt.parent;
					}
					return false;
				},
				content: async function(event, trigger, player) {
					var killer = trigger.source;
					player.logSkill("meiyui_xuanhuo", killer);

					if (killer === player) {
						game.log(player, "因【眩惑】摸了两张牌");
						await player.draw(2);
					} else {
						game.log(player, "与", killer, "因【眩惑】各摸两张牌");
						await player.draw(1);
						if (killer.isIn()) {
							await killer.draw(1);
						}
	...
	"akira_canjie": {
		...
		trigger: { global: "useCardToTargeted" },
		filter: function(event, player) {
			if (player.hasSkill("akira_canjie_banned")) return false;
			if (!get.tag(event.card, "damage") && !["sha", "juedou", "huogong", "nanman", "wanjian"].includes(event.card.name)) return false;
			if (!event.targets || event.targets.length !== 1) return false;
		...
		cost: async function(event, trigger, player) {
			var target = trigger.target;
			var prompt = "参解：是否令" + (target === player ? "自己" : get.translation(target)) + "受到的伤害-1" + (target !== player ? "，并使你成为额外目标" : "") + "？";
			var next = player.chooseBool(prompt);
			next.set("ai", function() {
				var trg = _status.event.getTrigger();
				var p = _status.event.player;
				var tgt = trg.target;
				var card = trg.card;
				var src = trg.player;

				if (tgt === p) return get.attitude(p, src) <= 0;
				if (get.attitude(p, tgt) <= 0) return false;
				if (tgt.hp >= tgt.maxHp && tgt.hasSkillTag("maixie")) return false;
				if (p.hp <= 1) return false;

				var tName = tgt.name1 || tgt.name;
				var isCoreTeammate = (tName === 'nanaka' || tName === 'kako' || tgt.name2 === 'nanaka' || tgt.name2 === 'kako');
				if (isCoreTeammate && p.hp >= 2) return true;

				var hasCounter = false;
				if (card.name === "sha" && (p.hasCard(c => get.name(c) === "shan" || c.name === "lve", "h") || p.hasSkillTag("freeShan"))) {
					hasCounter = true;
				} else if (get.type(card) === "trick" && p.hasCard(c => get.name(c) === "wuxie", "h")) {
					hasCounter = true;
				}

				var hasBeenTargeted = game.getGlobalHistory("useCard", function(evt) {
					return evt.getParent("phase") === _status.currentPhase && evt.targets && evt.targets.includes(p);
				}).length > 0;
				var needGandanTrigger = !hasBeenTargeted;

				if (hasCounter) return true;
				if (tgt.hp <= 1 && p.hp >= 3) return true;
				if (p.hp >= 3 && needGandanTrigger) return true;
		...
		content: async function(event, trigger, player) {
			...
			if (!trigger.customArgs[target.playerid]) trigger.customArgs[target.playerid] = {};
			if (typeof trigger.customArgs[target.playerid].extraDamage !== "number") {
				trigger.customArgs[target.playerid].extraDamage = 0;
			}
			trigger.customArgs[target.playerid].extraDamage--;
			game.log(card, "对", target, "的伤害-1");

			if (target !== player && !trigger.targets.includes(player)) {
				trigger.targets.add(player);
				player.line(trigger.player, "fire");
				game.log(player, "成为了", card, "的额外目标");
			}

			var useEvent = trigger.getParent("useCard");
			if (useEvent) {
				useEvent._akira_canjie_trigger = {
					player: player,
					source: source
				};
			}
		...
		subSkill: {
			duel_phase: {
				...
				silent: true,
				filter: function(event, player) {
					return event._akira_canjie_trigger && event._akira_canjie_trigger.player === player;
				},
				content: async function(event, trigger, player) {
					var source = trigger._akira_canjie_trigger.source;
					if (player.isIn() && source && source.isIn() && player.hasCards("he")) {
						var chooseRes = await player.chooseCard("he", "参解：是否将一张牌当做【决斗】对" + get.translation(source) + "使用？").set("ai", function(card) {
							var p = _status.event.player;
							var src = _status.event.source;
							
							if (["tao", "jiu"].includes(get.name(card))) return 0;
							var eff = get.effect(src, { name: "juedou" }, p, p);
							if (eff <= 0) return 0;

							var myShaCount = p.countCards("h", { name: "sha" });
							if (p.hp <= 1 && myShaCount === 0) return 0;

							var isUsePhase = _status.currentPhase && _status.currentPhase.name === "phaseUse" && _status.currentPhase.player === src;
							var bonus = isUsePhase ? 3 : 0;
							return (6 + bonus) - get.value(card);
						}).set("source", source).forResult();

						if (chooseRes.bool && chooseRes.cards && chooseRes.cards.length > 0) {
							player.logSkill("akira_canjie", source);
							player.addTempSkill("akira_canjie_damage_check");
							var duelEvent = player.useCard({ name: "juedou", isCard: true }, chooseRes.cards, source, false);
							duelEvent.akira_canjie_duel = true;
							duelEvent.akira_canjie_target = source;
							await duelEvent;
							player.removeSkill("akira_canjie_damage_check");
						}
					}

					player.addSkill("akira_canjie_banned");
					player.storage.akira_canjie_banned_count = 2;
					game.log(player, "令技能", "#g【参解】", "失效直至下个回合结束");
				}
			},
			banned: {
				charlotte: true,
			...
			banned_clear: {
				...
				silent: true,
				filter: function(event, player) {
					return player.hasSkill("akira_canjie_banned") && typeof player.storage.akira_canjie_banned_count === "number";
				},
				content: function(event, trigger, player) {
					player.storage.akira_canjie_banned_count--;
					if (player.storage.akira_canjie_banned_count <= 0) {
						player.removeSkill("akira_canjie_banned");
						delete player.storage.akira_canjie_banned_count;
						game.log(player, "恢复了技能", "#g【参解】");
					}
				}
			},
			damage_check: {
				...
				filter: function(event, player) {
					var evt = event.getParent("useCard");
					return evt && evt.akira_canjie_duel && event.player === evt.akira_canjie_target;
				},
				content: async function(event, trigger, player) {
					var target = trigger.player;
					if (target && target.isIn()) {
						...
						if (cur === target) {
							var evt = _status.event;
							while (evt && evt !== _status.window) {
								if (evt.player === target && evt.name && evt.name.startsWith("phase") && !["phaseLoop", "phase"].includes(evt.name)) {
									evt.finish();
									evt.skipped = true;
									game.log(target, "的", "#y" + evt.name, "被强制结束");
									break;
								}
								if (!evt.getParent || typeof evt.getParent !== "function") break;
								evt = evt.getParent();
							}
						}
					}
	...
	"akira_gandan": {
		...
		filter: function(event, player) {
			var histories = game.getGlobalHistory("useCard", function(evt) {
				return evt.getParent("phase") === event;
			});
			var cond1 = histories.some(evt => evt.targets && evt.targets.includes(player));
			var cond2 = histories.some(evt => evt.targets && evt.targets.length > 1);
			return cond1 || cond2;
		},
		cost: async function(event, trigger, player) {
			event.result = { bool: true };
		...
		content: async function(event, trigger, player) {
			var histories = game.getGlobalHistory("useCard", function(evt) {
				return evt.getParent("phase") === trigger;
			});
			...
			if (cond1) {
				player.logSkill("akira_gandan");
				game.log(player, "触发了", "#g【肝胆①】");
				var card = get.cardPile(c => get.type(c) === "basic");
				if (card) {
					await player.gain(card, "gain2");
				} else {
					await player.draw();
			...
			if (cond2) {
				var targetRes = await player.chooseTarget(
					"肝胆②：选择一名其他角色，你与其各将一张牌当做【无中生有】使用",
					function(card, p, current) {
						return current !== p;
					}
				).set("ai", function(current) {
					var p = _status.event.player;
					var att = get.attitude(p, current);
					if (att <= 0) return -100;
					
					var cName = current.name1 || current.name;
					if (cName === 'nanaka' || current.name2 === 'nanaka' || cName === 'kako' || current.name2 === 'kako') {
						return 25 + att;
					}

					var hCount = current.countCards("h");
					if (hCount === 0) return 1;
					return att * 2 + (4 - Math.min(hCount, 3));
				}).forResult();

				if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
					var other = targetRes.targets[0];
					player.logSkill("akira_gandan", other);
					player.line(other, "green");

					if (player.hasCards("he")) {
						var res1 = await player.chooseCard("he", "肝胆：请选择一张牌当做【无中生有】对自己使用").set("ai", function(card) {
							if (get.name(card) === "tao" || get.name(card) === "jiu") return 0;
							return 9 - get.value(card);
						}).forResult();
						if (res1.bool && res1.cards && res1.cards.length > 0) {
							await player.useCard({ name: "wuzhong", isCard: true }, res1.cards, player);
						}
					}

					if (other.isIn() && other.hasCards("he")) {
						var res2 = await other.chooseCard("he", "肝胆：请选择一张牌当做【无中生有】对自己使用").set("ai", function(card) {
							if (get.name(card) === "tao" || get.name(card) === "jiu") return 0;
							return 9 - get.value(card);
						}).forResult();
						if (res2.bool && res2.cards && res2.cards.length > 0) {
							await other.useCard({ name: "wuzhong", isCard: true }, res2.cards, other);
						}
					}
				}
			}
	...
	"kako_xiamu": {
		...
		cost: async function(event, trigger, player) {
			var target = trigger.player;
			var prompt = "夏目：是否对 " + get.translation(target) + " 发动【夏目】？（移动各区域牌并赋予默识）";
			var next = player.chooseBool(prompt);
			next.set("ai", function() {
				var p = _status.event.player;
				var trg = _status.event.getTrigger().player;
				var att = get.attitude(p, trg);
				if (att > 0) return true;
				if (p.countCards("j", card => trg.canAddJudge(card)) > 0) return true;
				return false;
			});
			var res = await next.forResult();
			if (res.bool) {
		...
		content: async function(event, trigger, player) {
			player.storage.kako_xiamu_phase = game.phaseNumber;
			var target = trigger.player;
			player.logSkill("kako_xiamu", target);

			var myCardsH = player.getCards("h");
			var myCardsE = player.getCards("e");
			var myCardsJ = player.getCards("j");

			var targetCardsH = target.getCards("h");
			var targetCardsE = target.getCards("e");
			var targetCardsJ = target.getCards("j");

			var dialogArgs = [
				`###【夏目】递牌###<div class="text center">请从你的各区域挑选至多各一张牌移动给对方对应区域</div>`
			];

			dialogArgs.push("<div class=\"text center\">—— 你的可选区域 ——</div>");
			if (myCardsH.length) {
				dialogArgs.push("你的手牌区");
				dialogArgs.push(myCardsH);
			}
			if (myCardsE.length) {
				dialogArgs.push("你的装备区");
				dialogArgs.push(myCardsE);
			}
			if (myCardsJ.length) {
				dialogArgs.push("你的判定区");
				dialogArgs.push(myCardsJ);
			}

			dialogArgs.push("<div class=\"text center\">—— 对方对应区域状态 ——</div>");
			var stateStr = `手牌区：${targetCardsH.length}张（不可见）<br>装备区：${targetCardsE.length ? targetCardsE.map(c => get.translation(c)).join("、") : "空"}<br>判定区：${targetCardsJ.length ? targetCardsJ.map(c => get.translation(c)).join("、") : "空"}`;
			dialogArgs.push(`<div class="text center">${stateStr}</div>`);

			var next = player.chooseButton(dialogArgs, [1, 3]);
			next.set("filterButton", function(button) {
				var card = button.link;
				var p = _status.event.player;
				if (get.owner(card) !== p) return false;
				
				var selected = ui.selected.buttons.map(b => b.link);
				var pos = get.position(card);
				for (var i = 0; i < selected.length; i++) {
					if (get.position(selected[i]) === pos) return false;
				}
				
				var tgt = _status.event.target;
				if (pos === "e" && !tgt.canEquip(card)) return false;
				if (pos === "j" && !tgt.canAddJudge(card)) return false;
			...
			next.set("ai", function(button) {
				...
				if (pos === "h") {
					if (att > 0) {
						var isMyTurn = (_status.currentPhase === p);
						if (!isMyTurn) {
							if (["shan", "tao", "jiu", "wuxie"].includes(name)) return 15;
							return 8 - val;
						}
						if (isMyTurn) {
							if (["sha", "juedou", "shunshou", "guohe", "nanman", "wanjian"].includes(name)) return 15;
							return 8 - val;
						}
					} else {
						return name === "du" ? 30 : (8 - val);
					}
				}

				if (pos === "e") {
					if (att > 0) {
						if (name === "baiyin" && tgt.isDamaged()) return 16;
						return 6;
					}
					return 5 - val;
				}
			...
			if (res.bool && res.links && res.links.length > 0) {
				for (var card of res.links) {
					var originalPos = get.position(card);
					player.$give(card, target, false);
					await game.delay(0.2);

					if (originalPos === "h") {
						await target.gain([card], player, "give");
					} else if (originalPos === "e") {
						await target.equip(card);
					} else if (originalPos === "j") {
						await target.addJudge(card);
					}
				}
			}

			var targetRes = await player.chooseTarget("夏目：令一名角色获得【默识】直至其下个回合结束", true).set("ai", function(current) {
				var p = _status.event.player;
				var att = get.attitude(p, current);
				if (att <= 0) return -10;
				if (current.countCards("h") === 0) return 0; 

				var score = att * 2;
				var cName = current.name1 || current.name;

				if (cName === 'meiyui' || current.name2 === 'meiyui' || current.hasSkill('meiyui_canghai')) {
					score += 25;
				}

				if (cName === 'nanaka' || current.name2 === 'nanaka' || current.hasSkill('nanaka_huaxin')) {
					score += 18;
				}
				if (current === p) score += 5;
				return score;
			}).forResult();

			if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
				var chosen = targetRes.targets[0];
				chosen.addSkill("kako_moshi");
				chosen.storage.kako_moshi_round_count = 2;
				chosen.storage.kako_moshi_source = player;
				game.log(chosen, "获得了技能", "#g【默识】");
			}
		},
		group: "kako_xiamu_clear",
		subSkill: {
			clear: {
				trigger: { global: "phaseBefore" },
				forced: true,
				silent: true,
				content: function(event, trigger, player) {
					delete player.storage.kako_xiamu_phase;
				}
			}
		}
	},
	...
	"kako_shuqian": {
		...
		group: ["kako_shuqian_round_clear"],
		filter: function(event, player) {
			if (!player.storage.kako_shuqian_used) player.storage.kako_shuqian_used = [];
			
			var evt = event.getl(player);
			if (!evt || event.player === player) return false;

			var used = player.storage.kako_shuqian_used;
			if (evt.hs && evt.hs.length > 0 && !used.includes("h")) return true;
			if (evt.es && evt.es.length > 0 && !used.includes("e")) return true;
			if (evt.js && evt.js.length > 0 && !used.includes("j")) return true;
		...
		content: async function(event, trigger, player) {
			...
			if (evt.hs && evt.hs.length > 0 && !used.includes("h")) {
				used.add("h");
				player.storage.kako_shuqian_used = used;
				player.logSkill("kako_shuqian");
				game.log(player, "触发了", "#g【书签·手牌区】");

				var hasBurstEnemy = game.hasPlayer(cur => get.attitude(player, cur) < 0 && cur.countCards("h") > cur.hp + 1);
				var defaultChoice = hasBurstEnemy ? "弃牌阶段" : "摸牌阶段";

				var ctrl = await player.chooseControl("摸牌阶段", "弃牌阶段").set("prompt", "书签：请选择阶段类型").set("choice", defaultChoice).forResult();
				var chooseTgt = await player.chooseTarget("书签：选择一名角色于回合结束后执行一个" + ctrl.control, true).set("ai", function(current) {
					var p = _status.event.player;
					var att = get.attitude(p, current);
					if (ctrl.control === "摸牌阶段") {
						return att > 0 ? (att * 3 + (5 - current.countCards("h"))) : -10;
					} else {
						var overflow = current.countCards("h") - current.hp;
						return att < 0 ? (-att * Math.max(1, overflow + 1)) : -100;
					}
				}).forResult();

				if (chooseTgt.bool && chooseTgt.targets && chooseTgt.targets.length > 0) {
					var tgt = chooseTgt.targets[0];
					var phaseName = ctrl.control === "摸牌阶段" ? "phaseDraw" : "phaseDiscard";
					tgt.insertPhase().set("phaseList", [phaseName]);
					game.log(tgt, "将于回合结束后执行一个额外的", "#y" + ctrl.control);
				}
			}

			if (evt.es && evt.es.length > 0 && !used.includes("e")) {
				used.add("e");
				player.storage.kako_shuqian_used = used;
				player.logSkill("kako_shuqian");
				game.log(player, "触发了", "#g【书签·装备区】");

				var chooseTgt2 = await player.chooseTarget("书签：令一名角色将一张牌当做【树上开花】使用", true).set("ai", function(current) {
					var att = get.attitude(_status.event.player, current);
					if (att <= 0) return -10;
					return att * (current.countCards("he") > 0 ? 2 : 0);
				}).forResult();

				if (chooseTgt2.bool && chooseTgt2.targets && chooseTgt2.targets.length > 0) {
					var t2 = chooseTgt2.targets[0];
					if (t2.hasCards("he")) {
						var resKai = await t2.chooseCard("he", "书签：是否将一张牌当做【树上开花】使用？").set("ai", function(card) {
							if (["tao", "jiu"].includes(get.name(card))) return 0;
							return 8 - get.value(card);
						}).forResult();
						if (resKai.bool && resKai.cards && resKai.cards.length > 0) {
							await t2.useCard({ name: "kaicai", isCard: true }, resKai.cards, t2);
						}
					}
				}
			}

			if (evt.js && evt.js.length > 0 && !used.includes("j")) {
				used.add("j");
				player.storage.kako_shuqian_used = used;
				player.logSkill("kako_shuqian");
				game.log(player, "触发了", "#g【书签·判定区】");

				var chooseTgt3 = await player.chooseTarget("书签：对一名角色造成1点伤害", true).set("ai", function(current) {
					var att = get.attitude(_status.event.player, current);
					if (att >= 0) return -10;
					var score = -att * 2;
					if (current.hp <= 1) score += 10;
					return score;
				}).forResult();

				if (chooseTgt3.bool && chooseTgt3.targets && chooseTgt3.targets.length > 0) {
					await player.damage(chooseTgt3.targets[0]);
				}
			}
		},
		subSkill: {
			round_clear: {
				...
				silent: true,
				content: function(event, trigger, player) {
					player.storage.kako_shuqian_used = [];
				}
			}
		}
	},
	...
	"kako_moshi": {
		...
		filter: function(event, player) {
			...
			var lastCard = history[history.length - 1].card;
			return player.hasUseTarget({ name: lastCard.name, nature: lastCard.nature });
		},
		filterCard: true,
		selectCard: 1,
		position: "he",
		viewAs: function(cards, player) {
			...
			var lastCard = history[history.length - 1].card;
			return { name: lastCard.name, nature: lastCard.nature };
		},
		viewAsFilter: function(player) {
			var history = player.getHistory("useCard", function(evt) {
				return ["basic", "trick"].includes(get.type(evt.card));
			...
			if (!history.length) return false;
			var lastCard = history[history.length - 1].card;
			return player.hasCards("he") && player.hasUseTarget({ name: lastCard.name, nature: lastCard.nature });
		},
		prompt: function() {
			var player = _status.event.player;
			var history = player.getHistory("useCard", function(evt) {
			...
			if (history.length) {
				var lastCard = history[history.length - 1].card;
				return "默识：将一张牌当做【" + get.translation(lastCard) + "】使用（出牌阶段限一次）";
			}
			return "默识：暂未记录牌";
		},
		check: function(card) {
		...
		ai: {
			order: function() {
				...
				if (history.length > 0) {
					var lastCard = history[history.length - 1].card;
					return get.order({ name: lastCard.name, nature: lastCard.nature }) + 0.1;
				}
				return 0;
			},
			result: { player: 1 }
		},
		group: ["kako_moshi_end", "kako_moshi_lifetime"],
		onremove: function(player, skill) {
			var kako = player.storage.kako_moshi_source;
			if (kako && kako.isIn()) {
				kako.storage.kako_shuqian_used = [];
				game.log(player, "失去了", "#g【默识】", "，重置了", kako, "的", "#g【书签】");
			}
			delete player.storage.kako_moshi_round_count;
			delete player.storage.kako_moshi_source;
		},
		subSkill: {
			end: {
				trigger: { player: "phaseJieshuBegin" },
				filter: function(event, player) {
					return player.getHistory("useCard", evt => evt.isPhaseUsing() && ["basic", "trick"].includes(get.type(evt.card))).length > 0 && player.countCards("h") > 0;
				},
				direct: true,
				content: async function(event, trigger, player) {
					var history = player.getHistory("useCard", evt => evt.isPhaseUsing() && ["basic", "trick"].includes(get.type(evt.card)));
					if (!history.length || !player.countCards("h")) return;

					var firstCard = history[0].card;
					var vcard = { name: firstCard.name, nature: firstCard.nature };

					if (!player.hasUseTarget(vcard, true, true)) return;

					var name = "kako_moshi_end_backup";
					game.broadcastAll((n, c) => {
						lib.skill[n].viewAs = c;
					}, name, vcard);

					var next = player.chooseToUse();
					next.logSkill = "kako_moshi";
					next.set("openskilldialog", `默识：将一张手牌当【${get.translation(vcard)}】使用`);
			...
			end_backup: {
				...
				check: function(card) {
					if (["tao", "jiu"].includes(get.name(card))) return 0;
					var vcard = lib.skill.kako_moshi_end_backup?.viewAs;
					if (vcard && get.value(vcard) > get.value(card)) {
						return 8 - get.value(card);
					}
					return 0;
				},
				sub: true,
				sourceSkill: "kako_moshi",
				_priority: 0
			},
			lifetime: {
				trigger: { player: "phaseAfter" },
				...
				silent: true,
				content: function(event, trigger, player) {
					if (typeof player.storage.kako_moshi_round_count === "number") {
						player.storage.kako_moshi_round_count--;
						if (player.storage.kako_moshi_round_count <= 0) {
							player.removeSkill("kako_moshi");
						}
					}
				}
			}
	...
	"Konoha_feihua": {
		audio: "ext:魔法纪录/audio/skill:2",
		usable: 1,
		group: ["Konoha_feihua_skip", "Konoha_feihua_out"],
		subSkill: {
			skip: {
				trigger: { player: "phaseDrawBegin1" },
				filter: function(event, player) { return !event.numFixed; },
				cost: async function(event, trigger, player) {
					var res = await player.chooseBool("飞花：是否放弃摸牌，视为使用一张【杜鹃花开】？").set("ai", function() {
						return 1;
					}).forResult();
					if (res.bool) event.result = { bool: true };
				},
				content: async function(event, trigger, player) {
					...
					var vcard = game.createCard({ name: "dujuanhuakai", suit: "heart", number: 3, isCard: true });
					await player.useCard(vcard, player);
				}
			},
			out: {
				enable: ["chooseToUse", "chooseToRespond"],
				filter: function(event, player) {
					return _status.currentPhase !== player && player.getExpansions("Konoha_jiahu").length > 0;
				},
				hiddenCard: function(player, name) {
					if (_status.currentPhase === player) return false;
					var flowers = player.getExpansions("Konoha_jiahu");
					return flowers.some(c => c.name === name);
				},
				chooseButton: {
					dialog: function(event, player) {
						return ui.create.dialog("飞花：选择作为【花】打出的牌", player.getExpansions("Konoha_jiahu"), "hidden");
					},
					filter: function(button, player) {
						var card = button.link;
						var evt = _status.event;
						while (evt && !evt.filterCard) {
							evt = evt.getParent();
						}
				...
				ai: {
					...
					skillTagFilter: function(player, tag, arg) {
						...
						if (tag === "save") return flowers.some(c => c.name === "tao" || c.name === "jiu");
						if (tag === "wuxie") return flowers.some(c => c.name === "wuxiekeji");
						if (tag === "respondShan") return flowers.some(c => c.name === "shan");
						if (tag === "respondSha") return flowers.some(c => c.name === "sha");
					}
				}
			}
		}
	...
	"Konoha_diewu": {
		...
		filter: function(event, player) {
			...
			return get.distance(player, event.player) <= 1 || get.distance(event.player, player) <= 1;
		},
		cost: async function(event, trigger, player) {
			var prompt = "蝶舞：是否对 " + get.translation(trigger.player) + " 发动【蝶舞】防伤？";
			var next = player.chooseBool(prompt);
			next.set("ai", function() {
				var p = _status.event.player;
				var tgt = _status.event.getTrigger().player;
				var att = get.attitude(p, tgt);
				if (att <= 0) return false; 
				if (tgt.hp <= 2) return true;
				return att > 0;
			});
			var res = await next.forResult();
			if (res.bool) {
		...
		content: async function(event, trigger, player) {
			player.logSkill("Konoha_diewu", trigger.player);

			var flowers = player.getExpansions("Konoha_jiahu");
			var flowerSuits = [];
			for (var i = 0; i < flowers.length; i++) {
				var fs = get.suit(flowers[i]);
				if (fs && fs !== "none") flowerSuits.add(fs);
			}

			var standardSuits = ["spade", "heart", "club", "diamond"];
			var remainingSuits = standardSuits.filter(s => !flowerSuits.includes(s));

			var myCards = player.getCards("h");
			var availableSuits = remainingSuits.filter(s => {
				return myCards.some(c => get.suit(c) === s);
			});

			var chosenSuit = null;
			if (availableSuits.length > 0) {
				chosenSuit = availableSuits.randomGet();
			} else {
				var existingSuits = [];
				myCards.forEach(c => {
					var cs = get.suit(c);
					if (cs && cs !== "none") existingSuits.add(cs);
				});
				if (existingSuits.length > 0) chosenSuit = existingSuits.randomGet();
			}

			if (chosenSuit) {
				var toShow = myCards.filter(c => get.suit(c) === chosenSuit);
				if (toShow.length > 0) {
					await player.showCards(toShow, `${get.translation(player)}展示了所有【${get.translation(chosenSuit)}】手牌`);
				}
			}

			var chooseRes = await player.chooseCard("h", 1, "蝶舞：请扣置一张手牌供伤害来源猜测花色").set("ai", function(card) {
				var cs = _status.event.chosenSuit;
				if (cs && get.suit(card) !== cs) return 10 - get.value(card);
				return 5 - get.value(card);
			}).set("chosenSuit", chosenSuit).forResult();

			if (!chooseRes.bool || !chooseRes.cards || !chooseRes.cards.length) return;
			var hiddenCard = chooseRes.cards[0];
			var realSuit = get.suit(hiddenCard);

			player.lose(hiddenCard, ui.special, "toSpecial");
			var source = trigger.source;
			var guessRight = false;

			if (source && source.isIn()) {
				var guessRes = await source.chooseControl(["spade", "heart", "club", "diamond"])
					.set("prompt", "蝶舞：请猜测 " + get.translation(player) + " 扣置卡牌的花色")
					.set("choiceList", ["黑桃", "红桃", "梅花", "方片"])
					.set("ai", function() {
						var cs = _status.event.chosenSuit;
						var suits = ["spade", "heart", "club", "diamond"];
						if (cs) suits.remove(cs);
						return suits.randomGet();
					})
					.set("chosenSuit", chosenSuit)
					.forResult();

				source.chat("我猜是" + get.translation(guessRes.control));
				game.log(source, "猜测的花色为", "#y" + get.translation(guessRes.control));

				if (guessRes.control === realSuit) {
					guessRight = true;
				}
			}

			game.log(player, "实际扣置牌的花色为", "#y" + get.translation(realSuit));

			if (!guessRight) {
				game.log(source || "无来源", "猜测", "#r错误", "，伤害被防止！");
				trigger.cancel();
				...
				await player.gain([hiddenCard], "gain2");
			} else {
				game.log(source, "猜测", "#g正确", "！获得了扣置牌");
				await source.gain([hiddenCard], "gain2");
			}
		}
	},
	...
	"Konoha_jiahu": {
		...
		cost: async function(event, trigger, player) {
			...
			var res = await player.chooseCard("he", 1, prompt).set("ai", function(card) {
				...
				var isDef = ['shan', 'tao', 'jiu', 'wuxie'].includes(card.name);
				
				if (color === 'none') return 10 - val;
				if (isDef && p.countCards('h', ['shan', 'tao', 'jiu', 'wuxie']) > 1) {
					return 8 - val;
				}
				return 0;
			}).forResult();
		...
		content: async function(event, trigger, player) {
			...
			var vcard = game.createCard({ name: trigger.card.name, suit: "none" });
			
			var evt = player.addToExpansion([vcard], player, "gain2");
		...
		subSkill: {
			judge: {
				...
				filter: function(event, player) { return player.getExpansions("Konoha_jiahu").length > 0; },
				cost: async function(event, trigger, player) {
					var res = await player.chooseCardButton("家护：是否流失1点体力，使用“花”替换判定牌？", player.getExpansions("Konoha_jiahu")).set("ai", function(button) {
						var player = _status.event.player;
						var trgEvt = _status.event.getTrigger();
						var target = trgEvt.player;
						var att = get.attitude(player, target);
						
						if (att <= 0) return 0;
						
						var tName = target.name1 || target.name;
						var isHazuki = (tName === 'hazuki' || target.name2 === 'hazuki' || target.hasSkill('Hazuki_chedian'));
						if (isHazuki && trgEvt.name === 'Hazuki_chedian') {
							return 100;
						}
					...
					if (res.bool && res.links && res.links.length > 0) {
						event.result = { bool: true, links: res.links };
					} else {
						event.result = { bool: false };
					}
				},
				content: async function(event, trigger, player) {
					if (!event.result || !event.result.bool || !event.result.links || event.result.links.length === 0) return;
					
					await player.loseHp(1);
					var card = event.result.links[0];
					var oldCard = trigger.player.judging[0];
					
					if (oldCard.clone) {
						oldCard.clone.classList.remove('thrownhighlight');
						game.broadcast(c => { if(c.clone) c.clone.classList.remove('thrownhighlight'); }, oldCard);
						game.addVideo('deletenode', player, get.cardsInfo([oldCard.clone]));
					}
					game.cardsDiscard(oldCard);
					trigger.player.judging[0] = card;
					trigger.orderingCards.add(card);
					await player.gain(oldCard, "gain2");
					
					player.addMark("Konoha_jiahu_dist", 1, false);
				}
			}
		},
		mod: {
			globalFrom: function(from, to, dist) {
				if (from.hasMark("Konoha_jiahu_dist")) return dist - from.countMark("Konoha_jiahu_dist");
			}
		}
	},
	...
	"Hazuki_feiyun": {
		...
		content: async function(event, trigger, player) {
			...
			var next = player.chooseButton(["飞云：观看其手牌，你可以选择其中一张红色牌", cards]);
			next.set("filterButton", btn => get.color(btn.link) === "red");
			next.set("ai", function(btn) {
				var p = _status.event.player;
				var trg = _status.event.target;
				var att = get.attitude(p, trg);
				var card = btn.link;
				if (att < 0 && (trg.hasSkill("Konoha_diewu") || trg.name1 === "konoha")) return 20 + get.value(card);
				return get.value(card);
			}).set("target", target);
			...
			if (res.bool && res.links && res.links.length > 0) {
				...
				var ctrlRes = await player.chooseControl("获得此牌", "置于牌堆顶并重置掣电").set("ai", function() {
					var p = _status.event.player;
					if (p.hasSkill("Hazuki_chedian_banned")) return "置于牌堆顶并重置掣电";
					return "获得此牌";
				}).forResult();
				
				if (ctrlRes.control === "获得此牌") {
					await player.gain([card], target, "giveAuto");
				} else {
					await target.lose([card], ui.cardPile, "insert", "visible");
					game.log(player, "将", card, "置于了牌堆顶");
					
					player.removeSkill("Hazuki_chedian_banned");
					if (typeof player.getStat().skill.Hazuki_chedian === 'number') {
						delete player.getStat().skill.Hazuki_chedian;
					}
					game.log(player, "重置了", "#g【掣电】");
				}
			}
		}
	},
	...
	"Hazuki_chedian": {
		...
		trigger: { player: ["useCardAfter", "respondAfter"] },
		filter: function(event, player, name) {
			if (player.hasSkill("Hazuki_chedian_banned")) return false;
			var card = event.card;
			if (!card) return false;
			
			var isDamageCard = get.tag(card, 'damage') || ['sha', 'juedou', 'huogong', 'nanman', 'wanjian'].includes(card.name);
			var isDefCard = ['shan', 'wuxie'].includes(card.name);
			var isRespondSha = (name === "respondAfter" && card.name === "sha");
			
			if (!isDamageCard && !isDefCard && !isRespondSha) return false;
			if (event.Hazuki_chedian_hpChanged) return false;
			
			return true;
		},
		cost: async function(event, trigger, player) {
			var res = await player.chooseBool("掣电：是否进行一次判定？").set("ai", () => 1).forResult();
			if (res.bool) { event.result = { bool: true }; }
		},
		content: async function(event, trigger, player) {
			player.addTempSkill("Hazuki_chedian_banned", "phaseAfter");
			
			var judgeRes = await player.judge("Hazuki_chedian");
			var color = judgeRes.result.color;
			var judgeCard = judgeRes.result.card;
			var chengshi = false;
			
			if (color === "black" && player.hasMark("Hazuki_chedian_black2none")) {
				player.removeMark("Hazuki_chedian_black2none", player.countMark("Hazuki_chedian_black2none"));
				color = "none";
				chengshi = true;
				game.log(player, "消耗标记，本次判定视为", "#y无色");
			}
			if (color === "none") {
				chengshi = true;
			}
			
			var notBlack = (color !== "black");
			var notRed = (color !== "red");
			
			if (notBlack) {
				if (player.hasUseTarget(judgeCard)) {
					var useRes = await player.chooseUseTarget(judgeCard, false, "掣电：是否使用该判定牌？（若取消则摸一张牌）");
					if (!useRes.bool) {
						await player.draw(1);
						game.log(player, "选择了摸一张牌");
					}
				} else {
					await player.draw(1);
					game.log(player, "无法使用该判定牌，摸一张牌");
				}
			}
			
			if (notRed) {
				player.addTempSkill("Hazuki_chedian_add", "phaseAfter");
				player.addMark("Hazuki_chedian_add", 1, false);
				game.log(player, "使", "#g【飞云】", "本回合可发动次数+1");
				
				if (!chengshi) {
					player.addMark("Hazuki_chedian_black2none", 1, false);
					game.log(player, "获得了标记，下次黑色判定将视为", "#y无色");
				}
			}
			
			if (chengshi) {
				game.log(player, "触发了", "#y【乘势】", "！");
				var targetRes = await player.chooseTarget("乘势：可令一名角色于本回合后执行一个额外的摸牌与出牌阶段", true).set("ai", function(target) {
					var p = _status.event.player;
					var att = get.attitude(p, target);
					if (att <= 0) return -1;
					
					var tName = target.name1 || target.name;
					if (tName === 'ayame' || target.name2 === 'ayame' || target.hasSkill('Ayame_liuxing') || target.hasSkill('Ayame_meiyan')) {
						return 30 + att;
					}
					if (tName === 'konoha' || target.name2 === 'konoha') {
						return 20 + att;
					}
					return 10 + att;
				}).forResult();
				...
				if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
					...
					target.insertPhase().set("phaseList", ["phaseDraw", "phaseUse"]);
				}
			}
		},
		subSkill: {
			banned: { charlotte: true },
			black2none: {
				mark: true,
				marktext: "势",
				intro: { content: "下次【掣电】判定为黑色时视为无色，并触发【乘势】" }
			},
			add: {
				charlotte: true,
				onremove: true,
				...
				intro: {
					...
					content: (storage, player) => `本回合〖飞云〗可发动次数+${storage || 0}`,
				},
				sub: true,
				"_priority": 0,
			},
			tracker: {
				...
				content: function(event, trigger, player) {
					var evts = [trigger.getParent("useCard"), trigger.getParent("respond")];
					for (var i = 0; i < evts.length; i++) {
						var e = evts[i];
						if (e && e.card === trigger.card) {
							e.Hazuki_chedian_hpChanged = true;
						}
					}
				}
			}
	...
	"Ayame_feihuo": {
		...
		group: ["Ayame_feihuo_record", "Ayame_feihuo_discard", "Ayame_feihuo_damage", "Ayame_feihuo_clear"],
		subSkill: {
			record: {
				...
				content: function(event, trigger, player) {
					player.storage.Ayame_feihuo_len = player.storage.Ayame_feihuo_len || [];
					var len = get.translation(trigger.card.name).length;
					player.storage.Ayame_feihuo_len.push(len);
				}
			},
			discard: {
				trigger: { player: ["useCardToPlayered", "respond"] },
				filter: function(event, player) {
					var card = event.card;
					if (!card) return false;
					var isFire = (get.color(card) === "red" || card.nature === "fire" || get.nature(card) === "fire");
					if (!isFire) return false;

					if (event.name === "useCardToPlayered") {
						return event.target && event.target !== player && event.target.countCards("he") > 0;
					} else if (event.name === "respond") {
						return event.respondTo && event.respondTo[0] && event.respondTo[0] !== player && event.respondTo[0].countCards("he") > 0;
					}
					return false;
				},
				cost: async function(event, trigger, player) {
					var target = trigger.name === "useCardToPlayered" ? trigger.target : trigger.respondTo[0];
					var res = await player.chooseBool("飞火：是否令 " + get.translation(target) + " 弃置一张牌？").set("ai", function() {
						var p = _status.event.player;
						var trg = _status.event.target;
						var att = get.attitude(p, trg);
						if (att <= 0 && (trg.hasSkill("Konoha_diewu") || trg.name1 === "konoha")) return true;
						return att <= 0;
					}).set("target", target).forResult();
					if (res.bool) event.result = { bool: true, cost_data: target };
				},
				content: async function(event, trigger, player) {
					var target = event.cost_data;
					player.logSkill("Ayame_feihuo", target);
					await target.chooseToDiscard(1, "he", true);
				}
			},
			damage: {
				trigger: { source: "damageBegin1" },
				forced: true,
				filter: function(event, player) {
					if (!event.card || !event.hasNature("fire")) return false;
					var len = get.translation(event.card.name).length;
					var lens = player.storage.Ayame_feihuo_len || [];
					var count = 0;
				...
				content: async function(event, trigger, player) {
					trigger.num++;
					game.log(trigger.player, "的火属性伤害被", "#g【飞火】", "增加了1点！");
				}
			},
			clear: {
				trigger: { global: "phaseAfter" },
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					delete player.storage.Ayame_feihuo_len;
				}
			}
		}
	},
	...
	"Ayame_liuxing": {
		...
		ai: {
			...
			result: {
				player: function(player) {
					if (game.phaseNumber === 0) return 1;
					if (game.hasPlayer(current => get.attitude(player, current) < 0 && current.hp <= 2)) return 1;
					return 1;
				}
			}
		},
		content: async function(event, trigger, player) {
			player.awakenSkill("Ayame_liuxing");
			var vcard = game.createCard({ name: "dujuanhuakai", suit: "heart", number: 3, isCard: true });
			await player.useCard(vcard, player);
			player.addTempSkill("Ayame_liuxing_buff", "phaseAfter");
			await player.draw(1);
		},
		subSkill: {
			buff: {
				...
				onremove: function(player) {
					player.removeSkill("Ayame_liuxing");
					player.addSkill("Ayame_meiyan");
					game.log(player, "获得了", "#r【魅焱】");
				}
			}
		}
	},
	...
	"Ayame_meiyan": {
		...
		filter: function(event, player) { return true; },
		content: async function(event, trigger, player) {
			var costRes = await player.chooseControl("受到1点伤害", "弃置一张黑色牌").set("ai", function() {
				var p = _status.event.player;
				if (p.hp > 2) return "受到1点伤害";
				return "弃置一张黑色牌";
			}).forResult();
			
			var costOk = false;
			if (costRes.control === "受到1点伤害") {
				await player.damage(1, "nosource");
				costOk = true;
			} else {
				var discardRes = await player.chooseToDiscard("he", 1, true, c => get.color(c) === "black").forResult();
				if (discardRes.bool) costOk = true;
			}
			
			if (costOk) {
				player.addTempSkill("Ayame_meiyan_buff", "phaseUseAfter");
				player.addMark("Ayame_meiyan_count", 1, false);
				if (player.countMark("Ayame_meiyan_count") >= 2) {
					player.removeMark("Ayame_meiyan_count", player.countMark("Ayame_meiyan_count"));
					player.removeSkill("Ayame_meiyan");
					player.addSkill("Ayame_liuxing");
					player.restoreSkill("Ayame_liuxing");
					game.log(player, "获得了", "#y【流星】");
				}
			}
		},
		subSkill: {
			buff: {
				...
				forced: true,
				filter: function(event, player) { return event.targets && event.targets.length > 0; },
				content: async function(event, trigger, player) {
					var res = await player.chooseTarget("魅焱：可为" + get.translation(trigger.card) + "增加一名额外目标（防止对其造成的伤害）", function(card, p, target) {
						return !trigger.targets.includes(target) && lib.filter.targetEnabled2(trigger.card, p, target);
					}).set("ai", function(target) {
						var player = _status.event.player;
						var card = _status.event.getTrigger().card;
						var att = get.attitude(player, target);
						
						if (get.tag(card, 'damage') || ['sha', 'juedou', 'nanman', 'wanjian'].includes(card.name)) {
							var tName = target.name1 || target.name;
							var isHazuki = (tName === 'hazuki' || target.name2 === 'hazuki' || target.hasSkill('Hazuki_feiyun'));
							if (att > 0 && isHazuki) return 30;
					...
					if (res.bool && res.targets && res.targets.length > 0) {
						...
						player.storage.Ayame_meiyan_hidden = player.storage.Ayame_meiyan_hidden || [];
						player.storage.Ayame_meiyan_hidden.push({ card: trigger.card, target: target });
						player.addTempSkill("Ayame_meiyan_prevent", "useCardAfter");
					}
				}
			},
			prevent: {
				...
				filter: function(event, player) {
					if (!event.card) return false;
					var hiddens = player.storage.Ayame_meiyan_hidden || [];
					return hiddens.some(h => h.card === event.card && h.target === event.player);
				},
				content: async function(event, trigger, player) {
					trigger.cancel();
					game.log(player, "防止了", trigger.card, "对额外目标", trigger.player, "的伤害！");
				}
			}
		}
	},
	...
	"hanna_fuwang_blocker": {
		...
		skillBlocker: function(skill, player) {
			var storage = player.storage.hanna_fuwang_disabled;
			return storage && storage.includes(skill);
		},
		mark: true,
		intro: {
			content: function(storage, player, skill) {
				var st = player.storage.hanna_fuwang_disabled;
				if (st && st.length) {
					return "被失效的技能：【" + get.translation(st) + "】";
				}
				return "无失效技能";
			}
		}
    ...
    "hanna_fuwang": {
		...
		content: async function(event, trigger, player) {
			const bannedList = ["bifa", "buqu", "gzbuqu", "songci", "funan", "xinfu_guhuo", "reguhuo", "huashen", "rehuashen", "old_guhuo", "shouxi", "xinpojun", "taoluan", "xintaoluan", "xinfu_yingshi", "zhenwei", "zhengnan", "xinzhengnan"];
			const skills2 = trigger.player.getSkills(null, false, false).filter((skill) => {
				const info = get.info(skill);
				return info && !info.charlotte && !info.persevereSkill && !info.hiddenSkill && !info.zhuSkill && !info.juexingji && !info.limited && !info.dutySkill && !info.equipSkill && !bannedList.includes(skill);
			});

			if (skills2.length === 0) return;

			const list = skills2.map(skill => [skill, '<div class="text center">【' + get.translation(skill) + '】</div>']);
			const result = await player.chooseButton(['覆妄：选择废除并获得其一个技能', [list, 'textbutton']], true).set("ai", function(button) {
				var skill = button.link;
				var info = get.info(skill);
				if (!info) return 0;
				if (info.ai && info.ai.threaten) {
					var threaten = typeof info.ai.threaten === 'function' ? info.ai.threaten(_status.event.player, _status.event.target) : info.ai.threaten;
					if (threaten < 0) return -100; 
					return threaten;
				}
				if (info.ai && info.ai.combo) return 1.5;
				if (info.enable) return 2; 
				return 1;
			}).forResult();

			if (result.bool && result.links && result.links.length > 0) {
				const skill = result.links[0];
				player.logSkill("hanna_fuwang", trigger.player);
				
				trigger.player.storage.hanna_fuwang_disabled = trigger.player.storage.hanna_fuwang_disabled || [];
				trigger.player.storage.hanna_fuwang_disabled.push(skill);
				trigger.player.addSkill("hanna_fuwang_blocker"); 

				player.addSkill(skill);
				
				player.storage.hanna_fuwang_records = player.storage.hanna_fuwang_records || [];
				player.storage.hanna_fuwang_records.push({ target: trigger.player, skill: skill });
				
				player.addTempSkill("hanna_fuwang_clear", { player: ["phaseBegin", "die"] });
				game.log(player, "令", trigger.player, "的技能失效了！", "#y【" + get.translation(skill) + "】", "并将其获得");
			}
		},
		group: "hanna_fuwang_clear",
		subSkill: {
			clear: {
				trigger: { player: ["phaseBegin", "die"] },
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					var records = player.storage.hanna_fuwang_records || [];
					for (var i = 0; i < records.length; i++) {
						var target = records[i].target;
						var skill = records[i].skill;
						if (target && target.storage.hanna_fuwang_disabled) {
							target.storage.hanna_fuwang_disabled.remove(skill);
							if (target.storage.hanna_fuwang_disabled.length === 0) {
								target.removeSkill("hanna_fuwang_blocker");
								delete target.storage.hanna_fuwang_disabled;
							}
						}
						player.removeSkill(skill);
					}
					delete player.storage.hanna_fuwang_records;
					player.removeSkill("hanna_fuwang_clear");
				}
			}
		}
	},
	...
	"hanna_cuanshi": {
		audio: "ext:魔法纪录/audio/skill:2",
		group: ["hanna_cuanshi_before", "hanna_cuanshi_zhunbei", "hanna_cuanshi_jieshu"],
		subSkill: {
			before: {
				audio: "ext:魔法纪录/audio/skill:2",
				trigger: { player: "phaseBefore" },
				...
				filter: function(event, player) {
					var skills = player.getSkills(null, false, false).filter(function(i) {
						var info = get.info(i);
						return info && !info.charlotte && !info.equipSkill && !info.global;
					});
				...
				content: async function(event, trigger, player) {
					...
					var next = player.chooseButton(['篡蚀：你的技能数大于体力上限，请选择失去一个技能并摸三张牌', [list, 'textbutton']], true);
					next.set('ai', function(button) {
						var skill = button.link;
						if (skill === "hanna_qiuji" || skill === "hanna_cuanshi") return -100; 
						if (skill === "hanna_fuwang") return 10;
						
						var info = get.info(skill);
						if (info && info.ai && info.ai.threaten) {
							var t = typeof info.ai.threaten === 'function' ? info.ai.threaten(_status.event.player, _status.event.player) : info.ai.threaten;
							if (t < 0) return 100; 
						}
						return 50; 
					});
					...
					if (res.bool && res.links && res.links.length > 0) {
						var skillToLose = res.links[0];
						player.removeSkill(skillToLose);
						game.log(player, "失去了技能", "#y【" + get.translation(skillToLose) + "】");
						
						await player.draw(3);
						player.addTempSkill("hanna_cuanshi_buff", "phaseAfter");
					}
				}
			},
			zhunbei: {
				...
				content: async function(event, trigger, player) {
					var hasTarget = game.hasPlayer(function(current){
						return player.canUse({name: "sha", nature: "thunder", isCard: true}, current, false);
					});

					if (!hasTarget || !player.hasCard(c => get.color(c) === 'black', "he")) {
						await player.draw(1);
						game.log(player, "无法使用雷【杀】，改为摸一张牌");
						return;
					}

					var next = player.chooseCardTarget({
						position: "he",
						prompt: "篡蚀：你可以弃置一张黑色牌，视为对一名角色使用一张雷【杀】（点击【取消】改为摸一张牌）",
						filterCard: function(card, player) {
							return get.color(card) === 'black';
						},
						filterTarget: function(card, player, target) {
						...
						ai2: function(target) {
							return get.effect(target, {name: "sha", nature: "thunder"}, _status.event.player, _status.event.player);
						}
					});
					
					var res = await next.forResult();
					
					if (res.bool && res.targets && res.targets.length > 0 && res.cards && res.cards.length > 0) {
						await player.discard(res.cards);
						game.log(player, "弃置了", res.cards, "视为使用了一张雷【杀】");
						
						var vcard = { name: "sha", nature: "thunder", isCard: true };
						await player.useCard(vcard, res.targets, false);
					} else {
						await player.draw(1);
						game.log(player, "选择了摸一张牌");
					}
	...
	"hanna_qiuji": {
		...
		usable: 1, 
		hiddenCard: function(player, name) {
			return name === "tiesuo" && player.hasCard(card => get.suit(card) === "club", "he");
		},
		filter: function(event, player) {
			if (!player.hasCard(card => get.suit(card) === "club", "he")) return false;
		...
		filterTarget: function(card, player, target) {
			const selected_card = ui.selected.cards[0];
			const event = _status.event, backup = event._backup;
			if (!selected_card || game.checkMod(selected_card, player, "unchanged", "cardEnabled2", player) === false) return false;
			const cardx = get.autoViewAs({ name: "tiesuo" }, [selected_card]);
		...
		filterOk: function() {
			...
			if (recast && selected === 0) return true;
			else if (use) {
				const select = backup.selectTarget(cardx, player);
		...
		content: async function(event, trigger, player) {
			...
			game.log(player, "将", event.cards, "作为【囚集】进行了重铸");
		},
		ai: {
			order: function(item, player) {
		...
		group: ["hanna_qiuji_effect"],
		subSkill: {
			effect: {
				trigger: { target: "useCardToTargeted" },
				forced: true,
				filter: function(event, player) {
					return event.player !== player && event.player.isLinked() && event.targets.length === 1 && event.targets[0] === player;
				},
				content: async function(event, trigger, player) {
					const target = trigger.player; 
					
					const res = await target.chooseControl("令其选择一种花色并获得你该花色的所有手牌，然后解除连环", "此牌对其无效")
						.set("prompt", "囚集：你正在指定 " + get.translation(player) + " 为唯一目标，请选择一项")
						.set("ai", function() {
							var att = get.attitude(_status.event.player, _status.event.target);
							if (att > 0) return "令其选择一种花色并获得你该花色的所有手牌，然后解除连环"; 
							if (_status.event.player.countCards("h") <= 1) return "令其选择一种花色并获得你该花色的所有手牌，然后解除连环";
							return "此牌对其无效";
						})
						.set("target", player).set("player", target).forResult();
						
					if (res.control === "令其选择一种花色并获得你该花色的所有手牌，然后解除连环") {
						const guessRes = await player.chooseControl(["spade", "heart", "club", "diamond"])
							.set("prompt", "囚集：请选择一种花色，获得 " + get.translation(target) + " 所有的该花色手牌")
							.set("choiceList", ["黑桃", "红桃", "梅花", "方片"])
							.set("target", target)
							.set("ai", function() {
								var currentTarget = _status.event.target; 
								var handCards = currentTarget.getCards("h");
								
								if (handCards.length === 0) return ["spade", "heart", "club", "diamond"].randomGet();
								
								var counts = { spade: 0, heart: 0, club: 0, diamond: 0 };
								for (var c of handCards) {
									var s = get.suit(c);
						...
						const suit = guessRes.control;
						game.log(player, "选择了花色", "#y" + get.translation(suit));
						
						const cards = target.getCards("h", c => get.suit(c) === suit);
						if (cards.length > 0) {
							await player.gain(cards, target, "giveAuto");
							game.log(player, "获得了", target, "的", cards.length, "张", get.translation(suit), "牌");
						} else {
							game.log(target, "没有", get.translation(suit), "手牌");
						}
						await target.link(false);
					} else {
						trigger.excluded.add(player);
						game.log(trigger.card, "受到", "#g【囚集】", "的影响，对", player, "无效了");
					}
				}
			}
		}
	...
	"kanae_wuxiang": {
		audio: "ext:魔法纪录/audio/skill:2",
		group: ["kanae_wuxiang_track", "kanae_wuxiang_resolve", "kanae_wuxiang_clear"],
		trigger: { global: "useCardToTargeted" },
		filter: function(event, player) {
			var used = player.storage.kanae_wuxiang_used || 0;
			return used < 2 && event.card.name === 'sha' && (player.inRange(event.target) || event.target === player);
		},
		cost: async function(event, trigger, player) {
			var res = await player.chooseBool("无想：是否摸一张牌并考虑交给 " + get.translation(trigger.target) + " 一张牌？")
				.set("ai", function() { return true; }).forResult();
			if (res.bool) event.result = { bool: true };
		},
		content: async function(event, trigger, player) {
			player.storage.kanae_wuxiang_used = (player.storage.kanae_wuxiang_used || 0) + 1;
			await player.draw();
			var target = trigger.target;
			var currentTrigger = trigger; 
			var res = await player.chooseToGive(target, "he", "无想：是否交给 " + get.translation(target) + " 一张牌并展示？")
				.set("ai", function(card) {
					var p = _status.event.player;
					var tar = _status.event.target;
					var att = get.attitude(p, tar);
					
					if (att <= 0) return 0;
					
					if (currentTrigger && currentTrigger.card && currentTrigger.card.name === 'sha' && card.name === 'shan') {
						return 20;
					}
					return 6 - get.value(card);
				}).set("target", target).forResult();
			...
			if (res.bool && res.cards && res.cards.length > 0) {
				player.showCards(res.cards, get.translation(player) + " 发动【无想】交给了 " + get.translation(target) + " 一张牌");
				var pEvent = trigger.getParent("useCard");
				if (!pEvent.kanae_wuxiang_map) pEvent.kanae_wuxiang_map = [];
				pEvent.kanae_wuxiang_map.push({ kanae: player, target: target });
			}
		},
		subSkill: {
			track: {
				...
				forced: true, silent: true, charlotte: true,
				filter: function(event, player) {
					var pEvent = event.getParent("useCard");
					return pEvent && pEvent.kanae_wuxiang_map !== undefined;
				},
				content: function(event, trigger, player) {
					var pEvent = trigger.getParent("useCard");
					if (!pEvent.kanae_wuxiang_damaged) pEvent.kanae_wuxiang_damaged = [];
					pEvent.kanae_wuxiang_damaged.push(trigger.player);
				}
			},
			resolve: {
				trigger: { global: "useCardAfter" },
				forced: true, charlotte: true,
				filter: function(event, player) {
					return event.kanae_wuxiang_map && event.kanae_wuxiang_map.some(pair => pair.kanae === player);
				},
				content: async function(event, trigger, player) {
					var damagedTargets = trigger.kanae_wuxiang_damaged || [];
					var myPairs = trigger.kanae_wuxiang_map.filter(pair => pair.kanae === player);
					
					for (var i = 0; i < myPairs.length; i++) {
						var target = myPairs[i].target;
						if (!damagedTargets.includes(target) && target.isAlive()) {
							game.log(player, "与", target, "因", "#g【无想】", "触发了出牌效果");
							await player.chooseToUse("无想：请使用一张可使用的牌")
								.set('ai', card => get.value(card));
							await target.chooseToUse("无想：请使用一张可使用的牌")
								.set('ai', card => get.value(card));
						}
					}
				}
			...
			clear: {
				...
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					player.storage.kanae_wuxiang_used = 0;
				}
			}
		}
	},
    "kanae_wuxiang_gai": { //不上
        audio: "ext:魔法纪录/audio/skill:2",
        forced: true,
        group: ["kanae_wuxiang_track", "kanae_wuxiang_gai_resolve", "kanae_wuxiang_gai_clear"],
        trigger: { global: "useCardToTargeted" },
        filter: function(event, player) {
            var used = player.storage.kanae_wuxiang_gai_used || 0;
            return used < 2 && get.tag(event.card, 'damage') && (player.inRange(event.target) || event.target === player);
        },
        content: async function(event, trigger, player) {
            player.storage.kanae_wuxiang_gai_used = (player.storage.kanae_wuxiang_gai_used || 0) + 1;
            await player.draw();
            var target = trigger.target;
            var currentTrigger = trigger; 
            var res = await player.chooseToGive(target, "he", "无想·改：是否交给 " + get.translation(target) + " 一张牌并展示？")
                .set("ai", function(card) {
                    var p = _status.event.player;
                    var tar = _status.event.target;
                    var att = get.attitude(p, tar);
                    if (att > 0) {
                        var trigCard = currentTrigger.card;
                        if (trigCard.name === 'sha' && card.name === 'shan') return 30;
                        if ((trigCard.name === 'juedou' || trigCard.name === 'nanman') && card.name === 'sha') return 25;
                        if (trigCard.name === 'wanjian' && card.name === 'shan') return 25;
                        if (get.type(trigCard) === 'trick' && card.name === 'wuxie') return 40;
                        if (tar.hp <= 0 && card.name === 'tao') return 50; 
                        return 6 - get.value(card);
                    } else if (att < 0) {
                        return get.value(card) < 3 ? 10 : -1;
                    }
                    return 0;
                }).set("target", target).forResult();
            if (res.bool && res.cards && res.cards.length > 0) {
                player.showCards(res.cards, get.translation(player) + " 交给了 " + get.translation(target) + " 一张牌");
                var pEvent = trigger.getParent("useCard");
                if (!pEvent.kanae_wuxiang_map) pEvent.kanae_wuxiang_map = [];
                pEvent.kanae_wuxiang_map.push({ kanae: player, target: target });
            }
        ...
        subSkill: {
			resolve: {
				...
				content: async function(event, trigger, player) {
					...
					for (var i = 0; i < myPairs.length; i++) {
						var target = myPairs[i].target;
						if (!damagedTargets.includes(target) && target.isAlive()) {
							game.log(player, "与", target, "因", "#g【无想·改】", "触发了出牌效果");
							var useFilter = function(card) { return get.type(card) !== 'equip' && get.position(card) === 'h'; };
							
							await player.chooseToUse("无想·改：请使用一张手牌区非装备牌")
								.set('filterCard', useFilter).set('ai', card => get.value(card));
							await target.chooseToUse("无想·改：请使用一张手牌区非装备牌")
								.set('filterCard', useFilter).set('ai', card => get.value(card));
						}
					}
				}
			...
			clear: {
				...
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					player.storage.kanae_wuxiang_gai_used = 0;
				}
			}
        }
	},
	"kanae_huien": {
		...
		dutySkill: true,
		locked: false,
		derivation: ["kanae_wuxiang_gai"],
		group: [
			"kanae_huien_yushi", "kanae_huien_tao", 
			"kanae_huien_record", "kanae_huien_clear",
			"kanae_huien_boost_mark", "kanae_huien_buff_intercept",
			"kanae_huien_success"
			//, "kanae_huien_fail" 太强不上
		],
		subSkill: {
			yushi: {
				...
				filter: function(event, player) {
					if (player.storage.kanae_huien_used) return false;
					if (event.type === "phase") {
						if (_status.event && _status.event.name === 'AI') { 
							var enemies = player.getEnemies();
							var hasLowHpEnemy = enemies.some(e => e.hp <= 1);
							var hasMidHpEnemy = enemies.some(e => e.hp <= 2);
							var lostHp = player.maxHp - player.hp;
							var equips = player.getEquips();
							var hasCheapBlackEquip = equips.some(card => get.color(card) === 'black' && get.value(card) <= 5);
							var hasExtraEquip = equips.length >= 2;
							if (!(hasLowHpEnemy || (lostHp >= 2 && hasMidHpEnemy) || (hasCheapBlackEquip && hasExtraEquip))) {
								return false;
							}
						}
						return true;
					}
					return event.filterCard ? event.filterCard({ name: "yushijiesui" }, player, event) : true;
				},
				selectCard: 1,
				prompt: "将一张黑色装备当【玉石皆碎】使用（此牌数值+X）",
				check: function(card) { return 6 - get.value(card); }
			},
			tao: {
				enable: "chooseToUse",
				...
				filter: function(event, player) { 
					if (player.storage.kanae_huien_used) return false;
					if (!player.hasCards("h", card => get.color(card) === "red" && get.type(card) === "trick")) return false;
					if (event.filterCard) {
						return event.filterCard({ name: "tao", isCard: true }, player, event);
					}
					return _status.event.dying || game.hasPlayer(p => p.isDying());
				},
				viewAsFilter: function(player) {
					if (player.storage.kanae_huien_used) return false;
					return player.hasCards("h", card => get.color(card) === "red" && get.type(card) === "trick");
				},
				hiddenCard: function(player, name) {
					if (player.storage.kanae_huien_used) return false;
					if (name === "tao") {
			...
			record: {
				...
				forced: true, silent: true, charlotte: true,
				filter: function(event, player) {
					return event.skill === 'kanae_huien_yushi' || event.skill === 'kanae_huien_tao';
				},
				content: function(event, trigger, player) {
					player.storage.kanae_huien_used = true; 
				}
			},
			clear: {
				trigger: { global: "roundStart" },
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					delete player.storage.kanae_huien_used; 
				}
			},
			...
			boost_mark: {
				...
				content: function(event, trigger, player) {
					...
					if (lostHp > 0) {
						...
						trigger.customArgs.kanae_boost = lostHp;
						game.log(player, "凝聚了", lostHp, "点已损失体力，注入了", trigger.card);
					}
				}
			},
			buff_intercept: {
				...
				filter: function(event, player) {
					var pEvent = event.getParent("useCard");
					return pEvent && pEvent.customArgs && typeof pEvent.customArgs.kanae_boost === 'number';
				},
				content: function(event, trigger, player) {
					var pEvent = trigger.getParent("useCard");
					var boost = pEvent.customArgs.kanae_boost;
					trigger.num += boost;
					game.log(trigger.card, "爆发了", "#y【惠恩】", "，数值额外", "#r+" + boost);
				}
			},
			success: {
				audio: "ext:魔法纪录/audio/skill:2",
				...
				forced: true,
				filter: function(event, player) {
					return !(player.awoken && player.awoken.includes("kanae_huien"));
				},
				content: async function(event, trigger, player) {
					player.awakenSkill("kanae_huien");
					game.log(player, "成功完成使命");
					
					var res = await player.chooseTarget("使命成功：请选择一名角色，视为对其使用【悲叹之种】", function(card, p, target) {
						return true;
					}).set("ai", function(target) {
					...
					if (res.bool && res.targets && res.targets.length > 0) {
						...
						await target.addSkills("griefseed_skill");
						
						var griefseed = game.createCard("griefseed");
						await player.useCard(griefseed, target, false);
					}
				}
			},
			//fail: {
				...
				//forced: true,
				//filter: function(event, player) {
					//return player.hp > 0 && !(player.awoken && player.awoken.includes("kanae_huien"));
				//},
				//content: function(event, trigger, player) {
					//player.awakenSkill("kanae_huien");
					...
					//player.gainMaxHp(1);
					//player.recover(1);
					//player.removeSkill("kanae_wuxiang");
					//player.addSkill("kanae_wuxiang_gai");
					//game.log(player, "脱离了既定的宿命，获得了", "#y【无想·改】");
				//}
			//}
		}
	},
    ...
    "meru_zhanxing": {
		...
		async cost(event, trigger, player) {
			event.result = await player.chooseCard(
				"占星：你可以打出一张牌代替 " + get.translation(trigger.player) + " 的判定牌", 
				"he", 
				function(card) { return true; }
			).set("ai", function(card) {
				var player = _status.event.player;
				var triggerEvent = _status.event.getTrigger();
				var target = triggerEvent.player; 
				var att = get.attitude(player, target);
				var oldCard = target.judging[0];
				var suit = get.suit(card);
				var oldSuit = get.suit(oldCard);
				var score = 0;
				
				score += 5 - get.value(card);
				
				if (att > 0) { 
					if (suit === oldSuit) score += 5; 
					if (suit === 'heart') {
						if (player.isDamaged() || target.isDamaged()) score += 10; 
						else score += 2;
					}
					if (suit === 'club') score -= 15; 
				} else { 
					if (suit === oldSuit) score -= 5; 
					if (suit === 'heart') {
						if (player.hp === 1) score += 5; 
						else score -= 20; 
					}
					if (suit === 'club') score += 12; 
				}
				
				var judgeName = triggerEvent.judgename || oldCard.viewAs || oldCard.name;
				
				if (judgeName === 'meru_zhenbu') {
					if (att > 0) { 
						if (suit === 'diamond') score += 15; 
						if (suit === 'spade') score -= 20;   
					} else { 
						if (suit === 'spade' && player.hp > 1) score += 15; 
						if (suit === 'diamond') score -= 15; 
					}
				} else if (judgeName === 'lebusishu') {
					if (att > 0) score += (suit === 'heart' ? 12 : -10);   
					else score += (suit !== 'heart' ? 10 : -15); 
				} else if (judgeName === 'bingliangcunduan') {
					if (att > 0) score += (suit === 'club' ? 12 : -10);
					else score += (suit !== 'club' ? 10 : -15);
				} else if (judgeName === 'shandian') {
					var isSpade2to9 = (suit === 'spade' && card.number >= 2 && card.number <= 9);
					if (att > 0) score += (isSpade2to9 ? -20 : 10);
					else score += (isSpade2to9 ? 20 : -10);
				}
		...
		popup: false,
		async content(event, trigger, player) {
			const next = player.respond(event.cards, "meru_zhanxing", "highlight", "noOrdering");
			await next;
			if (next?.cards?.length) {
				...
				await player.gain(oldCard, "gain2");
				game.log(player, "获得了原判定牌", oldCard);

				await game.delay(2);
				
				var suit = get.suit(card);
				var isSelf = (trigger.player === player);
				
				if (suit === oldSuit) {
					game.log("花色一致，触发摸牌效果！");
					if (isSelf) {
						await player.draw(2);
					} else {
						await player.draw(1);
						if (trigger.player.isAlive()) await trigger.player.draw(1);
					}
				}
				...
				if (suit === "heart") {
					if (isSelf) {
						...
						await player.recover(1);
						if (trigger.player.isAlive()) await trigger.player.recover(1);
					}
				} else if (suit === "club") {
					if (isSelf) {
						var discardRes = await player.chooseBool("占星：是否弃置两张牌？").set("ai", () => true).forResult();
						if (discardRes?.bool) {
							await player.chooseToDiscard(2, "he", true);
						}
					} else {
						var discardRes = await player.chooseBool("占星：是否弃置你与其各一张牌？").set("ai", () => true).forResult();
						if (discardRes?.bool) {
							await player.chooseToDiscard(1, "he", true, "占星：请弃置自己一张牌");
							if (trigger.player.isAlive() && trigger.player.countCards("he") > 0) {
								await player.discardPlayerCard(trigger.player, "he", 1, true, "占星：请选择并弃置 " + get.translation(trigger.player) + " 的一张牌");
							}
						}
					}
				}
	...
	"meru_zhenbu": {
		...
		filter: function(event, player) {
			...
			var limit = Math.ceil(game.countPlayer() / 2);
			var used = player.storage.meru_zhenbu_used || 0;
			return used < limit;
		},
		content: async function(event, trigger, player) {
			var target = trigger.player;
			var promptStr = "真卜：是否进行一次判定？(方片：你与其各摸一；黑桃：你与其各流失1体力，回合结束回1血)";
			...
			var res = await player.chooseBool(promptStr).set("ai", function() {
				...
				var cards = player.getCards('he');
				var hasDiamond = cards.some(c => get.suit(c) === 'diamond');
				var hasHeart = cards.some(c => get.suit(c) === 'heart');
				var hasSpade = cards.some(c => get.suit(c) === 'spade');
				var hasClub = cards.some(c => get.suit(c) === 'club');
				
				if (att > 0) { 
					if (hasDiamond || hasHeart) return true;
					if (player.hp >= 3 && target.hp >= 3) return true;
					return false;
				} else {
					if (hasSpade && player.hp > 1) return true;
			...
			player.storage.meru_zhenbu_used = (player.storage.meru_zhenbu_used || 0) + 1;
			game.log(player, "发动了", "#g【真卜】", "（当前第 " + player.storage.meru_zhenbu_used + " 次）");
			
			var next = player.judge("meru_zhenbu");
			await next;
			var suit = next.result.suit;
			...
			if (suit === "diamond") {
				if (isSelf) {
					...
					await player.draw(1);
					if (target && target.isAlive()) await target.draw(1);
				}
			} else if (suit === "spade") {
				if (isSelf) {
					await player.loseHp(2);
					if (player.isAlive()) {
						player.addTempSkill("meru_zhenbu_recover", "phaseJieshuAfter");
						player.addMark("meru_zhenbu_recover", 2, false);
					}
				} else {
					await player.loseHp(1);
					...
					if (player.isAlive()) {
						player.addTempSkill("meru_zhenbu_recover", "phaseJieshuAfter");
						player.addMark("meru_zhenbu_recover", 1, false);
					}
					if (target && target.isAlive()) {
						target.addTempSkill("meru_zhenbu_recover", "phaseJieshuAfter");
						target.addMark("meru_zhenbu_recover", 1, false);
					}
				}
			}
		},
		group: "meru_zhenbu_clear",
		subSkill: {
			clear: {
				trigger: { global: "roundStart" },
				forced: true, silent: true, charlotte: true,
				content: function(event, trigger, player) {
					player.storage.meru_zhenbu_used = 0;
				}
			},
			recover: {
				trigger: { global: "phaseJieshuBegin" },
				...
				onremove: function(player) {
					player.removeMark("meru_zhenbu_recover", player.countMark("meru_zhenbu_recover"));
				},
				content: async function(event, trigger, player) {
					var count = player.countMark("meru_zhenbu_recover");
					if (count > 0) {
						game.log(player, "因", "#g【真卜】", "的延迟效果，回复了", count, "点体力");
						await player.recover(count);
					}
					player.removeSkill("meru_zhenbu_recover");
				},
				mark: true,
				marktext: "卜",
				intro: { content: "回合结束时回复 # 点体力" }
			}
		}
	},
	...
	"meru_zhiming": { //不上
		...
		forced: true,
		content: async function(event, trigger, player) {
			var declareRes = await player.chooseControl("heart", "diamond", "club", "spade")
				.set("prompt", "知命：请声明一种花色")
				.set("ai", () => ["heart", "diamond", "club", "spade"].randomGet())
				.forResult();
			var declaredSuit = declareRes.control;
			game.log(player, "声明了花色", "#y" + get.translation(declaredSuit));
			
			var card = get.cards()[0];
			player.showCards([card], get.translation(player) + " 展示了牌堆顶的牌");
			var realSuit = get.suit(card);
			
			player.storage.meru_zhiming_suit = realSuit;
			player.addTempSkill("meru_zhiming_suitmod", { player: "phaseBegin" });
			
			if (declaredSuit === realSuit) {
				game.log("占卜", "#g成功！");
				await player.gain(card, "gain2");
				player.addTempSkill("meru_zhiming_buff", { player: "phaseBegin" });
			} else {
				game.log("占卜", "#r失败！");
				await player.loseHp(1);
				
				card.fix(); 
				var pos = Math.floor(Math.random() * ui.cardPile.childNodes.length);
				ui.cardPile.insertBefore(card, ui.cardPile.childNodes[pos]);
				game.updateRoundNumber();
				game.log(card, "被随机置入了牌堆");
				
				player.addTempSkill("meru_zhiming_debuff", { player: "phaseBegin" });
			}
		},
		subSkill: {
			suitmod: {
				charlotte: true,
				mod: {
					suit: function(card, player, suit) {
						if (get.owner(card) === player && get.position(card) === 'h') {
							return player.storage.meru_zhiming_suit;
						}
					}
				}
			},
			buff: {
				...
				forced: true,
				filter: function(event, player) {
					return event.result && event.result.suit === player.storage.meru_zhiming_suit;
				},
				content: async function(event, trigger, player) {
					game.log(player, "因", "#g【知命】", "的精准占卜摸了一张牌");
					await player.draw();
				}
			},
			debuff: {
				...
				forced: true,
				filter: function(event, player) {
					if (!event.card || get.suit(event.card) !== player.storage.meru_zhiming_suit) return false;
					var type = get.type(event.card);
				...
				content: async function(event, trigger, player) {
					game.log(player, "因", "#r【知命】", "的反噬，强制成为了", trigger.card, "的额外目标！");
					
					if (trigger.getParent().targets.add) {
	...
	"rera_ziru": {
		...
		init: function(player) {
			if (!player.storage.rera_ziru) {
				player.addSkill("rera_yanlun");
			} else {
				player.addSkill("rera_nuanxin");
			}
		},
		intro: {
			content: function(storage, player) {
				var count = _status.renku ? _status.renku.length : 0;
				if (storage) {
					return "转换技（阴）。你拥有技能【暖心】。<br>当前仁区牌数：<b>" + count + "</b>";
				}
				return "转换技（阳）。你拥有技能【焰轮】。<br>当前仁区牌数：<b>" + count + "</b>";
			}
		},
		group: ["rera_ziru_trigger"],
		subSkill: {
			trigger: {
				trigger: {
				...
				filter: function(event, player) {
					...
					if (player.getStat().skill.rera_ziru > 0) return false;
				...
				content: async function(event, trigger, player) {
					...
					if (!stat.skill) stat.skill = {};
					stat.skill.rera_ziru = (stat.skill.rera_ziru || 0) + 1;

					player.logSkill("rera_ziru");

					var evt = trigger.getl(player);
					var discardCards = evt.cards2.filter(c => get.position(c) === "d");

					if (discardCards.length > 0) {
						var renkuRes = await player.chooseButton([
							"自如：是否将其中一张牌置入仁区？",
							discardCards
						]).set("ai", function(button) {
							var card = button.link;
							var val = get.value(card);
							var curR = _status.renku ? _status.renku.length : 0;
							if (["tao", "wuxie", "jiu"].includes(get.name(card))) return val + 6;
							if (get.type(card) === "equip") return val + 4;
							if (curR >= 5) return 2 - val;
							return 6 - val;
						}).forResult();

						if (renkuRes.bool && renkuRes.links && renkuRes.links.length > 0) {
							var toRenkuCard = renkuRes.links[0];
							if (!_status.renku) _status.renku = [];
							_status.renku.push(toRenkuCard);
							game.log(player, "将", toRenkuCard, "从弃牌堆置入了", "#y【仁区】");
							
							while (_status.renku.length > 6) {
								var overflowCard = _status.renku.shift();
								_status.renku_decreased_this_turn = true;
								game.cardsDiscard(overflowCard);
								game.log(overflowCard, "因仁区超过6张上限被置入弃牌堆");
							}
							await game.delayx();
						}
					}
					...
					player.changeZhuanhuanji("rera_ziru");
					var isYin = player.storage.rera_ziru;
					if (isYin) {
						player.removeSkill("rera_yanlun");
						player.addSkill("rera_nuanxin");
						game.log(player, "将【自如】转换为了", "#r【阴·暖心】");
					} else {
						player.removeSkill("rera_nuanxin");
						player.addSkill("rera_yanlun");
						game.log(player, "将【自如】转换为了", "#y【阳·焰轮】");
					}
				}
			}
		}
	...
	"rera_xiuhao": {
		...
		selectCard: [1, 3],
		position: "he",
		prompt: "修好：弃置任意张牌，令一名角色摸X+1张牌并整理手牌置入仁区",
		check: function(card) {
			var player = _status.event.player;

			if (["tao", "shan", "jiu", "wuxie"].includes(get.name(card)) && player.hp <= 2) return 0;

			var curH = player.countCards("h");
			if (curH > player.hp && get.value(card) <= 4) return 5 - get.value(card);
			return 6 - get.value(card);
		},
		filterTarget: function(card, player, target) {
			return true;
		...
		ai: {
			order: function() {
				...
				if (player.getStat().skill && player.getStat().skill.rera_ziru > 0) return 9;
				return 7;
			},
			result: {
				target: function(player, target) {
					if (target !== player && get.attitude(player, target) <= 0) return 0;

					if (target !== player && target.hp <= 2 && target.isDamaged()) return 16;

					if (target === player) {
						if (player.getStat().skill && player.getStat().skill.rera_ziru > 0) return 10;
						return 4;
					}
		...
		content: async function(event, trigger, player) {
			...
			if (target.isIn() && target.countCards("h") > 0) {
				...
				for (var i = 0; i < handcards.length; i++) {
					var s = get.suit(handcards[i]);
					if (s && s !== "none") suits.add(s);
				}

				var chosenSuit = null;
				if (suits.length > 0) {
					var suitList = suits.map(s => [s, get.translation(s) + "花色"]);
					var suitChoice = await target.chooseButton(["修好：是否将一种花色的所有手牌置入仁区？", [suitList, "textbutton"]]).set("ai", function(button) {
						var s = button.link;
						var cards = _status.event.player.getCards("h", c => get.suit(c) === s);
						return 15 - cards.reduce((sum, c) => sum + get.value(c), 0);
					}).forResult();

					if (suitChoice.bool && suitChoice.links && suitChoice.links.length > 0) {
						chosenSuit = suitChoice.links[0];
						var cardsToRenku = target.getCards("h", c => get.suit(c) === chosenSuit);
						
						if (cardsToRenku.length > 0) {
							target.$throw(cardsToRenku, 1000);
							game.log(target, "将", cardsToRenku, "置入了", "#y【仁区】");
							await target.lose(cardsToRenku, ui.special, "toRenku");
							
							if (!_status.renku) _status.renku = [];
							_status.renku.addArray(cardsToRenku);
							while (_status.renku.length > 6) {
								var overflowCard = _status.renku.shift();
								_status.renku_decreased_this_turn = true;
								game.cardsDiscard(overflowCard);
				...
				if (chosenSuit) {
					var choices = ["选项一", "选项二"];
					var choiceList = [
						"令" + get.translation(player) + "摸一张牌并重置【自如】",
						"回复1点体力"
					];
					var ctrlRes = await target.chooseControl(choices).set("choiceList", choiceList).set("prompt", "修好：请选择一项").set("ai", function() {
						var p = _status.event.player;
						var rera = _status.event.rera;
						if (p.isDamaged() && p.hp <= 2) return "选项二";
						if (get.attitude(p, rera) > 0) return "选项一";
						return p.isDamaged() ? "选项二" : "选项一";
					}).set("rera", player).forResult();

					if (ctrlRes.control === "选项一") {
						if (player.isIn()) {
							await player.draw(1);
							if (player.getStat().skill) {
								delete player.getStat().skill.rera_ziru;
							}
							game.log(target, "令", player, "重置了技能", "#g【自如】");
						}
					} else if (ctrlRes.control === "选项二") {
						if (target.isDamaged()) {
	...
	"rera_yanlun": {
		...
		filter: function(event, player) {
			...
			if (x <= 0) return false;
			return game.hasPlayer(target => target.isIn() && target.hp <= x && target.countCards("hej") > 0);
		},
		cost: async function(event, trigger, player) {
			var x = _status.renku ? _status.renku.length : 0;
			var next = player.chooseTarget("焰轮：是否对一名体力值不大于" + x + "的角色造成1点火焰伤害并获得其区域内一张牌？", function(card, player, target) {
				var curX = _status.renku ? _status.renku.length : 0;
				return target.isIn() && target.hp <= curX && target.countCards("hej") > 0;
			});
			next.set("ai", function(target) {
				var p = _status.event.player;
				var att = get.attitude(p, target);
				if (att >= 0) return -10;
				var score = -att * 2;
				if (target.hasSkill("seika_huanzhu") && target.countCards("h") === target.hp) score += 8;
				if (target.hp <= 1) score += 6;
				if (target.getEquip(2) && target.getEquip(2).name === "tengjia") score += 4;
				if (target.countCards("j") > 0) score += 3;
				return score;
			});
			var res = await next.forResult();
			if (res.bool && res.targets && res.targets.length > 0) {
				event.result = { bool: true, targets: res.targets };
			}
		},
		content: async function(event, trigger, player) {
			var target = event.targets[0];
			player.logSkill("rera_yanlun", target);
			
			await target.damage(1, "fire");

			if (player.isIn() && target.isIn() && target.countCards("hej") > 0) {
	...
	"rera_nuanxin": {
		...
		cost: async function(event, trigger, player) {
			var x = _status.renku ? _status.renku.length : 0;
			var next = player.chooseTarget([1, x], "暖心：是否令至多" + x + "名角色各摸一张牌？", function(card, player, target) {
				return target.isIn();
			});
			next.set("ai", function(target) {
				var p = _status.event.player;
				...
				if (_status.renku && _status.renku.length > 0) {
					renkuVal = Math.max(..._status.renku.map(c => get.value(c)));
				}
				if (renkuVal >= 7 && target !== p && ui.selected.targets.length >= x - 1) {
					return -1;
				}
				return att + (target === p ? 2 : 0);
			});
			var res = await next.forResult();
		...
		content: async function(event, trigger, player) {
			...
			player.logSkill("rera_nuanxin", targets);
			...
			if (targets.length < x && _status.renku && _status.renku.length > 0) {
				var res = await player.chooseButton(["暖心：选择获得仁区内的一张牌", _status.renku], true).set("ai", function(button) {
					return get.value(button.link);
				}).forResult();

				if (res.bool && res.links && res.links.length > 0) {
					var gainCard = res.links[0];
					_status.renku.remove(gainCard);
					_status.renku_decreased_this_turn = true;
					await player.gain([gainCard], "gain2");
					game.log(player, "从", "#y【仁区】", "获得了", gainCard);
				}
			}
		}
	},
	...
	"rera_nuanxing": {
		...
		group: ["rera_nuanxing_gift"],
		subSkill: {
			gift: {
				sourceSkill: "rera_nuanxing",
				trigger: {
					player: "giftAccepted",
				},
	...
	"seika_huanzhu": {
		...
		filter: function(event, player) {
			var roundStat = player.getStat("round");
			if (roundStat && roundStat.seika_huanzhu >= 1) return false;
			if (game.players.length <= 2) return false;
			
			var h = player.countCards("h");
			var hp = player.hp;
			var r = _status.renku ? _status.renku.length : 0;
			return h === hp && hp === r;
		},
		cost: async function(event, trigger, player) {
			var current = trigger.player;
			var others = game.filterPlayer(p => p !== current);
			if (!others.length) return;

			var next = player.chooseTarget(
				"还珠：选择一名其他角色与 " + get.translation(current) + " 交换座次",
				function(card, player, target) {
					return target !== _status.event.current;
				}
			);
			next.set("current", current);
			next.set("ai", function(target) {
				var p = _status.event.player;
				var cur = _status.event.current;
				var attCur = get.attitude(p, cur);
				var attTgt = get.attitude(p, target);

				if (attCur <= 0) {
					if (attTgt <= 0) return (target.hp >= 3 ? 3 : 1);
					if (target.hp <= 2) return -10;
					return -2;
				}
				if (attTgt > 0) return 5 + attTgt;
				return -3;
			});

			var res = await next.forResult();
			if (res.bool && res.targets && res.targets.length > 0) {
				event.result = { bool: true, targets: [current, res.targets[0]] };
			}
		},
		content: async function(event, trigger, player) {
			var stat = player.getStat();
			if (!stat.round) stat.round = {};
			stat.round.seika_huanzhu = (stat.round.seika_huanzhu || 0) + 1;

			var t1 = event.targets[0];
			var t2 = event.targets[1];
			player.logSkill("seika_huanzhu", [t1, t2]);

			game.broadcastAll(function(target1, target2) {
				game.swapSeat(target1, target2);
			}, t1, t2);
			game.log(t1, "与", t2, "交换了座次");
			await game.delayx();

			var x = _status.renku ? _status.renku.length : 0;
			if (x > 0) {
				var drawTargetRes = await player.chooseTarget(
					"还珠：选择其中一名角色摸 " + x + " 张牌",
					true,
					function(card, player, target) {
						return _status.event.validTargets.includes(target);
					}
				).set("validTargets", [t1, t2]).set("ai", function(target) {
					var p = _status.event.player;
					var att = get.attitude(p, target);
					if (att > 0) {
						return 10 + att + (4 - target.countCards("h")) + (target.hp <= 2 ? 3 : 0);
					}
					return -10 - (target.countCards("h"));
				}).forResult();

				if (drawTargetRes.bool && drawTargetRes.targets && drawTargetRes.targets.length > 0) {
					var luckyTarget = drawTargetRes.targets[0];
					await luckyTarget.draw(x);
				}
			}
		},
		ai: {
	...
	"seika_heying": {
		...
		filter: function(event, player) {
			...
			var hasRenku = _status.renku && _status.renku.length > 0;
			var hasCards = target.countCards("hej") > 0;
			return hasRenku || hasCards;
		},
		cost: async function(event, trigger, player) {
			var target = trigger.player;
			var hasRenku = _status.renku && _status.renku.length > 0;
			...
			var choices = [];
			var choiceList = [
				`令${get.translation(target)}获得仁区内的一张牌`,
				`弃置${get.translation(target)}区域内一张牌并将此牌置入仁区`
			];

			if (hasRenku) choices.push("选项一");
			else choiceList[0] = '<span style="opacity:0.5;">' + choiceList[0] + "</span>";

			if (hasCards) choices.push("选项二");
			else choiceList[1] = '<span style="opacity:0.5;">' + choiceList[1] + "</span>";

			choices.push("cancel2");

			var next = player.chooseControl(choices);
			next.set("prompt", "涸盈：是否对 " + get.translation(target) + " 发动【涸盈】？");
			next.set("choiceList", choiceList);
			next.set("ai", function() {
				var p = _status.event.player;
				var trg = _status.event.target;
				var att = get.attitude(p, trg);
				var myH = p.countCards("h");
				var trgH = trg.countCards("h");

				if (att > 0) {
					if (trg.countCards("j") > 0 && _status.event.controls.includes("选项二")) {
						return "选项二";
					}
					if (trg.hasSkill("mito_liangyi") && !trg.hasSkill("mito_liangyi_other_used") && _status.event.controls.includes("选项一")) {
						return "选项一";
					}
					if (_status.event.controls.includes("选项一")) {
						if (trgH + 1 === myH) return "选项一";
						return "选项一";
					}
					if (_status.event.controls.includes("选项二")) {
						if (trgH - 1 === myH && trgH >= 3) return "选项二";
					}
					return "cancel2";
				}

				if (att < 0) {
					if (trg.hasSkill("rera_yanlun") && _status.event.controls.includes("选项一")) {
						return "选项一";
					}
					if (_status.event.controls.includes("选项二")) {
						if (trg.isDamaged() && trgH - 1 === myH) return "cancel2";
						return "选项二";
					}
				}
			...
			var res = await next.forResult();
			if (res.control !== "cancel2") {
				event.result = { bool: true, cost_data: res.control };
			}
		},
		content: async function(event, trigger, player) {
			var target = trigger.player;
			var choice = event.cost_data;
			player.logSkill("seika_heying", target);

			if (choice === "选项一") {
				var gainRes = await target.chooseButton(["涸盈：获得仁区内的一张牌", _status.renku], true).set("ai", function(btn) {
					return get.value(btn.link);
				}).forResult();

				if (gainRes.bool && gainRes.links && gainRes.links.length > 0) {
					var gainC = gainRes.links[0];
					_status.renku.remove(gainC);
					_status.renku_decreased_this_turn = true;
					await target.gain([gainC], "gain2");
					game.log(target, "从", "#y【仁区】", "获得了", gainC);
				}
			} else if (choice === "选项二") {
				var disRes = await player.choosePlayerCard(target, "hej", true, "涸盈：选择并弃置 " + get.translation(target) + " 区域内的一张牌置入仁区").set("ai", function(button) {
					var card = button.link;
					var p = _status.event.player;
					var trg = _status.event.target;
					var att = get.attitude(p, trg);
					...
					if (att > 0) {
						if (pos === "j") return 30;
						if (["tao", "shan", "jiu", "wuxie"].includes(get.name(card)) && trg.hp <= 2) return 0;
						return 6 - get.value(card);
					}

					if (att < 0) {
						if (pos === "j") return 0;
						if (trg.hp <= 1 && trg.countCards("h") <= 1 && pos === "h") return 25;
						if (pos === "e") {
							var equipVal = get.value(card);
							if (get.subtype(card) === "equip2" || get.subtype(card) === "equip3") return 18 + equipVal;
							return 12 + equipVal;
						}
						return 6 + get.value(card);
					}
				...
				if (disRes.bool && disRes.links && disRes.links.length > 0) {
					var toRenkuCard = disRes.links[0];
					target.$throw([toRenkuCard], 1000);
					await target.lose([toRenkuCard], ui.special, "toRenku");
					
					if (!_status.renku) _status.renku = [];
					_status.renku.push(toRenkuCard);
					game.log(player, "将", target, "的", toRenkuCard, "置入了", "#y【仁区】");
			...
			if (player.isIn() && target.isIn() && player.countCards("h") === target.countCards("h")) {
				...
				var choiceList = [
					...
					"与" + get.translation(target) + "各摸一张牌"
				];

				var ctrlRes = await player.chooseControl(choices).set("choiceList", choiceList).set("prompt", "涸盈：你的手牌数与 " + get.translation(target) + " 相同，请选择一项").set("ai", function() {
					var p = _status.event.player;
					var trg = _status.event.target;
					var att = get.attitude(p, trg);

					if (att > 0) {
						if (trg.isDamaged() && trg.hp <= 2) return "选项一";
						return "选项二";
					}
					return "选项二";
				}).set("target", target).forResult();
	...
	"seika_manyi": {
		...
		filter: function(event, player) {
			...
			var curCount = _status.renku ? _status.renku.length : 0;
			if (typeof _status.renku_snapshot_count === "number" && curCount < _status.renku_snapshot_count) {
				return true;
			}

			var hasDiscarded = false;
			game.checkGlobalHistory("cardMove", function(evt) {
				if (evt.type === "discard" && evt.cards && evt.cards.some(c => get.position(c, true) === "d")) {
					hasDiscarded = true;
				}
			});
			return hasDiscarded;
		...
		cost: async function(event, trigger, player) {
			var next = player.chooseTarget("满溢：是否令一名角色摸一张牌？");
			next.set("ai", function(target) {
				var p = _status.event.player;
				var att = get.attitude(p, target);
				if (att <= 0) return -1;
				...
				var myH = p.countCards("h");
				var myHp = p.hp;
				var renkuCount = _status.renku ? _status.renku.length : 0;

				if (target === p && myH + 1 === myHp && myHp === renkuCount) {
					return 20;
				}
				if (target.countCards("h") <= 1) return att * 2 + 6;
				if (target.countCards("h") + 1 === myH) return att * 2 + 3;

				return att + (target === p ? 1 : 0);
			});
		...
		content: async function(event, trigger, player) {
			var target = event.targets[0];
			player.logSkill("seika_manyi", target);

			await target.draw(1);

			if (target.isIn() && player.isIn() && target.countCards("h") === player.countCards("h")) {

				var discardCards = [];
				game.checkGlobalHistory("cardMove", function(evt) {
					if (evt.type === "discard" && evt.cards) {
						var valid = evt.cards.filter(c => get.position(c, true) === "d");
						discardCards.addArray(valid);
					}
				});
				...
				if (discardCards.length > 0) choices.push("选项一");
				else choiceList[0] = '<span style="opacity:0.5;">' + choiceList[0] + "</span>";
				choices.push("选项二");

				var res = await target.chooseControl(choices).set("prompt", "满溢：你的手牌数与清佳相同，请选择一项").set("choiceList", choiceList).set("ai", function() {
					var trg = _status.event.player;
					var p = _status.event.seika;
					var att = get.attitude(trg, p);
					var hasDiscard = _status.event.hasDiscard;

					if (att > 0) {
						if (hasDiscard) return "选项一";
						return "选项二";
					}
					return hasDiscard ? "选项一" : "选项二";
				}).set("seika", player).set("hasDiscard", discardCards.length > 0).forResult();

				if (res.control === "选项一" && discardCards.length > 0) {
					var cardRes = await target.chooseButton(["满溢：选择获得弃牌堆中的一张牌", discardCards], true).set("ai", function(button) {
						return get.value(button.link);
					}).forResult();

					if (cardRes.bool && cardRes.links && cardRes.links.length > 0) {
						await target.gain(cardRes.links, "gain2");
					}
				} else {
					await player.draw(1);
				}
			...
			delete _status.renku_decreased_this_turn;
			delete _status.renku_snapshot_count;
		},
		group: ["seika_manyi_recorder", "seika_manyi_cleanup"],
		subSkill: {
			recorder: {
				trigger: { global: "phaseUseBefore" },
				forced: true,
				silent: true,
				content: function() {
					_status.renku_snapshot_count = _status.renku ? _status.renku.length : 0;
				}
			},
			cleanup: {
				trigger: { global: "phaseAfter" },
	...
	"mito_liangyi": {
		audio: "ext:魔法纪录/audio/skill:2",
		group: ["mito_liangyi_self", "mito_liangyi_other"],
		subSkill: {
			self: {
				audio: "ext:魔法纪录/audio/skill:2",
				trigger: { player: "gainAfter" },
				filter: function(event, player) {
					if (player.hasSkill("mito_liangyi_self_used")) return false;
					if (!_status.renku || !_status.renku.length) return false;
					if (!game.hasPlayer(p => p !== player && p.isIn())) return false;
					if (event.fromPile) return false;
					var pName = event.getParent().name;
					if (pName && (pName.startsWith("draw") || pName === "phaseDraw")) return false;
					return event.cards && event.cards.length > 0;
				},
				cost: async function(event, trigger, player) {
					var next = player.chooseButton([
						"良谊①：是否将仁区内的一张牌交给一名其他角色并摸一张牌？",
						_status.renku
					]);
					next.set("ai", function(btn) {
						var card = btn.link;
						var val = get.value(card);
						if (["tao", "wuxie", "jiu"].includes(get.name(card))) return val + 5;
						if (get.type(card) === "equip") return val + 3;
						return val;
					});
					var res = await next.forResult();
					if (res.bool && res.links && res.links.length > 0) {
						var chosenCard = res.links[0];
						var targetRes = await player.chooseTarget("良谊①：将此牌交给一名其他角色", true, function(card, p, target) {
							return target !== p;
						}).set("ai", function(target) {
							var p = _status.event.player;
							var att = get.attitude(p, target);
							...
							if (att < 0 && target.hasSkill("seika_huanzhu") && target.countCards("h") === target.hp && target.hp === renkuCount) {
								return 18;
							}
							if (att <= 0) return -10;

							if (target.hasSkill("seika_huanzhu") && target.countCards("h") + 1 === target.hp && target.hp === renkuCount) {
								return 25;
							}

							var score = att * 2;
							if (chosenCard.name === "tao" && target.hp <= 2) score += 6;
							if (get.subtype(chosenCard) === "equip2" && !target.getEquip(2)) score += 4;
						...
						if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
							event.result = {
								bool: true,
								cost_data: { card: chosenCard, target: targetRes.targets[0] }
							};
						}
					}
				},
				content: async function(event, trigger, player) {
					player.addTempSkill("mito_liangyi_self_used", "phaseAfter");
					var card = event.cost_data.card;
					var target = event.cost_data.target;

					player.logSkill("mito_liangyi", target);
					_status.renku.remove(card);
					_status.renku_decreased_this_turn = true;

					await target.gain([card], "gain2");
					game.log(player, "将仁区内的", card, "交给了", target);
			...
			other: {
				...
				filter: function(event, player) {
					if (player.hasSkill("mito_liangyi_other_used")) return false;
					var target = event.player;
				...
				cost: async function(event, trigger, player) {
					var target = trigger.player;
					var next = player.chooseBool("良谊②：是否令 " + get.translation(target) + " 展示并获得牌堆顶一张牌？");
					next.set("ai", function() {
				...
				content: async function(event, trigger, player) {
					player.addTempSkill("mito_liangyi_other_used", "phaseAfter");
					var target = trigger.player;
					player.logSkill("mito_liangyi", target);

					var cards = get.cards(1);
					await game.cardsGotoOrdering(cards);
					await target.showCards(cards, `${get.translation(target)}展示了牌堆顶的牌`);
					await target.gain(cards, "gain2");
				}
			}
		}
	},
	"mito_liangyi_self_used": { charlotte: true },
	"mito_liangyi_other_used": { charlotte: true },

	"mito_jiexin": {
		audio: "ext:魔法纪录/audio/skill:2",
		trigger: { global: "phaseAfter" },
		round: 2,
		getDiscardCount: function(target) {
			var num = 0;
			target.getHistory("lose", function(evt) {
				if (evt.type === "discard" && evt.cards2) {
					num += evt.cards2.filter(i => get.position(i, true) === "d").length;
				}
			});
			return num;
		},
		filter: function(event, player) {
			var roundStat = player.getStat("round");
			if (roundStat && roundStat.mito_jiexin >= 2) return false;

			var x = _status.renku ? _status.renku.length : 0;
			if (x <= 0) return false;
			var threshold = Math.ceil(x / 2);

			return game.hasPlayer(function(current) {
				var count = lib.skill.mito_jiexin.getDiscardCount(current);
		...
		cost: async function(event, trigger, player) {
			...
			var validTargets = game.filterPlayer(function(current) {
				var count = lib.skill.mito_jiexin.getDiscardCount(current);
				return count > 0 && count <= threshold;
			});
			if (!validTargets.length) return;

			var tgtRes = await player.chooseTarget(
				"结心：选择一名本回合弃牌且不大于" + threshold + "张的角色",
				true,
				function(card, p, target) {
					return _status.event.validTargets.includes(target);
				}
			).set("validTargets", validTargets).set("ai", function(target) {
				var p = _status.event.player;
				var att = get.attitude(p, target);
				if (att > 0) {
					var score = 10 + att + (4 - target.countCards("h"));
					if (target.isDamaged()) score += 8;
					return score;
				}
				return (p.isDamaged() || p.countCards("h") <= 2) ? 6 : -1;
			}).forResult();

			if (!tgtRes.bool || !tgtRes.targets || !tgtRes.targets.length) return;
			var target = tgtRes.targets[0];
			var discardCount = lib.skill.mito_jiexin.getDiscardCount(target);

			var chooserList = [
				`由${get.translation(player)}展示手牌`,
				`由${get.translation(target)}展示手牌`
			];
			var ctrlRes = await player.chooseControl(["选项一", "选项二"])
				.set("choiceList", chooserList)
				.set("prompt", "结心：选择由哪名角色展示手牌并摸牌、回复体力？")
				.set("ai", function() {
					var p = _status.event.player;
					var trg = _status.event.target;
					var att = get.attitude(p, trg);
					...
					if (trg.isDamaged() && trg.hp <= 2) return "选项二";

					if (p.isDamaged() && p.hp <= 2) return "选项一";

					return p.countCards("h") <= trg.countCards("h") ? "选项一" : "选项二";
				}).set("target", target).forResult();
			...
			event.result = {
				...
				cost_data: {
					target: target,
					shower: ctrlRes.control === "选项一" ? player : target,
					count: Math.min(discardCount, 3)
				}
			};
		},
		content: async function(event, trigger, player) {
			var stat = player.getStat();
			if (!stat.round) stat.round = {};
			stat.round.mito_jiexin = (stat.round.mito_jiexin || 0) + 1;

			var shower = event.cost_data.shower;
			var drawNum = event.cost_data.count;

			player.logSkill("mito_jiexin", shower);

			if (shower.countCards("h") > 0) {
				await shower.showHandcards(`${get.translation(shower)}展示了所有手牌`);
			}

			if (drawNum > 0) {
				await shower.draw(drawNum);
			}

			if (shower.isIn() && shower.isDamaged()) {
				await shower.recover(1);
			}

			if (_status.renku && _status.renku.length > 0 && drawNum > 0) {
				var toDiscardRenku = _status.renku.splice(0, drawNum);
				_status.renku_decreased_this_turn = true;
				for (var card of toDiscardRenku) {
					game.cardsDiscard(card);
				}
				game.log(player, "将仁区内的", toDiscardRenku, "置入了弃牌堆");
				await game.delayx();
			}
		}
	},
	...
	"mito_suixing": {
		...
		prompt: "穗幸：弃置一张手牌，令一名其他角色声明一种花色并展示牌堆顶的牌",
		ai: {
			order: 8.5,
			result: {
				target: function(player, target) {
					var att = get.attitude(player, target);
					if (att <= 0) return -10;
					if (target.hasSkill("rera_yanlun")) return 20;
					return att * 2 + (4 - target.countCards("h"));
				}
			}
		},
		content: async function(event, trigger, player) {
			...
			while (target.isIn()) {
				var declareRes = await target.chooseControl(["spade", "heart", "club", "diamond"])
					.set("prompt", "穗幸：请声明一种花色")
					.set("choiceList", ["黑桃 ♠", "红桃 ♥", "梅花 ♣", "方片 ♦"])
					.set("ai", function() {
						return ["heart", "spade", "diamond", "club"].randomGet();
					}).forResult();

				var declaredSuit = declareRes.control;
				target.chat("我声明" + get.translation(declaredSuit));
				game.log(target, "声明了花色", "#y" + get.translation(declaredSuit));

				var topCards = get.cards(1);
				await game.cardsGotoOrdering(topCards);
				var topCard = topCards[0];
				await player.showCards([topCard], `${get.translation(player)}展示了牌堆顶的牌`);
				var cardSuit = get.suit(topCard);

				if (cardSuit === declaredSuit) {
					correctCount++;
					game.log("花色匹配成功！当前猜对次数：", correctCount);
					
					if (!_status.renku) _status.renku = [];
					_status.renku.push(topCard);
					game.log(player, "将", topCard, "置入了", "#y【仁区】");
					...
					if (correctCount < 3) {
						var goOn = await player.chooseBool("穗幸：是否继续重复此流程？（已猜对" + correctCount + "/3次）").set("ai", function() {
							var curR = _status.renku ? _status.renku.length : 0;
							if (curR >= 6 && !game.hasPlayer(p => p.hasSkill("seika_manyi"))) return 0;
							return 1;
						}).forResult();
						if (!goOn.bool) break;
					} else {
					...
					var drawExtra = correctCount;
					game.log("花色不同，猜对次数为", correctCount);

					if (drawExtra > 0 && target.isIn()) {
						await target.draw(drawExtra);
						game.log(target, "摸了", drawExtra, "张牌");
					}
					if (player.isIn()) {
						await player.gain([topCard], "gain2");
						game.log(player, "获得了展示的牌", topCard);
					}
					break;
				}
			}
	...
	"Amane_twins_counter": {
		...
		content: async function(event, trigger, player) {
			...
			if (game.Amane_shared_count >= 4) {
				...
				game.log(player, "作为天音姐妹累计使用或打出了", "#y第4张牌");
				...
				game.countPlayer(function(current){
					if (current.hasSkill("Amane_twins_counter")) {
						...
						if (current.name === 'felicia' || current.name2 === 'felicia') {
				...
				for (var i = 0; i < felicias.length; i++) {
					game.log(felicias[i], "触发彩蛋，蹭到了天音姐妹的连携效果！");
		...
		subSkill: {
			caidan: {
				trigger: { global: ["gameStart", "roundStart", "phaseBefore"] },
				forced: true,
				silent: true,
				globalFixed: true,
				filter: function(event, player) {
					var hasyuexiao = game.hasPlayer(function(current) { return current.name === 'yuexiao' || current.name2 === 'yuexiao'; });
					var hasyueye = game.hasPlayer(function(current) { return current.name === 'yueye' || current.name2 === 'yueye'; });
					var hasFelicia = game.hasPlayer(function(current) { return current.name === 'felicia' || current.name2 === 'felicia'; });
					if (!hasyuexiao || !hasyueye || !hasFelicia) return false;

					return game.hasPlayer(function(current) {
						return (current.name === 'felicia' || current.name2 === 'felicia') && !current.hasSkill("Amane_twins_counter");
					});
				},
				content: function(event, trigger, player) {
					game.countPlayer(function(current) {
						if ((current.name === 'felicia' || current.name2 === 'felicia') && !current.hasSkill("Amane_twins_counter")) {

							current.setNickname("天音月练");

							current.addSkill("Amane_twins_counter");
							game.log(current, "化身为三妹", "#y【天音月练】", "，加入了天音姐妹的连携！");
							current.popup("天音月练", "gold");
						}
					});
				}
			}
    ...
    "Tsukuyo_muse": {
        ...
        subSkill: {
            draw: {
                ...
                content: function(event, trigger, player) {
                    trigger.bottom = true;
                    game.log(player, "因", "#g【暮色】", "改为从牌堆底摸牌");
                }
            },
            exchange: {
                ...
                content: async function(event, trigger, player) {
                    if (!trigger.Amane_twins_ready) return;
                    
                    player.popup("暮色");
                    game.log(player, "触发了", "#g【暮色】");
                    
                    var drawCards = get.cards(1);
                    await player.gain(drawCards, "gain2");
                    
                    var gainedCard;
                    if (player.countCards("h") === 0) {
                        var extraCards = get.cards(1);
                        await player.gain(extraCards, "gain2");
                        gainedCard = extraCards[0];
                    } else {
                        var topCard = ui.cardPile.firstChild;
                        var chooseRes = await player.chooseCard("h", 1, "请选择一张手牌置于牌堆顶并获得原牌堆顶牌")
                            .set("ai", card => 8 - get.value(card)).forResult();
                        
                        if (chooseRes.bool && chooseRes.cards && chooseRes.cards.length > 0) {
                            var outCard = chooseRes.cards[0];
                            await player.lose(outCard, ui.cardPile);
                            ui.cardPile.insertBefore(outCard, ui.cardPile.firstChild);
                    ...
                    if (gainedCard && get.color(gainedCard) === "red") {
                        var targetRes = await player.chooseTarget("暮色：请选择一名角色，与其各摸一张牌", function(card, p, target) {
                            return true;
                        }).set("ai", function(target) {
                            var att = get.attitude(_status.event.player, target);
                            return att > 0 ? (10 - target.countCards("h")) : 0;
                        }).forResult();
                        
                        if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
                            player.line(targetRes.targets[0], "green");
                            var toDraw = [player, targetRes.targets[0]].unique();
	...
	"Tsukuyo_yingyin": {
		...
		content: async function(event, trigger, player) {
			await player.draw(2);
			var res = await player.chooseCardTarget({
				prompt: "樱隐：请选择至多两张牌并作为【蓄谋】置于其他角色的判定区",
				selectCard: [1, 2],
				position: "he",
				filterCard: function(card, player) { return true; }, 
				filterTarget: function(card, p, target) { return target !== p; },
			...
			if (res.bool && res.targets && res.targets.length > 0 && res.cards && res.cards.length > 0) {
				...
				await player.lose(cards, ui.special, 'toJudge');
				for (var c of cards) {
					c.Amane_source_skill = "Tsukuyo_yingyin";
					c.Amane_source_player = player;
					await target.addJudge({ name: "xumou_jsrg" }, [c]);
				}
				game.log(player, "将", cards, "作为蓄谋牌置入了", target, "的判定区");
			}
		},
		ai: {
			order: 8.5,
		...
		group: ["Tsukuyo_yingyin_trigger", "Tsukuyo_yingyin_damage"],
		subSkill: {
			trigger: {
				trigger: { global: "phaseZhunbeiBegin" },
				forced: true, charlotte: true,
				filter: function(event, player) {
					return event.player.hasCard(c => {
						return (c.viewAs === "xumou_jsrg" || c.name === "xumou_jsrg") && 
							   c.cards && c.cards.length > 0 && 
							   c.cards[0].Amane_source_skill === "Tsukuyo_yingyin" && 
							   c.cards[0].Amane_source_player === player;
					}, "j");
				},
				content: async function(event, trigger, player) {
					var target = trigger.player;
					var marks = target.getCards("j", c => {
						return (c.viewAs === "xumou_jsrg" || c.name === "xumou_jsrg") && 
							   c.cards && c.cards.length > 0 && 
							   c.cards[0].Amane_source_skill === "Tsukuyo_yingyin" && 
							   c.cards[0].Amane_source_player === player;
					...
					if (marks.length > 0) {
						await target.gain(marks, "gain2"); 
						
						for (var m of marks) {
							if (m.cards && m.cards[0]) {
								delete m.cards[0].Amane_source_skill;
								delete m.cards[0].Amane_source_player;
							}
						}
						
						var optRes = await player.chooseControl("bahu", "oushu")
							.set("prompt", "樱隐：请为 " + get.translation(target) + " 选择一项效果")
							.set("choiceList", [
								"获得【跋扈】（准备阶段摸1，出杀上限+1）",
								"每打出偶数张牌时摸一张牌"
							]).set("ai", function() {
								var t = _status.event.target;
								if (t.countCards("h") <= 2 || t.hasSkillTag("useSha")) return "bahu";
								return "oushu";
							}).set("target", target).forResult();
							
						if (optRes.control === "bahu") {
							target.addTempSkill("Amane_bahu", "phaseAfter");
							game.log(target, "获得了", "#y【跋扈】");
						} else {
							target.addTempSkill("Amane_oushu_draw", "phaseAfter");
							game.log(target, "获得了偶数摸牌效果");
						}
					}
				}
			},
            ...
            exchange: {
                ...
                content: async function(event, trigger, player) {
                    ...
                    if (player.countCards("h") === 0) {
                        ...
                        var chooseRes = await player.chooseCard("h", 1, "请选择一张手牌置于牌堆底并获得原牌堆底牌")
                            .set("ai", card => 8 - get.value(card)).forResult();
                        
                        if (chooseRes.bool && chooseRes.cards && chooseRes.cards.length > 0) {
                            var outCard = chooseRes.cards[0];
                            await player.lose(outCard, ui.cardPile);
                            ui.cardPile.appendChild(outCard);
                            
                            await player.gain([bottomCard], "gain2");
                            gainedCard = bottomCard;
                        }
                    }
                    
                    if (gainedCard && get.color(gainedCard) === "black") {
                        var targetRes = await player.chooseTarget("夜光：请选择一名角色，获得其一张牌", function(card, p, target) {
                            return target !== p && target.countCards("he") > 0;
                        }).set("ai", function(target) {
                            var att = get.attitude(_status.event.player, target);
                            return att < 0 ? (target.countCards("e") > 0 ? 2 : 1) : 0;
                        }).forResult();
                        
                        if (targetRes.bool && targetRes.targets && targetRes.targets.length > 0) {
                            var t = targetRes.targets[0];
                            player.line(t, "green");
	...
	"Tsukasa_yingyu": {
		...
		content: async function(event, trigger, player) {
			var getRes = await player.chooseTarget("樱语：请选择一名角色，获得其至多两张牌", function(card, p, target) {
				return target !== p && target.countCards("he") > 0;
			}).set("ai", function(target) {
				var att = get.attitude(_status.event.player, target);
				return att < 0 ? target.countCards("he") : 0;
			}).forResult();

			if (getRes.bool && getRes.targets && getRes.targets.length > 0) {
				var target1 = getRes.targets[0];
				
				var gainRes = await player.choosePlayerCard(target1, "he", [1, 2], true)
					.set("prompt", "樱语：请选择获得 " + get.translation(target1) + " 的至多两张牌")
					.forResult(); 
				
				if (gainRes.bool && gainRes.links && gainRes.links.length > 0) {
					var gainedCards = gainRes.links;
					await player.gain(gainedCards, target1, "giveAuto"); 

					var putRes = await player.chooseCardTarget({
						prompt: "樱语：请选择 " + gainedCards.length + " 张牌并作为【蓄谋】置于另一名角色的判定区",
						selectCard: gainedCards.length,
						position: "he",
						filterCard: function(card, player) { return true; },
						filterTarget: function(card, p, target) { return target !== p; },
						ai1: function(card) {
							var val = get.value(card);
							var name = card.name;
							if (name === 'tao' || name === 'jiu' || name === 'wuzhongshengyou') return -10; 
							if (get.tag(card, "damage")) return 10; 
							return 6 - val; 
						},
						ai2: function(target) { return get.attitude(_status.event.player, target) < 0 ? 1 : 0; } 
					}).forResult();

					if (putRes.bool && putRes.targets && putRes.targets.length > 0 && putRes.cards && putRes.cards.length > 0) {
						var target2 = putRes.targets[0];
						player.line(target2, "green");
						await player.lose(putRes.cards, ui.special, 'toJudge');
						for (var c of putRes.cards) {
							c.Amane_source_skill = "Tsukasa_yingyu";
							c.Amane_source_player = player;
							await target2.addJudge({ name: "xumou_jsrg" }, [c]);
						}
						game.log(player, "将", putRes.cards, "作为蓄谋牌置入了", target2, "的判定区");
					}
				}
			}
		},
		...
		group: ["Tsukasa_yingyu_trigger", "Tsukasa_yingyu_damage"],
		subSkill: {
			trigger: {
				trigger: { global: "phaseZhunbeiBegin" },
				forced: true, charlotte: true,
				filter: function(event, player) {
					return event.player.hasCard(c => {
						return (c.viewAs === "xumou_jsrg" || c.name === "xumou_jsrg") && 
							   c.cards && c.cards.length > 0 && 
							   c.cards[0].Amane_source_skill === "Tsukasa_yingyu" && 
							   c.cards[0].Amane_source_player === player;
					}, "j");
				},
				content: async function(event, trigger, player) {
					...
					if (marks.length > 0) {
						await target.lose(marks, ui.special, "toJudge");
						
						for (var c of marks) {
							if (c.cards && c.cards.length > 0) {
								var realCard = c.cards[0];
								delete realCard.Amane_source_skill; 
								delete realCard.Amane_source_player;
								await player.useCard(realCard, target, false);
							} else {
								await player.useCard(c, target, false);
							}
						}
						
						var optRes = await player.chooseControl("Amane_benghuai", "oushu")
							.set("prompt", "樱语：请为 " + get.translation(target) + " 选择一项效果")
							.set("choiceList", [
								"获得【崩坏】（结束阶段流失体力或上限）",
								"每打出偶数张牌时弃置一张牌"
							]).set("ai", function() {
								var t = _status.event.target;
								if (t.hp <= 2) return "Amane_benghuai";
								if (t.countCards("h") >= 3) return "oushu";
								return "Amane_benghuai";
							}).set("target", target).forResult();
							
						if (optRes.control === "Amane_benghuai") {
							target.addTempSkill("Amane_benghuai", "phaseAfter");
							game.log(target, "获得了", "#y【崩坏】");
						} else {
							target.addTempSkill("Amane_oushu_discard", "phaseAfter");
							game.log(target, "获得了偶数弃牌效果");
						}
					}
				}
			},
	...
	"Amane_bahu": {
		...
		mark: true,
		marktext: "跋扈",
		intro: { content: "准备阶段摸1，出杀上限+1" },
		group: ["Amane_bahu_draw"],
		mod: {
			cardUsable: function(card, player, num) {
	...
	"Amane_benghuai": {
		...
		mark: true,
		marktext: "崩坏",
		intro: { content: "结束阶段，流失一点体力或体力上限" },
		trigger: { player: "phaseJieshuBegin" },
		forced: true,
		content: async function(event, trigger, player) {
			var res = await player.chooseControl("loseHp", "loseMaxHp")
				.set("prompt", "崩坏：请选择流失1点体力或减1点体力上限")
				.set("choiceList", ["流失1点体力", "减1点体力上限"])
				.set("ai", function() {
					var p = _status.event.player;
					if (p.hp > p.maxHp) return "loseMaxHp";
					if (p.hp > 2) return "loseHp";
					return "loseMaxHp";
				}).forResult();
				
			if (res.control === "loseMaxHp") {
				await player.loseMaxHp(1);
			} else {
				await player.loseHp(1);
			}
	...
	"Amane_oushu_draw": {
		...
		mark: true,
		marktext: "摸",
		intro: { content: "本回合每打出偶数张牌时摸一张牌" },
		trigger: { player: ["useCardAfter", "respondAfter"] },
		forced: true,
		filter: function(event, player) {
			var historyNum = player.getHistory("useCard").length + player.getHistory("respond").length;
			return historyNum > 0 && historyNum % 2 === 0;
		},
		content: async function(event, trigger, player) {
			game.log(player, "因打出了偶数张牌，触发了增益");
			await player.draw(1);
		}
	},
	"Amane_oushu_discard": {
		...
		mark: true,
		marktext: "弃",
		intro: { content: "本回合每打出偶数张牌时弃置一张牌" },
		trigger: { player: ["useCardAfter", "respondAfter"] },
		forced: true,
		filter: function(event, player) {
			var historyNum = player.getHistory("useCard").length + player.getHistory("respond").length;
			return historyNum > 0 && historyNum % 2 === 0 && player.countCards("he") > 0;
		},
		content: async function(event, trigger, player) {
			game.log(player, "因打出了偶数张牌，触发了反噬");
	...
	"dArc_shengjian_lv2": {
		...
		ai: {
			order: function(item, player) {
				...
				if (player.getCards('e').length === 0 && player.hasCard(function(c){
					return get.type(c) === 'equip' && get.value(c, player) > 0;
				}, 'h')) {
					return 0.1; 
				}
				return 9; 
			...
			result: {
				player: function (player) {
					...
					if (player.getCards('e').length > 0 && player.hp >= 2) return 20; 
					
					if (player.hp >= 3) return 15;
					if (player.hp === 1 && player.getCards('e').length > 0 && player.countCards('h', {name:'tao'}) > 0) return 1;
					return 0;
				}
			}
		...
		content: function () {
			...
			player.chooseControl("ClovisSword", "LightLance")
				...
				.set('ai', function() { 
					var p = _status.event.player;
					if (p.hasCard(function(c) { return c.name === "ClovisSword"; }, "e")) {
						return "LightLance";
					}
					return "ClovisSword"; 
				});
...
"Final_dArc_poge": {
	...
	"Melissa_bengmie_lv1": {
		audio: "ext:魔法纪录/audio/skill:2",
		trigger: { player: "shaMiss", target: "shaMiss", global: ["useCard", "respond"] },
		group: ["Melissa_bengmie_clear"],
		filter: function (event, player, name) {
			...
			if (name === "shaMiss") {
				...
				if (event.card && event.card.name === "wuxie") {
					...
					if (p && p.card && (p.player === player || event.player === player)) {
						return !history.includes(p.card.name) && !p.bengmie_wuxie_caught;
					}
				}
				return false;
			}
		...
		cost: async function(event, trigger, player) {
			...
			var target = (trigger.card && trigger.card.name === "wuxie") ? 
				((p && p.player === player) ? trigger.player : (p ? p.player : trigger.target)) : 
				((trigger.player === player) ? trigger.target : trigger.player);
			
			var res = await player.chooseBool("是否对 " + get.translation(target) + " 发动【崩灭】？").set("ai", function() {
				return get.attitude(_status.event.player, _status.event.target) <= 0;
			}).set("target", target).forResult();
		...
		content: async function (event, trigger, player) {
			...
			if (trigger.card && trigger.card.name === "wuxie") {
				if (p && p.card) {
					...
					targetCardName = p.card.name;
					p.bengmie_wuxie_caught = true; 
				}
			} else {
				history.push(trigger.card.name);
				targetCardName = trigger.card.name;
			}
			...
			var op1_prompt = "受到1点伤害，然后获得【" + get.translation(targetCardName) + "】";
			var op2_prompt = "弃置两张牌，本轮内其下次受到的伤害+1";
			
			var next = target.chooseControl("选项一", "选项二")
				.set("prompt", "请选择【崩灭】的代价")
				.set("choiceList", [op1_prompt, op2_prompt])
				.set("targetCardName", targetCardName) 
				.set("ai", function () {
					var p = _status.event.player; 
					var cardName = _status.event.targetCardName;
					var cardValue = cardName ? get.value({ name: cardName }, p) : 0;
					
					if (p.hp >= 2 && cardValue >= 6.5) return "选项一";
					if (p.hp <= 1) return "选项二"; 
					return p.countCards("he") >= 3 ? "选项二" : "选项一";
				});
			var result = await next.forResult();
			
			if (result.control === "选项一") {
				await target.damage(1, player);
				var physicalCards = (p && p.cards) ? p.cards.filter(c => ["o", "d"].includes(get.position(c, true))) : [];
				if (physicalCards.length > 0) {
					await target.gain(physicalCards, "gain2");
					if (p && p.cards) p.cards.removeArray(physicalCards);
				}
			} else {
				await target.chooseToDiscard("he", 2, true);
				target.addSkill("Melissa_bengmie_nu_lv1");
				target.addMark("Melissa_bengmie_nu_lv1", 1, false);
			}
		}
	},
	...
	"Melissa_bengmie_lv2": {
		...
		cost: async function(event, trigger, player) {
			...
			var res = await player.chooseBool("是否对 " + get.translation(target) + " 发动【崩灭】(Lv2)？").set("ai", function() {
				return get.attitude(_status.event.player, _status.event.target) <= 0;
			}).set("target", target).forResult();
		...
		content: async function (event, trigger, player) {
			...
			var op1_prompt = "受到1点伤害并获得【" + get.translation(targetCardName) + "】" + (op1_count > 0 ? "（将随机废除你一个装备栏）" : "");
			var op2_prompt = "弃置3张牌，本轮内其受到的伤害+1（可叠加）";
			
			var next = target.chooseControl("选项一", "选项二")
				.set("prompt", "请选择【崩灭】(Lv2)的代价")
				.set("choiceList", [op1_prompt, op2_prompt])
				.set("targetCardName", targetCardName)
				.set("ai", function () {
					var p = _status.event.player; 
			...
			if (result.control === "选项一") {
				target.storage.Melissa_bengmie_op1_count = op1_count + 1;
				await target.damage(1, player);
				
				if (op1_count > 0 && target.hasEnabledSlot) {
					var list = [];
					for (var i = 1; i <= 5; i++) {
						if (!target.isDisabled("equip" + i)) list.push("equip" + i);
					}
					if (list.length > 0) target.disableEquip(list.randomGet());
				}
				...
				await target.chooseToDiscard("he", 3, true);
				target.addSkill("Melissa_bengmie_nu_lv2");
				target.addMark("Melissa_bengmie_nu_lv2", 1, false);
			}
		}
	},

	"Melissa_bengmie_nu_lv1": { 
		charlotte: true, mark: true, marktext: "怒", 
		intro: { content: "本轮内下一次受到的伤害+#" },
		trigger: { player: "damageBegin4" }, 
		forced: true, popup: false,
		filter: function (event, player) { 
			return player.countMark("Melissa_bengmie_nu_lv1") > 0; 
		},
		content: function (event, trigger, player) {
			var extraDmg = player.countMark("Melissa_bengmie_nu_lv1");
			trigger.num += extraDmg;
			game.log(player, "受到", "#g【崩灭】", "锁定，受到的伤害", "#y+" + extraDmg, "！");
			player.removeMark("Melissa_bengmie_nu_lv1", extraDmg); 
			player.removeSkill("Melissa_bengmie_nu_lv1");
		}
	},

	"Melissa_bengmie_nu_lv2": { 
		charlotte: true, mark: true, marktext: "怒", 
		intro: { content: "本轮内每次受到的伤害+#" },
		trigger: { player: "damageBegin4" },
		forced: true, popup: false,
		filter: function (event, player) { 
			return player.countMark("Melissa_bengmie_nu_lv2") > 0; 
		},
		content: function (event, trigger, player) {
			var extraDmg = player.countMark("Melissa_bengmie_nu_lv2");
			trigger.num += extraDmg;
			game.log(player, "受到", "#g【崩灭】", "深渊锁定，受到的伤害", "#y+" + extraDmg, "！");
		}
	},

	"Melissa_bengmie_clear": {
		trigger: { global: "roundStart", player: "phaseAfter" }, 
		silent: true, charlotte: true,
		filter: function (event, player) { return true; },
		content: function (event, trigger, player) {
			if (trigger.name === "phase") delete player.storage.Melissa_bengmie_history;
			if (trigger.name === "roundStart") {
				game.players.forEach(function(p) {
					delete p.storage.Melissa_bengmie_op1_count;
					if (p.hasSkill("Melissa_bengmie_nu_lv1")) {
						p.removeMark("Melissa_bengmie_nu_lv1", p.countMark("Melissa_bengmie_nu_lv1"));
						p.removeSkill("Melissa_bengmie_nu_lv1");
					}
					if (p.hasSkill("Melissa_bengmie_nu_lv2")) {
						p.removeMark("Melissa_bengmie_nu_lv2", p.countMark("Melissa_bengmie_nu_lv2"));
						p.removeSkill("Melissa_bengmie_nu_lv2");
					}
				});
			}
		}
	...
	"Melissa_tongxin_core": {
		trigger: { player: "phaseBegin" },
		forced: true, charlotte: true, ruleSkill: true,
		filter: function (event, player) {
		...
		content: async function (event, trigger, player) {
			...
			for (var i = 0; i < oldPool.length; i++) {
				if (oldPool[i] !== player) oldPool[i].removeSkill("Melissa_tongxin_mark");
			}
			player.storage.Melissa_tx_pool = [player]; 

			var targetsRes = await player.chooseTarget("同心：你可以选择一名其他角色与之同心", [0, 1], function (card, p, target) {
				return target !== p;
			}).set("ai", function (target) {
				var att = get.attitude(_status.event.player, target);
				if (att <= 1) return 0;
				var score = att;
				score += target.countCards("h") * 0.5; 
				if ((target.hujia || 0) === 0) score += 2; 
				return score;
			}).forResultTargets();

			if (targetsRes && targetsRes.length > 0) {
				var target = targetsRes[0];
				player.line(target, "green");
				player.storage.Melissa_tx_pool.push(target); 
				target.addSkill("Melissa_tongxin_mark");
				game.log(player, "选择了", target, "作为本回合的同心角色");
			}
		}
	},
	...
	"Melissa_bingjian_lv1": {
		...
		group: ["Melissa_bingjian_clear", "Melissa_tongxin_core"],
		filter: function (event, player) {
			var cards = [];
			if (event.getg) cards = event.getg(player) || [];
			else cards = event.cards || [];
			
			if (cards.length === 0) return false;
			if ((player.storage.Melissa_bingjian_round_count || 0) >= 2) return false;
			
			var p1 = event.getParent();
			var p2 = event.getParent(2);
			if (p1 && p1.name === "Melissa_bingjian_lv1") return false;
			if (p2 && p2.name === "Melissa_bingjian_lv1") return false;
			
			if (player.hasSkill("Melissa_bingjian_running")) return false;
			return true;
		},
		cost: async function(event, trigger, player) {
			var res = await player.chooseBool("是否发动【并肩】与同心角色进行重铸？").set("ai", () => true).forResult();
			if (res.bool) event.result = { bool: true };
		},
		content: async function (event, trigger, player) {
			player.addTempSkill("Melissa_bingjian_running");
			player.storage.Melissa_bingjian_round_count = (player.storage.Melissa_bingjian_round_count || 0) + 1;
			
			var participants = player.getStorage("Melissa_tx_pool") || [player];
			var suitsCollected = []; 

			for (var i = 0; i < participants.length; i++) {
				var p = participants[i];
				if (p.isAlive()) {
					var result = await p.chooseToDiscard("he", [0, 2], "【并肩】你可以重铸至多两张牌")
						.set("suitsCollected", suitsCollected.slice()) 
						.set("ai", function(card) { 
							var val = 6 - get.value(card);
							if (val > 0) { 
								var collected = _status.event.suitsCollected || [];
								var s = get.suit(card);
								if (s !== "none") {
									if (!collected.includes(s)) val += 10; 
									else val -= 2; 
								}
							}
							return val;
					...
					if (result.bool && result.cards && result.cards.length > 0) {
						for (var k = 0; k < result.cards.length; k++) {
							var s = get.suit(result.cards[k]);
							if (s !== "none" && !suitsCollected.includes(s)) suitsCollected.push(s);
						}
			...
			if (suitsCollected.length >= 3) {
				game.log("团队重铸凑齐了", "#y" + suitsCollected.length, "种花色！");
				for (var j = 0; j < participants.length; j++) {
					if (participants[j].isAlive() && (participants[j].hujia || 0) < 5) await participants[j].changeHujia(1);
				}
			}
			
			if (suitsCollected.length >= 4) {
				game.log("触发【同心】效果！");
				for (var j = 0; j < participants.length; j++) {
					if (participants[j].isAlive()) {
						player.line(participants[j], "green");
						await participants[j].draw(1);
					}
				}
			}
			player.removeSkill("Melissa_bingjian_running");
		}
	},
	...
	"Melissa_bingjian_lv2": {
		...
		filter: function (event, player) {
			...
			if (cards.length === 0) return false;
			if ((player.storage.Melissa_bingjian_round_count || 0) >= 3) return false;
			
			var p1 = event.getParent();
			var p2 = event.getParent(2);
			if (p1 && p1.name === "Melissa_bingjian_lv2") return false;
			if (p2 && p2.name === "Melissa_bingjian_lv2") return false;
			
			if (player.hasSkill("Melissa_bingjian_running")) return false;
			return true;
		},
		cost: async function(event, trigger, player) {
			var res = await player.chooseBool("是否发动【并肩】(Lv2)与同心角色进行重铸？").set("ai", () => true).forResult();
			if (res.bool) event.result = { bool: true };
		},
		content: async function (event, trigger, player) {
			player.addTempSkill("Melissa_bingjian_running");
	...
	"Melissa_bingjian_clear": {
		trigger: { global: "roundStart" }, charlotte: true, forced: true, silent: true,
		content: function (event, trigger, player) { player.storage.Melissa_bingjian_round_count = 0; }
	},
"Melissa_wanyuan": {
		audio: "ext:魔法纪录/audio/skill:2",
		limited: true,
		...
		check: function (event, player) { return get.attitude(player, event.player) > 1; },
		content: async function (event, trigger, player) {
			"step 0"
			...
			var result = await player.chooseToDiscard("h", [1, 4], "【挽愿】弃置x张不同花色的手牌，令目标回1血并获x-1点护甲")
				...
				.set("filterCard", function(card, p) {
					if (ui.selected.cards.length === 0) return true;
					var suits = ui.selected.cards.map(c => get.suit(c));
					return !suits.includes(get.suit(card));
				})
				.set("ai", card => 8 - get.value(card))
				.forResult();
			
			if (result.bool) event.discardNum = result.cards.length;
			else event.finish();
			
			"step 1"
			await trigger.player.recover(1);
			if (event.discardNum > 1 && (trigger.player.hujia || 0) < 5) {
				await trigger.player.changeHujia(event.discardNum - 1);
			}
			...
			var upgradeResult = await player.chooseControl("升级崩灭", "升级并肩")
				...
				.set("ai", function() {
					...
					var friends = game.filterPlayer(function(current) { 
						return get.attitude(p, current) > 0; 
					});
					
					if (friends.length > 2) return "升级并肩";

					var hasBigBoss = game.hasPlayer(function(current) {
						return get.attitude(p, current) < 0 && current.hp >= 5;
					});
					if (hasBigBoss) return "升级崩灭";
					
					var lowHpCount = 0;
					for (var i = 0; i < friends.length; i++) {
						if (friends[i].hp <= 2 && (friends[i].hujia || 0) === 0) lowHpCount++;
					}
					if (lowHpCount >= 2) return "升级并肩"; 
					
					return p.hp >= 3 ? "升级崩灭" : "升级并肩";
				}).forResult();
	...
	"Elisa_longqi": {
		...
		content: async function (event, trigger, player) {
			var result = await player.chooseToDiscard("he", [1, 4], "龙骑：弃置1~4张花色不同的牌，获得阶梯收益", function (card, p) {
				if (ui.selected.cards.length === 0) return true;
			...
			if (result.bool && result.cards && result.cards.length > 0) {
				...
				if (x === 1) {
					await player.draw(1);
					player.addTempSkill("Elisa_longqi_range", "phaseUseEnd");
					game.log(player, "摸了一张牌，且本回合出牌阶段使用牌", "#y无距离限制");
				}
				else if (x === 2) {
					...
					var dzxj1 = game.createCard("dongzhuxianji");
					await player.useCard(dzxj1, player);
					
					var next3 = await player.chooseBool("【龙骑】以此法弃置了 3 张牌，是否令其失效，直至下个准备阶段开始时发动一次衍生的【龙哮】？").set("ai", function () { return true; }).forResult();
					if (next3.bool) {
					...
					var dzxj1 = game.createCard("dongzhuxianji");
					await player.useCard(dzxj1, player);
					var dzxj2 = game.createCard("dongzhuxianji");
					await player.useCard(dzxj2, player);
					
					var next4 = await player.chooseBool("【龙骑】以此法弃置了 4 张牌，是否令其失效，直至下个准备阶段开始时发动一次衍生的【龙哮】？").set("ai", function () { return true; }).forResult();
					if (next4.bool) {
						player.addSkill("Elisa_longqi_used"); 
					}
				}
	...
	"Elisa_longqi_used": {
		...
		charlotte: true,
		intro: {
			content: "龙骑已失效。你的下个准备阶段开始时解除，并触发衍生龙哮。"
		},
		trigger: { player: "phaseZhunbeiBegin" },
		forced: true, silent: true, 
		priority: 50, 
		content: async function(event, trigger, player) {
			...
			game.log(player, "的", "#g【龙骑】", "已解除失效状态");
			game.log(player, "触发了衍生", "#y【龙哮】", "！");
			
			var chooseTarget = await player.chooseTarget("衍生龙哮：请选择 1 名角色", 1, function(card, p, target) {
				return true;
			}).set("ai", function(target) {
				return -get.attitude(_status.event.player, target);
			}).forResult();
			...
			if (chooseTarget.bool && chooseTarget.targets && chooseTarget.targets.length > 0) {
				...
				if (uniqueSuits.length < 2) {
					...
					if (showResult.bool && showResult.cards && showResult.cards.length === 2) {
						...
						var choice = await target.chooseControl("受到1点火焰伤害", "弃置这些牌").set("ai", function () {
							var t = _status.event.player;
							return (t.hp <= 1 || (t.hp <= 2 && t.countCards("h") > 2)) ? "弃置这些牌" : "受到1点火焰伤害";
						}).forResult();
	...
	"Elisa_longxiao": {
		...
		filter: function (event, player) {
			...
			return Math.ceil((player.storage.Elisa_discard_count || 0) / 2) > 0;
		},
		content: async function (event, trigger, player) {
			var x = player.storage.Elisa_discard_count || 0;
			var targetCount = Math.ceil(x / 2);

			var chooseTarget = await player.chooseTarget("龙哮：本回合弃牌数 " + x + " 张，请选择至多 " + targetCount + " 名角色", [1, targetCount], function (card, p, target) {
				return true; 
			}).set("ai", function (target) {
				return -get.attitude(_status.event.player, target);
			}).forResult();
			...
			if (chooseTarget.bool && chooseTarget.targets && chooseTarget.targets.length > 0) {
				...
				if (hasDamage || totalDiscarded >= 4) {
					var next = await player.chooseBool("【龙哮】满足连招条件，是否令其失效，直至下个结束阶段开始时发动衍生的【龙骑】？").set("ai", function () { return true; }).forResult();
					if (next.bool) {
	...
	"Elisa_longxiao_used": {
		...
		charlotte: true,
		intro: {
			content: "龙哮已失效。直至你的下个结束阶段开始时解除，并触发衍生龙骑。"
		},
		trigger: { player: "phaseJieshuBegin" },
		forced: true, silent: true, 
		priority: 50, 
		content: async function(event, trigger, player) {
			...
			game.log(player, "的", "#g【龙哮】", "已解除失效状态");
			
			game.log(player, "触发了衍生", "#y【龙骑】", "！");
			var dzxj = game.createCard("dongzhuxianji");
			await player.useCard(dzxj, player);
		}
	},

	"Elisa_discard_tracker": {
		trigger: { player: ["loseAfter", "cardsDiscardAfter"] },
		forced: true, silent: true, charlotte: true,
		filter: function(event, player) {
	...
	"Elisa_jinao": {
		...
		filter: function(event, player) {
			...
			if ((player.storage.Elisa_jinao_turncount || 0) >= limit) return false;
			return event.cards && event.cards.length > 0;
		},
		content: async function(event, trigger, player) {
			var isLongqiUsed = player.hasSkill("Elisa_longqi_used");
			var isLongxiaoUsed = player.hasSkill("Elisa_longxiao_used");
			var hasBackwater = player.hasSkill("Elisa_jinao_backwater");
			
			var drawNum = isLongqiUsed ? 2 : 1;
			var targetNum = isLongxiaoUsed ? 2 : 1;
			
			var canBackwater = false;
			if (isLongqiUsed && isLongxiaoUsed && !hasBackwater) {
			...
			var title = "矜傲";
			var logSkillName = "Elisa_jinao_1"; 
			
			if (hasBackwater) { 
				title = "矜傲[最佳状态]"; 
				logSkillName = "Elisa_jinao_4"; 
			}
			else if (isLongqiUsed && isLongxiaoUsed) { 
				title = "矜傲[双重失效]"; 
				logSkillName = "Elisa_jinao_4"; 
			} 
			else if (isLongqiUsed) { 
				title = "矜傲[龙骑失效]"; 
				logSkillName = "Elisa_jinao_2"; 
			}
			else if (isLongxiaoUsed) { 
				title = "矜傲[龙哮失效]"; 
				logSkillName = "Elisa_jinao_3"; 
			}

			var choiceList = ["普通发动"];
			if (canBackwater) choiceList.unshift("背水发动");
			choiceList.push("取消");

			var chooseMethod = await player.chooseControl(choiceList)
				.set("prompt", title + "：是否令至多 " + targetNum + " 名角色各摸 " + drawNum + " 张牌？")
				.set("ai", function () { return _status.event.canBackwater ? "背水发动" : "普通发动"; })
				.set("canBackwater", canBackwater)
				.forResult();

			if (chooseMethod.control === "取消" || !chooseMethod.control) return;

			var chooseTarget = await player.chooseTarget("请选择至多 " + targetNum + " 名角色", [1, targetNum]).set("ai", function(target) {
				return get.attitude(_status.event.player, target);
			}).forResult();

			if (chooseTarget.bool && chooseTarget.targets && chooseTarget.targets.length > 0) {
				
				player.logSkill(logSkillName, chooseTarget.targets);
				
				player.storage.Elisa_jinao_turncount = (player.storage.Elisa_jinao_turncount || 0) + 1;

				for (var i = 0; i < chooseTarget.targets.length; i++) {
					await chooseTarget.targets[i].draw(drawNum);
				}

				if (chooseMethod.control === "背水发动") {
					player.addSkill("Elisa_jinao_backwater"); 
					game.log(player, "发动了", "#r【背水】", "，技能每回合发动次数上限永久修改为 2 ！");
					...
					}).set("complexCard", true).set("prompt", "请弃置红、黑牌各一张，将【龙之雷火】置入装备区").forResult();

					if (discardResult.bool) {
						var leihuo = game.createCard({name: "DragonsFire", suit: "heart", number: 12}); 
		...
		subSkill: {
			cleaner: {
				...
				silent: true, forced: true, charlotte: true,
				content: function(event, trigger, player) {
					player.storage.Elisa_jinao_turncount = 0;
				}
			}
		}
	},
	...
	"Elisa_jinao_backwater": {
		charlotte: true, mark: true, marktext: "极",
		intro: { content: "矜傲已进入背水极状态，每回合限发动2次。" }
	},
	...
	"Corbeau_shizhan": {
		...
		content: function(event, trigger, player) {
			...
			if (num > 0) {
				player.addMark("Corbeau_hui", num, false);
				game.log(player, "获得了", num, "枚", "#y【秽】");
			}
		},
		removeHui: async function(player, num) {
			player.removeMark("Corbeau_hui", num);
			
			var next = player.judge(function(card){ return get.color(card) === 'black' ? 1 : 0; });
			var res = await next.forResult();
			if (res.color === 'black' && res.card) {
				await player.addJudge({ name: "xumou_jsrg" }, [res.card]);
			}
			
			if (num >= 3) {
				var targetRes = await player.chooseTarget("嗜战：本次移去秽≥3，是否令一名角色获得1点护甲？", [0, 1]).set("ai", function(target) {
					var p = _status.event.player;
					var att = get.attitude(p, target);
					...
					var isLow = function(character) {
						return character.hp <= 1 || (character.hp <= 2 && character.countCards("h") <= 1);
					};
					
					var targetLow = isLow(target);
					var selfLow = isLow(p);
					var isZhu = target.isZhu || (target.role === "zhu"); 
					
					if (isZhu && targetLow) return 100 + att;
					
					if (target === p) {
						if (selfLow) return 90; 
						return 50; 
					}
					
					if (targetLow) {
						if (selfLow) return 40 + att; 
						return 80 + att; 
					}
		...
		group: ["Corbeau_shizhan_tracker", "Corbeau_shizhan_tracker2", "Corbeau_shizhan_punish", "Corbeau_shizhan_clear"]
	},

	"Corbeau_shizhan_tracker": {
		trigger: { source: "damageBegin" },
		forced: true, silent: true, charlotte: true,
		filter: function(event, player) { return _status.currentPhase === player; },
		content: function(event, trigger, player) {
			if (trigger.player) trigger.Corbeau_hpBefore = trigger.player.hp;
		}
	},
	"Corbeau_shizhan_tracker2": {
		trigger: { source: "damageEnd" },
		forced: true, silent: true, charlotte: true,
		filter: function(event, player) { return _status.currentPhase === player; },
		content: function(event, trigger, player) {
			if (trigger.player && trigger.Corbeau_hpBefore !== undefined) {
				if (trigger.player.hp < trigger.Corbeau_hpBefore && trigger.nature !== "unreal") {
					player.storage.Corbeau_shizhan_validDamage = true;
				}
			}
		}
	},
	"Corbeau_shizhan_punish": {
		trigger: { player: "phaseJieshuBegin" },
		forced: true,
		filter: function(event, player) {
			return !player.storage.Corbeau_shizhan_validDamage;
		},
		content: function(event, trigger, player) {
			game.log(player, "本回合未造成致体力减少的伤害，受到", "#g【嗜战】", "的反噬");
			var loseEvt = player.loseHp(1);
			loseEvt.set("Corbeau_source", player); 
		}
	},
	"Corbeau_shizhan_clear": {
		trigger: { player: "phaseAfter" },
		forced: true, silent: true, charlotte: true,
		content: function(event, trigger, player) {
			player.storage.Corbeau_shizhan_validDamage = false;
		}
	},
	...
	"Corbeau_eyu": {
		...
		filter: function(event, player) {
			...
			if (event.name === 'lose') {
				...
				if (hCards.length === 0) return false;
				return player.countMark("Corbeau_hui") >= hCards.length + 1;
			} else if (event.name === 'damage') {
				if (event.num <= 0) return false;
				return player.countMark("Corbeau_hui") >= event.num + 1;
			}
			return false;
		},
		cost: async function(event, trigger, player) {
			var isLose = trigger.name === 'lose';
			...
			var res = await player.chooseTarget(promptStr, function(card, p, target) {
				...
				if (get.attitude(p, target) >= 0) return 0; 
				var isDanger = p.hp <= 3;
				var isBoss = p.hasSkill("Boss_Corbeau_siwu");
				var canBossSiwu = isBoss && (!game.Boss_Corbeau_siwu_count || game.Boss_Corbeau_siwu_count < 2);

				if (canBossSiwu && !isDanger && hui >= 8 && hui < 13) return 0; 
				if (!isLose && isDanger) return 100;
				if (isLose) {
					if (num === 1 && hui <= 4) return 0; 
				} else {
					var hpArmor = p.hp + (p.hujia || 0);
					if (num === 1 && hpArmor >= 5 && hui < 5) return 0; 
				}
		...
		content: async function(event, trigger, player) {
			var costMark = event.cost_data.costMark;
			...
			if (isLose) {
				...
				if (trigger.cards.length === 0) trigger.cancel();
				
				var hCardsToDiscard = target.getCards("h").randomGets(num);
				if(hCardsToDiscard.length > 0) await target.discard(hCardsToDiscard);
			} else {
				trigger.cancel(); 
				var loseEvt = target.loseHp(num);
				loseEvt.set('Corbeau_source', player); 
			}
		}
	},
	...
	"Corbeau_siwu": {
		...
		enable: "phaseUse",
		filter: function(event, player) {
			return player.countMark("Corbeau_hui") >= 4 && !player.hasSkill("Corbeau_siwu_round") && player.hasCard(function(c){ return get.color(c) === 'red'; }, "h");
		},
		...
		content: function(event, trigger, player) {
			"step 0"
			player.addTempSkill("Corbeau_siwu_round", "roundStart");
			lib.skill.Corbeau_shizhan.removeHui(player, 4);

			event.targetCard = event.cards[0];
			event.tName = event.targetCard.name;
			event.tSuit = get.suit(event.targetCard);
			
			player.discard(event.targetCard);
			
			game.broadcastAll(() => {
				_status.tempMusic = "effect_caomaoBJM";
				game.playBackgroundMusic();
			});
			player.$fullscreenpop("La Danse Macabre", "fire");

			if (!game.Corbeau_siwu_banned) game.Corbeau_siwu_banned = [];
			var alreadyBanned = game.Corbeau_siwu_banned.some(b => b.name === event.tName && b.suit === event.tSuit);
			if (!alreadyBanned) {
				game.Corbeau_siwu_banned.push({ name: event.tName, suit: event.tSuit });
			}

			event.filterFn = function(card) {
				return get.name(card, false) === event.tName && get.suit(card, false) === event.tSuit;
			};

			var cards = ["cardPile", "discardPile"].map(pos => Array.from(ui[pos].childNodes)).flat();
			var cardx = cards.filter(event.filterFn);
			if (cardx.length) {
				game.cardsGotoSpecial(cardx);
				game.log(cardx, "被移出了游戏");
			}

			event.throwTargets = game.filterPlayer();
			event.throwIndex = 0;
			"step 1"
			if (event.throwIndex < event.throwTargets.length) {
				var p = event.throwTargets[event.throwIndex];
				event.throwIndex++;
				var hCards = p.getCards("hej", event.filterFn);
				if (hCards.length) {
					p.$throw(hCards, 1000);
					game.log(hCards, "被移出了游戏");
					p.lose(hCards, ui.special);
				}
				event.redo();
			}
			"step 2"
			...
			player.chooseTarget("请选择至多两名其他角色各流失1点体力", [1, 2], lib.filter.notMe).set('ai', function(target){
				var p = _status.event.player;
				if (get.attitude(p, target) >= 0) return 0; 
				return 10 - target.hp;
			});
			...
			if (result.bool && result.targets && result.targets.length > 0) {
				...
				event.damageTargets = result.targets;
				event.damageIndex = 0;
			} else {
				event.finish();
			}
			"step 4"
			if (event.damageIndex < event.damageTargets.length) {
				var t = event.damageTargets[event.damageIndex];
				event.damageIndex++;
	...
	"Corbeau_siwu_banish": {
		...
		forced: true, silent: true, firstDo: true,
		filter: function(event, player) {
			if (!game.Corbeau_siwu_banned || game.Corbeau_siwu_banned.length === 0) return false;
			var cards = event.getd();
			return cards.some(card => {
				if (get.position(card, true) !== "d") return false;
				var cName = get.name(card, false);
				var cSuit = get.suit(card, false);
				return game.Corbeau_siwu_banned.some(b => b.name === cName && b.suit === cSuit);
			});
		},
		content: async function(event, trigger, player) {
	...
	"Isabeau_kamen": {
		...
		filter: function(event, player) {
			var cards = player.getCards("he");
			var used = player.getStorage('Isabeau_kamen_used') || [];
			var canUse = false;
			for (var i = 0; i < cards.length; i++) {
				if (get.suit(cards[i]) !== 'spade') continue;
				var type = get.type(cards[i]);
				if (type === 'delay') type = 'trick';
				if (!used.includes(type)) { canUse = true; break; }
			}
			return canUse;
		},
		
		hiddenCard: function(player, name) {
			var hasValidSpade = function(p) {
				var cards = p.getCards("he");
				var used = p.getStorage('Isabeau_kamen_used') || [];
				for (var c of cards) {
					if (get.suit(c) === 'spade' && !used.includes(get.type(c) === 'delay' ? 'trick' : get.type(c))) return true;
				}
				return false;
			};

			if (name === "jiu") return hasValidSpade(player);

			var maskName = "EnglandCrown";
			...
			if (pName.indexOf("Lapine") !== -1) maskName = "RabbitMask";
				...
				else if (x === 2) maskName = "CrowMask";
				else maskName = "CatMask";
			}

			if (name === maskName) {
				var hasMask = player.hasCard(c => ['RabbitMask', 'CrowMask', 'CatMask', 'EnglandCrown'].includes(c.name), 'e');
				if (!hasMask) return hasValidSpade(player);
			}
			return false;
		},
		...
		chooseButton: {
			dialog: function(event, player) {
				var list = ["jiu"];
				var hasMask = player.hasCard(function(c){
					return ['RabbitMask', 'CrowMask', 'CatMask', 'EnglandCrown'].includes(c.name);
				}, 'e');
				...
				if (!hasMask) {
					...
					if (lib.card[maskName]) list.push(maskName);
				}
				
				var vcards = [];
				for (var i = 0; i < list.length; i++) {
					var type = get.type({name: list[i]});
					vcards.push([type, "", list[i]]);
				}
				return ui.create.dialog("假面：请选择要转化的牌", [vcards, "vcard"], "hidden");
			},
			filter: function(button, player) {
				var evt = _status.event.getParent();
			...
			check: function(button) {
				if (button.link[2] !== "jiu") return 10;
				return 1;
			},
			backup: function(links, player) {
				return {
			...
			prompt: function(links, player) {
				return "将一张黑桃牌当做【" + get.translation(links[0][2]) + "】使用（每轮每种类型限一次）";
			}
		},
		ai: {
			order: function() { return get.order({ name: "jiu" }) + 0.1; },
			result: { player: 1 },
			save: true
		},
		group: ["Isabeau_kamen_equip", "Isabeau_kamen_record"],
		subSkill: {
			record: {
				trigger: { player: ["useCard", "respond"] },
				forced: true, silent: true, charlotte: true,
				filter: function(event, player) {
					return event.skill && event.skill.indexOf("Isabeau_kamen") === 0 && event.cards && event.cards.length > 0;
				},
				content: function(event, trigger, player) {
					var type = get.type(trigger.cards[0]);
					if (type === 'delay') type = 'trick';
					player.markAuto("Isabeau_kamen_used", [type]);
					player.addTempSkill("Isabeau_kamen_clear", "roundStart");
				}
			},
			clear: {
				charlotte: true,
				onremove: function(player) {
					player.unmarkAuto("Isabeau_kamen_used", player.getStorage("Isabeau_kamen_used"));
				}
			},
			equip: {
				trigger: { player: "enterGame", global: "roundStart" },
				forced: true,
				filter: function(event, player) {
					if (event.name === "roundStart" && game.roundNumber !== 1) return false;
					return !player.storage.Isabeau_kamen_init;
				},
				content: async function(event, trigger, player) {
					player.storage.Isabeau_kamen_init = true;
					var x = player.hp;
					if (x > 0) {
						await player.draw(x);
						var next = player.chooseCard("h", 1, true, "假面：请选择一张手牌作为【蓄谋】牌");
						var res = await next.forResult();
						if (res.bool && res.cards && res.cards.length > 0) {
							await player.addJudge({ name: "xumou_jsrg" }, [res.cards[0]]);
						}
					}
					var maskName = "EnglandCrown";
					var pName = player.name || "";
					...
					if (lib.card[maskName]) {
						var maskCard = game.createCard(maskName, "spade", 3); 
						await player.equip(maskCard);
					}
				}
			}
		}
	...
	"Corbeau_kuangyan": {
		...
		filterCard: function(card, player) {
			var type = get.type(card);
			if (type === 'delay') type = 'trick';
			var usedTypes = player.getStorage('Corbeau_kuangyan_used') || [];
			return !usedTypes.includes(type);
		},
		selectCard: 1,
		position: "he",
		filterTarget: lib.filter.notMe,
		prompt: "狂宴：流失1点体力并弃置一张牌，对一名其他角色造成1点伤害（每轮每种类型限一次）",
		check: function(card) {
			var player = _status.event.player;
			var hasMask = player.hasCard(function(c){ return c.name === 'CrowMask'; }, 'e');
			if (hasMask) return 10 - get.value(card);
			return 7 - get.value(card); 
		},
		content: async function(event, trigger, player) {
			var type = get.type(event.cards[0]);
			if (type === 'delay') type = 'trick';
			player.markAuto("Corbeau_kuangyan_used", [type]);
			player.addTempSkill("Corbeau_kuangyan_clear", "roundStart");

			var loseEvt = player.loseHp(1);
			loseEvt.set('Corbeau_source', player); 

			await event.targets[0].damage(1, player);
		},
		ai: {
			order: 9, 
			result: {
				player: function(player, target) {
					var hasMask = player.hasCard(function(c){ return c.name === 'CrowMask'; }, 'e');
					if (!hasMask && player.hp <= 2 && !player.hasCard(function(c){ return c.name === 'tao' || c.name === 'jiu'; }, 'h')) {
						return -100;
					}
					return 0; 
				},
		...
		subSkill: {
			clear: {
				charlotte: true,
				onremove: function(player) {
					player.unmarkAuto("Corbeau_kuangyan_used", player.getStorage("Corbeau_kuangyan_used"));
				}
			}
		}
	},
	...
	"Corbeau_siwu": {
		...
		content: async function(event, trigger, player) {
			player.addTempSkill("Corbeau_siwu_round", "roundStart");
			await lib.skill.Corbeau_shizhan.removeHui(player, 4);

			var targetCard = event.cards[0];
			var tName = targetCard.name;
			var tSuit = get.suit(targetCard);
			
			await player.discard(targetCard);
			...
			if (!game.Corbeau_siwu_banned) game.Corbeau_siwu_banned = [];
			var alreadyBanned = game.Corbeau_siwu_banned.some(b => b.name === tName && b.suit === tSuit);
			if (!alreadyBanned) {
				game.Corbeau_siwu_banned.push({ name: tName, suit: tSuit });
			}

			var filterFn = function(card) {
				return get.name(card, false) === tName && get.suit(card, false) === tSuit;
			};

			var cards = ["cardPile", "discardPile"].map(pos => Array.from(ui[pos].childNodes)).flat();
			var cardx = cards.filter(filterFn);
			if (cardx.length) {
			...
			for (var target of game.filterPlayer()) {
				var hCards = target.getCards("hej", filterFn);
				if (hCards.length) {
					target.$throw(hCards, 1000);
					game.log(hCards, "被移出了游戏");
					await target.lose(hCards, ui.special);
				}
			}
	...
	"Boss_Corbeau_siwu": {
		...
		animationColor: "fire",
		filter: function(event, player) {
			return player.countMark("Corbeau_hui") >= 11;
		},
		content: function(event, trigger, player) {
			"step 0"
			game.broadcastAll(function() {
				if (ui.backgroundMusic) {
					ui.backgroundMusic.pause();
					ui.backgroundMusic.src = lib.assetURL + "extension/魔法纪录/audio/background/Boss_Corbeau.mp3";
					ui.backgroundMusic.loop = true;
					ui.backgroundMusic.play();
				}
			});
			player.$fullscreenpop("La Danse Macabre", "fire");
			if (typeof player.awakenSkill === "function") player.awakenSkill(event.skill);

			lib.skill.Corbeau_shizhan.removeHui(player, 11);
			game.Boss_Corbeau_siwu_count = (game.Boss_Corbeau_siwu_count || 0) + 1;

			event.allPlayers = game.filterPlayer();
			event.loseIndex = 0;
			game.log(player, "发动了", "#g【死舞】", "，令全场流失了 1 点体力！");
			"step 1"
			if (event.loseIndex < event.allPlayers.length) {
				var p = event.allPlayers[event.loseIndex];
				event.loseIndex++;
				var loseEvt = p.loseHp(1);
				loseEvt.set("Corbeau_source", player);
				event.redo();
			}
			"step 2"
			var banned = game.Boss_Corbeau_banned || [];
			var choices = ["heart", "diamond"].filter(function(s) { return !banned.includes(s); });
			
			if (choices.length === 0) {
				game.log("所有红色花色已被移出游戏！");
				event.finish();
			} else {
				player.chooseControl(choices).set("prompt", "死舞：请选择一种红色花色，将其移出游戏！").set("ai", function() {
					var p = _status.event.player;
					var choices = _status.event.controls;
					var enemies = game.filterPlayer(function(current) { return get.attitude(p, current) < 0; });
					if (enemies.some(function(e) { return e.hp <= 2; }) && choices.includes("heart")) return "heart";
					if (choices.includes("diamond")) return "diamond";
					return choices[0];
				});
			}
			"step 3"
			if (result && result.control) {
				var banSuit = result.control;
				game.log(player, "选择了将花色为", "#r【" + get.translation(banSuit) + "】", "的牌移出游戏！");

				if (!game.Boss_Corbeau_banned) game.Boss_Corbeau_banned = [];
				if (!game.Boss_Corbeau_banned.includes(banSuit)) game.Boss_Corbeau_banned.push(banSuit);

				event.filterFn = function(c) { return get.suit(c, false) === banSuit && get.color(c, false) === "red"; };

				var cards = ["cardPile", "discardPile"].map(function(pos) { return Array.from(ui[pos].childNodes); }).flat().filter(event.filterFn);
				if (cards.length > 0) {
			...
			"step 5"
			game.addGlobalSkill("Boss_Corbeau_siwu_banish");
			game.addGlobalSkill("Boss_Corbeau_siwangzhiwu");
			for (var i = 0; i < game.players.length; i++) { 
			    game.players[i].addSkill("Boss_Corbeau_siwangzhiwu"); 
			}
			game.log("黑死病笼罩全场，全局技能", "#p【死亡之舞】", "已生效！");

			if (game.Boss_Corbeau_siwu_count === 1) {
				if (typeof player.restoreSkill === "function") {
					player.restoreSkill("Boss_Corbeau_siwu");
				} else if (player.storage.awaken && player.storage.awaken.includes("Boss_Corbeau_siwu")) {
					player.storage.awaken.remove("Boss_Corbeau_siwu");
				}
				game.log(player, "由于是首次发动，重置了限定技", "#g【死舞】");
			}
		},
		subSkill: {
			banish: {
				trigger: { global: ["loseAfter", "equipAfter", "loseAsyncAfter", "cardsDiscardAfter"] },
				forced: true, silent: true, firstDo: true, popup: false,
				filter: function(event, player) {
					if (!game.Boss_Corbeau_banned || game.Boss_Corbeau_banned.length === 0) return false;
					return event.getd().some(function(c) { return get.position(c, true) === "d" && get.color(c, false) === "red" && game.Boss_Corbeau_banned.includes(get.suit(c, false)); });
				},
				content: async function(event, trigger, player) {
					var cards = trigger.getd().filter(function(c) { return get.position(c, true) === "d" && get.color(c, false) === "red" && game.Boss_Corbeau_banned.includes(get.suit(c, false)); });
					if (cards.length > 0) {
						await game.cardsGotoSpecial(cards);
						game.log(cards, "被移出了游戏");
					}
				},
				"skill_id": "Boss_Corbeau_siwu_banish",
				sub: true,
				sourceSkill: "Boss_Corbeau_siwu"
			}
		}
	},
	"Boss_Corbeau_siwangzhiwu": {
		audio: "ext:魔法纪录/audio/skill:2",
		mark: true, 
		marktext: "死",
		intro: {
			content: function(storage, player) {
				var count = game.Boss_Corbeau_siwu_count || 1;
				return "【死亡之舞】已被激活！已叠加 <span style='color:red'><b>" + count + "</b></span> 层。<br>回合结束时，将流失 <span style='color:red'><b>" + count + "</b></span> 点体力。";
			}
		},
		group: ["Boss_Corbeau_siwangzhiwu_lose", "Boss_Corbeau_siwangzhiwu_recover"],
		subSkill: {
			lose: {
				trigger: { player: "phaseJieshuBegin" },
				forced: true, charlotte: true,
				filter: function(event, player) {
					return !player.hasSkill("Isabeau_kamen") && !player.hasSkill("Boss_Corbeau_kamen");
				},
				content: function(event, trigger, player) {
					var count = game.Boss_Corbeau_siwu_count || 1;
					game.log(player, "受到了", "#g【死亡之舞】", "的影响，流失了 " + count + " 点体力");
					player.loseHp(count);
				}
			},
			recover: {
				...
				content: function(event, trigger, player) {
					var count = game.Boss_Corbeau_siwu_count || 1;
					game.log(player, "受到", "#y【假面】", "的庇护，在死亡之舞中回复了 " + count + " 点体力");
	...
	"Boss_Corbeau_kamen": {
		...
		enable: ["chooseToUse", "chooseToRespond"],
		filter: function(event, player) {
			var hasMask = player.hasCard(c => c.name === "CrowMask", 'e');
			if (hasMask) return false;

			var cards = player.getCards("he");
			var used = player.getStorage('Boss_Corbeau_kamen_used') || [];
			var canUse = false;
			for (var i = 0; i < cards.length; i++) {
				if (get.suit(cards[i]) !== 'spade') continue;
				var type = get.type(cards[i]);
		...
		chooseButton: {
			...
			check: function(button) { return 10; },
			backup: function(links, player) {
				return {
					audio: "ext:魔法纪录/audio/skill:2",
					filterCard: function(card, player) {
					...
					selectCard: 1, position: "he",
					viewAs: { name: links[0][2] }, popname: true, ai: { save: true } 
				};
			},
			prompt: function(links, player) {
				return "将一张黑桃牌当做【" + get.translation(links[0][2]) + "】使用（每轮每种类型限一次）";
		...
		group: ["Boss_Corbeau_kamen_equip", "Boss_Corbeau_kamen_record", "Boss_Corbeau_kamen_draw"],
		subSkill: {
			record: {
				trigger: { player: ["useCard", "respond"] },
				forced: true, silent: true, charlotte: true,
				filter: function(event, player) {
					return event.skill && event.skill.indexOf("Boss_Corbeau_kamen") === 0 && event.cards && event.cards.length > 0;
				},
				content: function(event, trigger, player) {
					var type = get.type(trigger.cards[0]);
					if (type === 'delay') type = 'trick';
					player.markAuto("Boss_Corbeau_kamen_used", [type]);
					player.addTempSkill("Boss_Corbeau_kamen_clear", "roundStart");
				}
			},
			clear: {
				charlotte: true,
				onremove: function(player) {
					player.unmarkAuto("Boss_Corbeau_kamen_used", player.getStorage("Boss_Corbeau_kamen_used"));
				}
			},
			equip: {
				trigger: { player: "enterGame", global: "roundStart" },
				...
				filter: function(event, player) {
					if (event.name === "roundStart" && game.roundNumber !== 1) return false;
					return !player.storage.Boss_Corbeau_kamen_init;
				},
				content: async function(event, trigger, player) {
					player.storage.Boss_Corbeau_kamen_init = true;
					var x = player.hujia || 0;
					if (x > 0) {
						await player.draw(x);
						var next = player.chooseCard("h", 1, true, "假面：请选择一张手牌作为【蓄谋】牌");
						var res = await next.forResult();
			...
			draw: {
				...
				filter: function(event, player) { return !event.numFixed; },
				content: function(event, trigger, player) {
					var currentHujia = player.hujia || 0;
					var currentHp = player.hp || 0;
					var diff = Math.abs(currentHujia - currentHp);
					var extra = Math.floor(diff / 2);
					if (extra > 0) {
						trigger.num += extra;
						game.log(player, "发动了", "#g【假面】", "，额外摸了", extra, "张牌");
					}
				}
			}
		}
	...
	"Minuo_liyin": {
		...
		trigger: { global: "phaseZhunbeiBegin" },
		filter: function(event, player) {
			return event.player !== player && !player.hasSkill("Minuo_liyin_used") && player.countCards("he") >= 2;
		},
		cost: async function(event, trigger, player) {
			var target = trigger.player;
	...
	"Minuo_zongjian": {
		...
		content: async function(event, trigger, player) {
			...
			if (hasValidCard) {
				var res = await player.chooseToDiscard("he", 1, "纵间：是否弃置一张花色类别均不同的牌，令 " + get.translation(target) + " 摸一牌并执行额外阶段？").set("filterCard", function(card) {
					var s = get.suit(card), t = get.type(card);
					if (t === 'delay') t = 'trick';
				...
				if (res.bool && res.cards && res.cards.length > 0) {
					...
					game.log(player, "挥动了九尾猫鞭，令", target, "摸了一张牌并于当前回合结束后执行额外的", "#y" + get.translation(pName));
		...
		group: "Minuo_zongjian_tracker",
		subSkill: {
			tracker: {
				trigger: { global: ["judgeAfter", "useCardAfter", "loseAfter", "cardsDiscardAfter"] },
				forced: true, silent: true, charlotte: true,
				filter: function(event, player) {
					if (event.name === "judge" && event.card && get.type(event.card) === "delay") return true; 
					if (event.name === "useCard") {
						if (event.card && (event.card.name === "jsrg_xvmou" || event.card.name === "xumou_jsrg")) return true;
						if (event.cards && event.cards.some(c => c.original === 'j')) return true;
					}
					if (event.name === "lose" || event.name === "cardsDiscard") {
						if (event.type === "use" || event.type === "respond") return false;
						if (event.cards && event.cards.some(c => c.original === 'j' || c.hasGaintag("jsrg_xvmou") || c.hasGaintag("xumou_jsrg"))) return true;
					}
					return false;
				},
				content: function(event, trigger, player) {
					var target = trigger.player || trigger.getParent().player;
					if (target) {
						target.addTempSkill("Minuo_zongjian_condition", "phaseAfter");
					}
				}
			...
			clear: {
				charlotte: true,
				onremove: function(player) {
					player.unmarkAuto("Minuo_zongjian_suits", player.getStorage("Minuo_zongjian_suits"));
					player.unmarkAuto("Minuo_zongjian_types", player.getStorage("Minuo_zongjian_types"));
					player.unmarkAuto("Minuo_zongjian_targets", player.getStorage("Minuo_zongjian_targets"));
				}
			},
			condition: { charlotte: true }
		}
	},
	...
	"Lapine_jiegui": {
		audio: "ext:魔法纪录/audio/skill:2",
		trigger: { player: ["damageEnd", "loseHpEnd", "recoverEnd"], global: "dying" },
		filter: function(event, player) {
			if (event.name === "dying") {
				var cards = player.getCards("hej").filter(c => lib.filter.cardDiscardable(c, player));
				if (cards.length < 2) return false;
				var usedNames = player.getStorage("Lapine_jiegui_names") || [];
				for (var i = 0; i < cards.length; i++) {
					for (var j = i + 1; j < cards.length; j++) {
		...
		content: async function(event, trigger, player) {
			if (trigger.name === "dying") {
				var target = trigger.player;
				var usedNames = player.getStorage('Lapine_jiegui_names') || [];
				var allCards = player.getCards("hej").filter(c => lib.filter.cardDiscardable(c, player));
				
				var res = await player.chooseCardButton(
					"劫归：弃置两个不同区域、花色类别均不同的牌挽救 " + get.translation(target) + " ？", 
					allCards, 
					[2, 2]
				).set("filterButton", function(button) {
					var card = button.link;
					if (_status.event.usedNames.includes(card.name)) return false;
					if (ui.selected.buttons.length === 0) return true;
					var c1 = ui.selected.buttons[0].link;
					if (get.position(c1, true) === get.position(card, true)) return false;
					if (get.suit(c1) === get.suit(card) && get.suit(c1) !== "none") return false;
					var t1 = get.type(c1) === 'delay' ? 'trick' : get.type(c1);
					var t2 = get.type(card) === 'delay' ? 'trick' : get.type(card);
					if (t1 === t2) return false;
					return true;
				}).set("usedNames", usedNames).set("ai", function(button) {
					var p = _status.event.player;
					var tgt = _status.event.getTrigger().player;
					if (p === tgt) return 10 - get.value(button.link);
					if (get.attitude(p, tgt) > 0) return 6 - get.value(button.link);
					return 0;
				}).forResult();

				if (res.bool && res.links && res.links.length === 2) {
					await player.discard(res.links);
					player.markAuto("Lapine_jiegui_names", [res.links[0].name, res.links[1].name]);
					player.addTempSkill("Lapine_jiegui_clear", "roundStart");
					await target.recover(1);
					if (target === player) {
						player.awakenSkill("Lapine_jiegui"); 
						await player.reinit(player.name, "WeepingHare");
						game.log(player, "陷入绝望！化身为了", "#y【哭泣兔的魔女】");
					}
				}
			} else {
				var res = await player.chooseBool("劫归：是否摸一张牌并进行一次【蓄谋】？").set("ai", () => true).forResult();
				if (res.bool) {
					await player.draw(1);
					var cardRes = await player.chooseCard("h", 1, true, "劫归：请选择一张手牌作为【蓄谋】牌").set("ai", c => 8 - get.value(c)).forResult();
			...
			effect: {
				target: function(card, player, target) {
					if (get.tag(card, "damage") >= 1) {
						...
						var usedXinve = target.getStorage("Lapine_xinve_names") || [];
						var hasXinve = target.hasSkill("Lapine_xinve") && (!card.name || !usedXinve.includes(card.name));
						var canJiegui = target.getCards("hej").length >= 2; 
						if (hasXinve && target.hp > 1) return [1, 1];
						if (canJiegui && target.hp <= get.tag(card, "damage")) return [1, 2];
					}
				}
			}
		},
		group: "Lapine_jiegui_clear",
		subSkill: {
			clear: {
				charlotte: true, trigger: { global: "roundStart" }, forced: true, silent: true,
				content: function(event, trigger, player) {
					player.unmarkAuto("Lapine_jiegui_names", player.getStorage("Lapine_jiegui_names"));
				}
			}
		}
	},
	...
	"Lapine_xinve": {
		...
		filter: function(event, player) {
			if (event.num === undefined || event.num <= 0) return false;
			var useEvt = event.getParent("useCard");
			var cardName = null;
			if (useEvt && useEvt.card && useEvt.card.name) cardName = useEvt.card.name;
			else if (event.cards && event.cards.length > 0 && event.cards[0].name) cardName = event.cards[0].name;
			
			if (cardName) {
				var usedNames = player.getStorage("Lapine_xinve_names") || [];
				if (usedNames.includes(cardName)) return false;
			}
			return true;
		},
		content: async function(event, trigger, player) {
			var useEvt = trigger.getParent("useCard");
			var cardsToGain = null;
			var cardName = null;

			if (useEvt && useEvt.card && useEvt.card.name) cardName = useEvt.card.name;
			else if (trigger.cards && trigger.cards.length > 0 && trigger.cards[0].name) cardName = trigger.cards[0].name;

			if (get.itemtype(trigger.cards) === "cards") cardsToGain = trigger.cards;
			else if (useEvt && get.itemtype(useEvt.cards) === "cards") cardsToGain = useEvt.cards;

			if (cardsToGain && cardsToGain.length > 0) {
				if (typeof cardsToGain.someInD === "function" && cardsToGain.someInD("od")) {
					await player.gain(cardsToGain.filterInD("od"), "gain2");
				} else if (cardsToGain[0] && (get.position(cardsToGain[0], true) === "o" || get.position(cardsToGain[0], true) === "d")) {
			...
			if (cardName) {
				player.markAuto("Lapine_xinve_names", [cardName]);
				player.addTempSkill("Lapine_xinve_clear", "roundStart");
			}
		},
		group: "Lapine_xinve_clear",
		subSkill: {
			clear: {
				charlotte: true, trigger: { global: "roundStart" }, forced: true, silent: true,
				content: function(event, trigger, player) {
					player.unmarkAuto("Lapine_xinve_names", player.getStorage("Lapine_xinve_names"));
				}
			}
		}
	},
	...
	"WeepingHare_jiegui": {
		...
		content: async function(event, trigger, player) {
			if (trigger.name === "dying") {
				...
				var usedNames = player.getStorage('Lapine_jiegui_names') || [];
				var allCards = player.getCards("hej").filter(c => lib.filter.cardDiscardable(c, player));
				var res = await player.chooseCardButton("劫归：弃置两个不同区域、花色类别均不同的牌挽救 " + get.translation(target) + " ？", allCards, [2, 2]).set("filterButton", function(button) {
					var card = button.link;
					if (_status.event.usedNames.includes(card.name)) return false;
					if (ui.selected.buttons.length === 0) return true;
					var c1 = ui.selected.buttons[0].link;
				...
				if (res.bool && res.links && res.links.length === 2) {
					await player.discard(res.links);
					player.markAuto("Lapine_jiegui_names", [res.links[0].name, res.links[1].name]);
					player.addTempSkill("WeepingHare_jiegui_clear", "roundStart");
					await target.recover(1);
					if (target === player) {
						player.awakenSkill("WeepingHare_jiegui"); 
						await player.reinit(player.name, "Lapine");
						game.log(player, "永劫回归！变回了", "#y【拉皮努】");
					}
				}
			} else {
				var res = await player.chooseBool("劫归：是否摸一张牌并进行一次【蓄谋】？").set("ai", () => true).forResult();
		...
		group: "WeepingHare_jiegui_clear",
		subSkill: {
			clear: {
				charlotte: true, trigger: { global: "roundStart" }, forced: true, silent: true,
				content: function(event, trigger, player) {
	...
	"WeepingHare_hebing": {
		...
		forced: true,
		filter: function(event, player) {
			return player.countCards("ej") > 0 || player.hp > 0;
		},
		content: async function(event, trigger, player) {
			var ejCards = player.getCards("ej");
			var maxEJ = ejCards.length;
			var maxHp = Math.max(0, player.hp); 
			var maxN = Math.max(maxEJ, maxHp);
			if (maxN < 1) maxN = 1;

			var othersTotalCards = 0;
			var otherPlayers = game.filterPlayer(p => p !== player && p.countCards("he") > 0);
			if (!otherPlayers.length) {
				if (player.getStat().skill) player.getStat().skill.WeepingHare_hebing--;
				return;
			}
			for (var op of otherPlayers) othersTotalCards += op.countCards("he");
			maxN = Math.min(maxN, othersTotalCards);

			var nChoices = [];
			for (var i = 1; i <= maxN; i++) nChoices.push(i.toString());

			var nRes = await player.chooseControl(nChoices).set("prompt", "阖兵：请选择本次分配的总点数").set("ai", function() {
				var p = _status.event.player;
				var ejs = p.getCards("ej");
				if (ejs.length > 0) return Math.min(_status.event.maxN, ejs.length).toString();
				if (p.hp >= 3) return "1";
				return "1";
			}).set("maxN", maxN).forResult();
			...
			var targetsRes = await player.chooseTarget("阖兵：请选择至多 " + N + " 名其他角色，随机获得共计 " + N + " 张牌", [1, N], function(card, p, target) {
				return target !== p && target.countCards("he") > 0;
			}).set("ai", function(target) {
				var p = _status.event.player;
				if (get.attitude(p, target) >= 0) return 0;
				return get.effect(target, { name: "shunshou_copy" }, p, p);
			}).forResult();

			if (!targetsRes.bool || !targetsRes.targets || targetsRes.targets.length === 0) {
				if (player.getStat().skill) player.getStat().skill.WeepingHare_hebing--;
				return;
			}
			...
			for (var i = 0; i < T; i++) {
				if (i === T - 1) {
					...
					var aRes = await player.chooseControl(allocChoices).set("prompt", "阖兵：请分配从 " + get.translation(targets[i]) + " 处获得的牌数（剩余待分配：" + remaining + "）").set("ai", function() {
			...
			var costMethods = [];
			if (maxEJ >= N) costMethods.push("弃置" + N + "张装备/判定牌");
			...
			var methodRes = await player.chooseControl(costMethods).set("prompt", "阖兵：请选择支付方式").set("ai", function() {
				var controls = _status.event.controls;
				if (controls[0].includes("装备/判定牌")) return controls[0];
				return controls[controls.length - 1];
			}).forResult();

			var method = methodRes.control || costMethods[0];

			if (method.includes("装备/判定牌")) {
				if (ejCards.length === N) {
					await player.discard(ejCards);
				} else {
					var discardRes = await player.choosePlayerCard(player, "ej", N, true, "阖兵：请选择并弃置 " + N + " 张装备区或判定区内的牌").set("ai", function(button) {
						var card = button.link;
						if (get.position(card) === "j") return 15;
						return 7 - get.value(card);
					}).forResult();
					...
					if (discardRes.bool && discardRes.links && discardRes.links.length > 0) {
						...
						await player.discard(ejCards.slice(0, N)); 
					}
				}
			} else {
			...
			for (var i = 0; i < targets.length; i++) {
				...
				for (var j = 0; j < takeCount; j++) {
					...
					if (type === "basic") {
						game.log(player, "获得的牌为", "#r基本牌", "，出【杀】次数+1");
						player.addTempSkill("WeepingHare_hebing_shaCount");
						player.storage.WeepingHare_hebing_shaCount = (player.storage.WeepingHare_hebing_shaCount || 0) + 1;
					} 
					else if (type === "trick") {
						game.log(player, "获得的牌为", "#y锦囊牌", "，额外摸一张牌");
						await player.draw(1);
					} 
					else if (type === "equip") {
						game.log("获得的牌为", "#g装备牌", "，", target, "的非锁定技失效直到其下个回合开始！");
		...
		subSkill: {
			...
			shaCount: {
				...
				mod: {
					cardUsable: function(card, player, num) {
						if (card.name === "sha") {
							return num + (player.storage.WeepingHare_hebing_shaCount || 0);
						}
					}
				},
				onremove: function(player) {
					delete player.storage.WeepingHare_hebing_shaCount;
				}
			},
			// 装备牌：标准封印模块
			fengyin: {
				...
				intro: {
					content: function(storage, player, skill) {
						var list = player.getSkills(null, false, false).filter(function(i) {
							return lib.skill.WeepingHare_hebing_fengyin.skillBlocker(i, player);
						});
						if (list.length) {
	...
	"Isabeau_duozhuan": {
		...
		chooseButton: {
			dialog: function (event, player) {
				...
				return ui.create.dialog("夺转：消耗因并弃置一张手牌，视为使用或打出该牌", [list, "vcard"], "hidden");
			},
			filter: function (button, player) {
				var vName = button.link[2];
				var vNature = button.link[3];
			...
			backup: function (links, player) {
				...
				return {
					...
					prompt: "消耗 " + nLen + " 枚【因】并弃置一张手牌，视为使用或打出【" + (vNature ? get.translation(vNature) : "") + get.translation(vName) + "】"
				};
			},
			prompt: function (links, player) {
				return "夺转：请选择要转化的牌名";
...
// pve挑战专属
	...
	"boss_corbeau_wave_control": {
		...
		filter: function (event, player) {
			...
			if (!_status.corbeau_level) return false;
		...
		content: function (event, trigger, player) {
			...
			next.setContent(lib.skill.boss_corbeau_wave_control.contentx);
		},
		contentx: function () {
			"step 0"
			game.delay(1.5);
			...
			if (_status.corbeau_level == 1) {
				_status.corbeau_level = 2;
				game.changeBoss("Corbeau"); 
				game.boss.dataset.position = 6;
				
				lib.skill.boss_corbeau_wave_control.clearEnemies();
				game.addBossFellow(4, "england_soldier");
				game.addBossFellow(5, ["elite_soldier"].randomGet()); 
				game.addBossFellow(7, ["FlècheLame"].randomGet());
				...
				game.broadcastAll(function() {
					if (ui.backgroundMusic) {
						ui.backgroundMusic.pause();
						ui.backgroundMusic.src = lib.assetURL + "extension/魔法纪录/audio/background/eng_soldier.mp3";
						ui.backgroundMusic.loop = true;
						ui.backgroundMusic.play();
					}
				});
				
			} else if (_status.corbeau_level == 2) {
				_status.corbeau_level = 3;
				_status.corbeau_wave3_round = 0; 
				
				game.changeBoss("Boss_Corbeau"); 
				game.boss.dataset.position = 5;
				lib.skill.boss_corbeau_wave_control.clearEnemies();
				game.addBossFellow(4, "Lapine");
				game.addBossFellow(6, "Minuo");
			}
			"step 2"
			game.arrangePlayers();
			lib.skill.boss_corbeau_wave_control.resetPlayers();
			...
			game.countPlayer(function(current) {
				if (current.side === game.boss.side) {
					...
					} else if (_status.corbeau_level === 3) {
						// 第三波：给 被虐的月夜乌、米诺、拉皮努 挂载
						if (current.name === "Boss_Corbeau" || current.name === "Minuo" || current.name === "Lapine") {
							current.addSkill("pve_jvli");
							current.addSkill("pve_mozhu1");
						}
					}
			...
			// 绝对不要直接篡改 _status.event！使用局部变量去探测
			var loopEvent = _status.event;
			while (loopEvent && loopEvent.name != "phaseLoop" && loopEvent.parent) {
				loopEvent = loopEvent.parent;
			}
			
			// 只有找到了 phaseLoop（证明游戏已经正式开始），再去重置回合
			if (loopEvent && loopEvent.name == "phaseLoop") {
				loopEvent.step = 0;
				loopEvent.player = game.boss;
			}
			// 无论 phaseLoop 是否存在，强行将下一次回合开始的起点锁定为 Boss
		...
		clearEnemies: function() {
			...
			for (var i = 0; i < dead.length; i++) {
				// 清除小怪尸体（保留 Boss 防止报错）
				if (dead[i].side == game.boss.side && dead[i] != game.boss) {
					dead[i].delete();
		...
		resetPlayers: function() {
			...
			var dead = game.dead.slice(0);
			for (var i = 0; i < dead.length; i++) {
				if (dead[i].side == game.me.side && dead[i].maxHp > 0) {
					dead[i].revive(dead[i].maxHp);
					dnum++;
				}
			}
			for (var i = 0; i < game.players.length; i++) {
				if (game.players[i].side == true) continue;
	...
	"boss_corbeau_retreat": {
		...
		popup: false,
		filter: function(event, player) {
			return _status.corbeau_level === 3 && player === game.boss && !_status.corbeau_retreated;
		},
		content: function(event, trigger, player) {
			_status.corbeau_wave3_round = (_status.corbeau_wave3_round || 0) + 1;
			if (_status.corbeau_wave3_round === 3) {
				_status.corbeau_retreated = true;
				
				game.log(player, "警告：", "#r“黑死病即将降临，可鲁波将全力以赴！”");
				player.$fullscreenpop("死亡降临", "fire");
				
				var targets = game.filterPlayer(function(current){
					return current.side == player.side && current != player && (current.name == "Minuo" || current.name == "Lapine" || current.name == "WeepingHare");
				});
				
				if (targets.length > 0) {
					for(var i = 0; i < targets.length; i++) {
						game.log(targets[i], "撤退了");
						targets[i].hide();
						game.addVideo('hidePlayer', targets[i]);
						targets[i].removed = true;
						targets[i].classList.add("dead");
						targets[i].remove();
						game.players.remove(targets[i]);
					}
				}
				...
				game.addGlobalSkill("Boss_Corbeau_siwangzhiwu");
				game.countPlayer(function(current){
					current.addSkill("Boss_Corbeau_siwangzhiwu");
				});
				game.log("黑死病笼罩全场，全局技能", "#p【死亡之舞】", "已生效！");
			}
		}
	...
	"boss_corbeau_fix_draw": {
		...
		filter: function(event, player) {
			// 只在第一波发牌结束后触发一次
			return _status.corbeau_level === 1;
		},
		content: function(event, trigger, player) {
			// 筛选出 Boss 阵营的小怪
			var minions = game.filterPlayer(function(current) {
				return current != game.boss && current.side == game.boss.side;
			});
			
			minions.forEach(function(current) {
				var cards = current.getCards('h');
				if (cards.length > 4) {
					// 剥夺多出的4张牌
					var toLose = cards.slice(0, cards.length - 4);
					current.$throw(toLose, 1000);
					var evt = current.lose(toLose, ui.discardPile);
					evt._triggered = null; 
				}
			});
			
			game.removeGlobalSkill("boss_corbeau_fix_draw");
		}
	},
	...
	"pve_zhanxun": {
		...
		intro: {
			...
			name2: "战勋",
			content: function(storage, player) {
				let marks = player.countMark("pve_zhanxun");
				let str = "当前拥有 " + marks + " 点战功。<br>";
				if (marks >= 10) str += "【20点】手牌上限+1<br>";
				if (marks >= 20) str += "【40点】攻击距离+1<br>";
				if (marks >= 30) str += "【60点】摸牌阶段多摸一张牌<br>";
				if (marks >= 40) str += "【80点】体力上限+1并回复一点体力<br>";
				if (marks >= 50) str += "【100点】出杀上限+1<br>";
				if (marks >= 60) str += "【120点】结束阶段摸一张牌";
				return str;
			}
		},
		addMark: function(player, num) {
			let current = player.countMark("pve_zhanxun");
			let toAdd = Math.min(num, 120 - current);
			if(toAdd > 0) {
				player.addMark("pve_zhanxun", toAdd);
				if (current < 40 && current + toAdd >= 40) {
					player.gainMaxHp(1);
					player.recover(1);
					game.log(player, "战功达到了40点，体力上限+1并回复一点体力！");
				}
			}
		},
		mod: {
			maxHandcard: function(player, num) {
				if (player.countMark("pve_zhanxun") >= 20) return num + 1;
			},
			attackFrom: function(from, to, distance) {
				if (from.countMark("pve_zhanxun") >= 40) return distance - 1;
			},
			cardUsable: function(card, player, num) {
				if (card.name == "sha" && player.countMark("pve_zhanxun") >= 100) return num + 1;
			}
		},
		trigger: { player: ["pve_zhanxun_beginAfter", "pve_zhanxun_addAfter"] },
		filter: function(event, player) { return false; },
		forced: true,
		popup: false,
		locked: false,
		group: ["pve_zhanxun_begin", "pve_zhanxun_add", "pve_zhanxun_draw", "pve_zhanxun_end"],
		subSkill: {
			begin: {
				audio: "pve_zhanxun",
				persevereSkill: true,
				trigger: { global: "phaseBefore", player: "enterGame" },
				filter: function(event, player) {
					return event.name != "phase" || game.phaseNumber == 5;
				},
				forced: true, locked: false,
				content: async function(event, trigger, player) {
					lib.skill.pve_zhanxun.addMark(player, 0);
				},
				"skill_id": "pve_zhanxun_begin",
				sub: true, sourceSkill: "pve_zhanxun", "_priority": 0
			},
			add: {
				audio: "pve_zhanxun",
				persevereSkill: true,
				...
				filter: function(event, player) {
					if (player.countMark("pve_zhanxun") >= 120) return false;
					if (event.name === "damage") return event.num > 0;
					if (event.name === "die") return event.source === player;
					return event.getg && event.getg(player).length > 0;
				},
				...
				content: async function(event, trigger, player) {
					let toAdd = 0;
					if (event.triggername === "gainAfter") toAdd = 2;
					else if (event.triggername === "useCardAfter") toAdd = 1;
					else if (event.triggername === "damageEnd") toAdd = 3;
					else if (event.triggername === "damageSource") toAdd = 4;
					else if (event.triggername === "dieAfter") toAdd = 10;
					
					lib.skill.pve_zhanxun.addMark(player, toAdd);
				},
				"skill_id": "pve_zhanxun_add",
				sub: true, sourceSkill: "pve_zhanxun", "_priority": 0
			},
			draw: {
				persevereSkill: true,
				trigger: { player: "phaseDrawBegin2" },
				filter: function(event, player) {
					return !event.numFixed && player.countMark("pve_zhanxun") >= 60;
				},
				forced: true,
				content: async function(event, trigger, player) {
					trigger.num++;
				...
				trigger: { player: "phaseJieshuBegin" },
				filter: function(event, player) {
					return player.countMark("pve_zhanxun") >= 120;
				},
				forced: true,
				content: async function(event, trigger, player) {
					player.draw(1);
	...
	"pve_skill_overview": {
		...
		intro: {
			...
			name2: "技能总览",
			content: function(storage, player) {
				var skills = player.getSkills("invisible", true, false).filter(function(skill, index, list) {
					if (skill == "pve_skill_overview" || list.indexOf(skill) != index) return false;
					if (!lib.skill[skill] || !lib.translate[skill]) return false;
					return !!lib.translate[skill + "_info"] || !!(lib.dynamicTranslate && lib.dynamicTranslate[skill]);
				});
				if (!skills.length) return "当前没有可显示的技能。";
				return skills.map(function(skill) {
					var info = get.skillInfoTranslation ? get.skillInfoTranslation(skill, player, false) : lib.translate[skill + "_info"];
					return '<div class="skill">【' + get.translation(skill) + '】</div><div>' + info + '</div>';
				}).join('<div style="height:8px"></div>');
			}
		}
	},
	"boss_corbeau_grant_zhanxun": {
		trigger: { global: "roundStart" },
		forced: true,
		filter: function (event, player) {
			return game.roundNumber <= 1 && player === game.boss;
		},
		silent: true,
		charlotte: true,
		popup: false,
	...
	"eng_huifeng": {
		...
		prompt: "将一张黑色牌当无距离限制的【杀】使用或打出",
		check: function(card) {
			return 4 - get.value(card);
		},
		mod: {
			targetInRange: function(card, player, target) {
				if (card && card.skill == "eng_huifeng") return true;
				if (_status.event && _status.event.skill == "eng_huifeng") return true;
			}
		},
		ai: {
			respondSha: true,
			order: function() { return get.order({name: "sha"}) + 0.1; },
			result: { 
				player: 0, 
				target: function(player, target) {
		...
		subSkill: {
			discard: {
				...
				forced: true,
				filter: function(event, player) {
					return event.card && event.card.name == "sha" && event.player.countCards("he") > 0 && !player.hasSkill("eng_huifeng_used");
				},
				content: async function(event, trigger, player) {
					player.addTempSkill("eng_huifeng_used", "roundStart");
					await trigger.player.chooseToDiscard("he", 1, true).set("prompt", "晦锋：请弃置一张牌");
				}
			},
			used: { charlotte: true }
		}
	...
	"eng_dianwei": {
		...
		trigger: { global: "phaseJieshuBegin" },
		filter: function(event, player) {
			var hpChangedCount = player.storage.eng_dianwei_hp_changed || 0;
			if (hpChangedCount >= 2) return true;

			var history = player.getHistory("useCard").concat(player.getHistory("respond"));
			history.sort(function(a, b) { return a.time - b.time; });
			
			var consecutiveBasicCount = 0;
			for (var i = 0; i < history.length; i++) {
				var record = history[i];
				if (record.card && get.type(record.card) == "basic") {
					consecutiveBasicCount++;
					if (consecutiveBasicCount >= 2) return true;
				} else {
		...
		content: async function(event, trigger, player) {
			...
			var choices = ["摸一张牌"];
			if (target.isAlive() && target != player) {
				choices.push("令其摸牌并视为对其决斗");
			}
			choices.push("取消");

			var res = await player.chooseControl(choices).set("prompt", "典卫：请选择一项").set("ai", function() {
				var p = _status.event.player;
				var t = _status.event.target;
				// 【AI 核心逻辑】：敌意够深，且自己手里的【杀】不虚对手，果断选择决斗碰瓷！
				if (_status.event.choices.includes("令其摸牌并视为对其决斗") && get.attitude(p, t) < 0 && p.countCards("h", "sha") >= t.countCards("h", "sha")) {
					return "令其摸牌并视为对其决斗";
				}
				return "摸一张牌";
			}).set("target", target).set("choices", choices).forResult();

			if (res.control == "摸一张牌") {
				await player.draw(1);
			} else if (res.control == "令其摸牌并视为对其决斗") {
				await target.draw(1);
			...
			aiOrder: function(player, card, num) {
				if (_status.currentPhase == player) {
					if (get.type(card) == "basic") {
						return num + 5; 
					} else if (player.hasCard(function(c){ return get.type(c) == "basic" && player.hasValueTarget(c); }, "h")) {
						return Math.max(0.1, num - 5); 
					}
				}
			}
		},
		group: ["eng_dianwei_hp_tracker", "eng_dianwei_clear"],
		subSkill: {
			hp_tracker: {
				trigger: { global: ["damageEnd", "recoverEnd"] },
				forced: true,
				...
				content: function(event, trigger, player) {
					if (!player.storage.eng_dianwei_hp_changed) player.storage.eng_dianwei_hp_changed = 0;
					player.storage.eng_dianwei_hp_changed++;
				}
			},
			clear: {
				...
				silent: true,
				content: function(event, trigger, player) {
					player.storage.eng_dianwei_hp_changed = 0;
				}
			}
		}
	},
	...
	"elite_yuanzhan": {
		...
		selectCard: [1, Infinity],
		position: "h",
		prompt: "将任意张手牌交给一名其他角色。若不少于两张，你摸一张牌且其下个出牌阶段内使用【杀】的次数上限+1",
		check: function(card) {
			var player = _status.event.player;
			if (card.name == "sha" && player.countCards("h", "sha") <= 1 && player.hasSkill("elite_weijiao")) {
				return -1;
			}
			return 6 - get.value(card);
		},
		content: async function(event, trigger, player) {
			...
			if (num >= 2) {
				await player.draw(1);
				target.addSkill("elite_yuanzhan_buff");
				game.log(target, "获得了", "#y【援战】", "的加成，下个出牌阶段使用【杀】的次数上限+1");
			}
		},
		ai: {
			order: 8,
		...
		subSkill: {
			buff: {
				charlotte: true,
				mark: true,
				intro: { content: "下个出牌阶段使用【杀】的次数上限+1" },
				mod: {
					cardUsable: function(card, player, num) {
						if (card.name == "sha") return num + 1;
					}
				...
				silent: true,
				content: function(event, trigger, player) {
					player.removeSkill("elite_yuanzhan_buff");
				}
			}
		}
	},
	...
	"elite_weijiao": {
		...
		trigger: { global: "damageEnd" },
		filter: function(event, player) {
			if (player.hasSkill("elite_weijiao_used")) return false;
			if (!event.card || event.card.name != "sha") return false;
			if (!event.source || event.source == player) return false;
			if (get.attitude(player, event.source) <= 0) return false; 
			if (get.attitude(player, event.player) >= 0) return false; 
			if (!event.player.isAlive()) return false;
			return player.hasCard(function(c){ return c.name == "sha" && !c.isVCard; }, "hs");
		},
		content: async function(event, trigger, player) {
			player.addTempSkill("elite_weijiao_used", "roundStart");
			player.addTempSkill("elite_weijiao_dist", "useCardAfter"); 
			
			var next = player.chooseToUse(function(card, p){
				return card.name == "sha" && !card.isVCard; 
			}, "围剿：是否对 " + get.translation(trigger.player) + " 使用一张无距离限制的实体【杀】？");
			
			next.set("targetRequired", true);
			next.set("filterTarget", function(card, p, target){
				return target == _status.event.dmgTarget; 
			});
			next.set("dmgTarget", trigger.player);
			next.set("ai", function(card){
				var target = _status.event.dmgTarget;
				return get.effect(target, card, _status.event.player, _status.event.player);
			});
			
			var res = await next.forResult();
			if (!res.bool) {
				player.removeSkill("elite_weijiao_dist");
			}
		},
		subSkill: {
			used: { charlotte: true },
	...
	"flame_lishi": {
		...
		cost: async function(event, trigger, player) {
			...
			var res = await player.chooseControl(choices).set("prompt", "离矢：请选择一项强化此【杀】").set("ai", function() {
				var p = _status.event.player;
				var t = _status.event.target;
				if (get.attitude(p, t) < 0) {
					if (t.countCards("h", "shan") > 0 || t.hp == 1) return "令此【杀】不可被响应";
					return "造成伤害后令友方视为对其使用【杀】";
				}
				return "cancel2";
			}).set("target", target).forResult();

			if (res.control && res.control != "cancel2") {
				event.result = { bool: true, cost_data: res.control };
			}
		},
		content: async function(event, trigger, player) {
			...
			if (choice == "令此【杀】不可被响应") {
				if (!trigger.getParent().directHit) trigger.getParent().directHit = [];
				trigger.getParent().directHit.add(target);
				game.log(trigger.card, "不可被响应");
			} else if (choice == "造成伤害后令友方视为对其使用【杀】") {
				player.addTempSkill("flame_lishi_followUp");
				player.storage.flame_lishi_target = target;
			}
		},
		subSkill: {
			followUp: {
				...
				forced: true,
				filter: function(event, player) {
					return player.storage.flame_lishi_target && event.player == player.storage.flame_lishi_target && event.card && event.card.name == "sha";
				},
				content: async function(event, trigger, player) {
					var target = player.storage.flame_lishi_target;
					delete player.storage.flame_lishi_target;
					player.removeSkill("flame_lishi_followUp");

					if (!target.isAlive()) return; 

					var friends = game.filterPlayer(function(current) {
						return current.isAlive() && get.attitude(player, current) > 0;
					});
					if (friends.length === 0) return;

					var res = await player.chooseTarget("离矢：请选择一名友方角色，视为对 " + get.translation(target) + " 使用一张【杀】", function(card, p, t) {
						return _status.event.friends.includes(t);
					}).set("friends", friends).set("ai", function(t) {
						var att = get.attitude(_status.event.player, t);
						if (att <= 0) return 0;
						if (t.name.indexOf("Corbeau") !== -1) return att + 20; 
						return att;
					}).forResult();

					if (res.bool && res.targets.length) {
						var friend = res.targets[0];
						game.log(friend, "由于", player, "的", "#g【离矢】", "，视为对", target, "使用了一张", "#y【杀】");
						await friend.useCard({ name: "sha", isCard: true }, target, false);
					}
				}
			}
		}
	...
	"flame_weiren": {
		...
		filter: function(event, player) {
			var cards = player.getCards("he");
			var colors = { red: 0, black: 0, none: 0 };
			for (var i = 0; i < cards.length; i++) {
				colors[get.color(cards[i])]++;
			}
			return colors.red >= 2 || colors.black >= 2 || colors.none >= 2;
		},
		filterTarget: function(card, player, target) {
			return target != player && get.attitude(player, target) > 0;
		},
		...
		selectCard: 2, 
		position: "he",
		prompt: "卫仞：交给一名友方角色两张同色牌并摸两张牌，你与其各获得一枚【仞】标记",
		check: function(card) { return 6 - get.value(card); },
		content: async function(event, trigger, player) {
			var target = event.targets[0];
			await player.give(event.cards, target);
			...
			player.addMark("flame_ren_mark", 1, false);
			target.addMark("flame_ren_mark", 1, false);
			game.log(player, "与", target, "各获得了 1 枚", "#y【仞】标记");
			
			player.addSkill("flame_weiren_clear");
			if (!player.storage.flame_weiren_targets) player.storage.flame_weiren_targets = [];
			player.storage.flame_weiren_targets.push({ p: player, t: target, duration: 2 });
		},
		ai: {
			order: 9.5, 
			result: { 
				...
				target: function(player, target) { 
					var val = 1;
					if (target.name.indexOf("Corbeau") !== -1) val += 10;
					if (!target.countMark("flame_ren_mark")) val += 5;
					return val; 
				} 
			}
		},
		group: ["flame_weiren_armor", "flame_weiren_trigger"],
		subSkill: {
			armor: {
				trigger: { source: "damageEnd" },
				forced: true,
				...
				content: async function(event, trigger, player) {
					if (!player.storage.flame_weiren_damageCount) player.storage.flame_weiren_damageCount = 0;
					player.storage.flame_weiren_damageCount += trigger.num;

					while (player.storage.flame_weiren_damageCount >= 2) {
						player.storage.flame_weiren_damageCount -= 2;
						var res = await player.chooseTarget("卫仞：已累计造成2点【杀】伤害，请选择一名角色获得1点护甲", true).set("ai", function(target) {
							var att = get.attitude(_status.event.player, target);
							if (att <= 0) return 0;
							var val = att;
							if (target.hp <= 2) val += 15;
							if (target.name.indexOf("Corbeau") !== -1) val += 10;
							return val;
						}).forResult();
						...
						if (res.bool && res.targets.length) {
							var t = res.targets[0];
							await t.changeHujia(1);
							game.log(t, "获得了", "#g1点护甲");
						}
					}
				}
			},
			clear: {
				...
				charlotte: true,
				filter: function(event, player) {
					return player.storage.flame_weiren_targets && player.storage.flame_weiren_targets.length > 0;
				},
				content: function(event, trigger, player) {
					var keeps = [];
					for (var obj of player.storage.flame_weiren_targets) {
						obj.duration -= 1;
						if (obj.duration <= 0) {
							if (obj.p.countMark("flame_ren_mark") > 0) obj.p.removeMark("flame_ren_mark", 1);
							if (obj.t.countMark("flame_ren_mark") > 0) obj.t.removeMark("flame_ren_mark", 1);
						} else {
							keeps.push(obj);
						}
					}
					player.storage.flame_weiren_targets = keeps;
					if (keeps.length === 0) player.removeSkill("flame_weiren_clear");
				}
			}
		}
	},
	...
	"flame_weiren_trigger": {
		charlotte: true,
		trigger: { global: ["damageEnd", "damageSource"] },
		filter: function(event, player) {
			var target = (event.name == "damageSource") ? event.source : event.player;
			if (!target || !target.countMark("flame_ren_mark")) return false;

			if (event.name == "damageSource") {
				return !target.hasSkill("flame_weiren_deal_used");
			} else {
				return !target.hasSkill("flame_weiren_take_used");
			}
		},
		forced: true,
		popup: false,
		content: async function(event, trigger, player) {
			var isDealing = (trigger.name == "damageSource");
			var renChar = isDealing ? trigger.source : trigger.player;

			if (isDealing) renChar.addTempSkill("flame_weiren_deal_used", "roundStart");
			else renChar.addTempSkill("flame_weiren_take_used", "roundStart");

			renChar.popup("卫仞", "fire");
			game.log(renChar, "触发了", "#y【卫仞】", "！");

			await renChar.damage("unreal", 1, "nosource");

			var damageCard = null;
			if (ui.discardPile) {
				var discards = ui.discardPile.childNodes;
				for (var i = 0; i < discards.length; i++) {
					if (get.tag(discards[i], "damage") || discards[i].name == "sha" || discards[i].name == "juedou") {
						damageCard = discards[i];
						break;
					}
				}
			}
			if (damageCard) {
				await renChar.gain(damageCard, "gain2");
				game.log(renChar, "从弃牌堆获得了伤害牌", damageCard);
			}
		}
	},
	"flame_ren_mark": {
		...
		intro: {
			name: "仞",
			content: "标记数：#。每回合首次，拥有【仞】的角色造成或受到伤害时，视为对其造成一点虚拟伤害，然后其从弃牌堆中获得一张伤害牌。"
		}
	},
	"flame_weiren_deal_used": { charlotte: true },
	"flame_weiren_take_used": { charlotte: true },

	"flame_jiou": {
		audio: "ext:魔法纪录/audio/skill:2",
		forced: true,
		mod: {
			globalFrom: function(from, to, current) { return current - 1; },
			globalTo: function(from, to, current) { return current + 1; }
		},
		group: ["flame_jiou_damage", "flame_jiou_shield", "flame_jiou_eyu"],
		subSkill: {
			damage: {
				trigger: { global: "damageEnd" },
				filter: function(event, player) {
					if (player.hasSkill("flame_jiou_used")) return false; 
					return (event.unreal || event.nature === "unreal") && event.player && event.player.isAlive() && get.attitude(player, event.player) > 0;
				},
				forced: true,
				async content(event, trigger, player) {
					player.addTempSkill("flame_jiou_used", "roundStart");
					player.logSkill("flame_jiou", trigger.player);
					var targets = (player == trigger.player) ? [player] : [player, trigger.player];
			...
			shield: {
				...
				filter: function(event, player) {
					if (player.hasSkill("flame_jiou_used")) return false; 
					return event.num < 0 && event.player && event.player.isAlive() && get.attitude(player, event.player) > 0;
				},
				forced: true,
				async content(event, trigger, player) {
					player.addTempSkill("flame_jiou_used", "roundStart");
			...
			eyu: {
				trigger: { global: "flame_prevent_lose" },
				filter: function(event, player) {
					if (player.hasSkill("flame_jiou_used")) return false; 
					return event.player && event.player.isAlive() && get.attitude(player, event.player) > 0;
				},
				forced: true,
				async content(event, trigger, player) {
					player.addTempSkill("flame_jiou_used", "roundStart");
					var eyuPlayer = trigger.player;
					player.logSkill("flame_jiou", eyuPlayer);
					var targets = (player == eyuPlayer) ? [player] : [player, eyuPlayer];
					await game.asyncDraw(targets, 1);
				}
			},
			used: { charlotte: true }
